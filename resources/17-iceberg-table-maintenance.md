...
3. Run **four core** procedures: compaction (nightly), snapshot expiry (weekly), orphan-file sweep (weekly), manifest rewrite (weekly). On MoR tables with accumulating position deletes, optionally add a **fifth** procedure: *delete-file-only* compaction via Spark `rewrite_position_delete_files` (that specific PROCEDURE is Spark-only — Trino 467 does NOT have it, [trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)). **IMPORTANT — a Trino-only shop is NOT stuck:** clearing the position-delete *read overhead* does NOT require Spark, because the nightly Trino `EXECUTE optimize` already rewrites data files and **applies + drops the position deletes** for the files it rewrites ([#12617](https://github.com/trinodb/trino/issues/12617)/[#24086](https://github.com/trinodb/trino/issues/24086)); if your delete-bearing data files are already large, raise `file_size_threshold` above their size to force the rewrite. The Spark fifth procedure is only the *cheaper delete-file-only* path, not a prerequisite. See §1b.
4. **Each maintenance operation has a DIFFERENT API surface in Trino vs Spark — read the engine split FIRST, then the canonical order below. Do NOT conflate the two.**
   ...
   **Trino-EXECUTE-vs-Spark-CALL split (the load-bearing distinction — memorize this BEFORE writing any procedure call):**
   ...
   | Snapshot expiry | `ALTER TABLE iceberg.<schema>.<table> EXECUTE expire_snapshots(retention_threshold => '7d')`  <!-- Trino: ALTER TABLE ... EXECUTE; NOT `EXECUTE PROCEDURE` (prepared-statement clause only), NOT `CALL` (CALL for expire_snapshots is Spark-only — but CALL rollback_to_snapshot IS native Trino 467; see PIN below) --> | `CALL iceberg.system.expire_snapshots(table => '<schema>.<table>', older_than => current_timestamp - interval '7' day, retain_last => 10)` |
   | Orphan-file sweep | `ALTER TABLE iceberg.<schema>.<table> EXECUTE remove_orphan_files(retention_threshold => '7d')`  <!-- Trino: ALTER TABLE ... EXECUTE; NOT `EXECUTE PROCEDURE`, NOT `CALL` (CALL for remove_orphan_files is Spark-only — but CALL rollback_to_snapshot IS native Trino 467; see PIN below) --> | `CALL iceberg.system.remove_orphan_files(table => '<schema>.<table>', older_than => current_timestamp - interval '3' day, dry_run => true)` |
   ...
   | Position-delete handling (MoR) | **The *delete-file-only* compaction procedure (`rewrite_position_delete_files`) is NOT available on Trino 467** ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)). **BUT Trino `EXECUTE optimize` DOES clear position-delete read overhead** — it rewrites data files and applies+drops the deletes referencing them ([#12617](https://github.com/trinodb/trino/issues/12617)/[#24086](https://github.com/trinodb/trino/issues/24086)); raise `file_size_threshold` above already-large delete-bearing files to force the rewrite. A Trino-only shop is NOT stuck. | `ALTER TABLE ... EXECUTE optimize(file_size_threshold => '512MB')` (clears the overhead, Trino-native). Optional cheaper delete-file-only path: Spark `CALL iceberg.system.rewrite_position_delete_files(table => '<schema>.<table>')` |
   ...
   **Engine-split rules:** Trino's API surface is `ALTER TABLE ... EXECUTE <proc>(...)`; Spark's is `CALL iceberg.system.<proc>(table => ..., ...)`. The procedure NAMES are different across engines for the same underlying Iceberg operation — `EXECUTE optimize` (Trino) commits the same kind of `replace` snapshot as `CALL iceberg.system.rewrite_data_files(...)` (Spark), but the SQL clause and procedure name are engine-specific. **Do NOT write `ALTER TABLE ... EXECUTE rewrite_data_files(...)` on Trino 467 — that's Spark CALL syntax pasted into a Trino EXECUTE form; Trino's procedure registry does NOT include `rewrite_data_files` and the call fails with `Procedure not registered`.** Likewise, do NOT claim "Spark is required for `expire_snapshots`" — `expire_snapshots` IS a Trino 467 EXECUTE procedure (with `retention_threshold` arg); Spark is required only for `rewrite_manifests`, `rewrite_position_delete_files`, `fast_forward`, sub-7-day expiry (Trino's min-retention floor), and Spark-only options like `retain_last` on `expire_snapshots`. See the full [§ Trino EXECUTE procedures vs Spark CALL procedures](#trino-execute-procedures-vs-spark-call-procedures--the-engine-confusion-disambiguation-matrix-read-before-writing-any-procedure-call) disambiguation matrix below.
   ...
   > **PIN (iter722) — `CALL` is NOT "Spark-only." ROLLBACK is the exception to the EXECUTE-vs-CALL split above.** Keywords: rollback Iceberg table Trino, restore previous snapshot, roll back to a snapshot in Trino, rollback_to_snapshot Trino vs Spark, we're on Trino not Spark rollback, is CALL Spark or Trino. The "`CALL` is Spark" inline notes on the **expire_snapshots / remove_orphan_files / rewrite_manifests** rows above are scoped to THOSE FOUR ROUTINE procedures only — they do NOT generalize. **`CALL <catalog>.system.rollback_to_snapshot('schema','table',snapshot_id)` is the NATIVE Trino 467 rollback form** (TWO separate string args: schema, then table; then a BIGINT snapshot_id). It is **NOT Spark-only — this IS how you roll back in Trino 467** (verified at [trino.io/docs/467/connector/iceberg.html](https://trino.io/docs/467/connector/iceberg.html): `CALL example.system.rollback_to_snapshot('testdb', 'customer_orders', 8954597067493422955)`). Do NOT label the `CALL` rollback form "(Spark)" — on Trino 467 it is the ONLY rollback form. `ALTER TABLE t EXECUTE rollback_to_snapshot(<id>)` is **Trino 469+** (does NOT exist on 467 — `ALTER TABLE ... EXECUTE` lists only `optimize`, `expire_snapshots`, `remove_orphan_files`, `drop_extended_stats` on 467). **Spark** uses a single COMBINED `'db.table'` string arg: `CALL iceberg.system.rollback_to_snapshot('analytics.events', <id>)` (two args, not three). To find the `snapshot_id` to roll back to, read it from `$history` (`made_current_at`, `snapshot_id`, `parent_id`, `is_current_ancestor` — filter `is_current_ancestor = true` for safe targets) or `$snapshots` (`committed_at`, `snapshot_id`, `parent_id`, `operation`, `summary`) — see the `$history` vs `$snapshots` router below. (`register_table` is the only other `CALL iceberg.system.*` procedure that is native to BOTH Trino 467 and Spark.)

   **DO-NOT-WRITE — the three banned `expire_snapshots` / `remove_orphan_files` invocation forms (Trino 467, iter542 Q3 regression-cure).** _Keyword anchors: Trino expire_snapshots syntax, cleanup old snapshots Trino, remove_orphan_files Trino, EXECUTE PROCEDURE invalid, Spark CALL vs Trino EXECUTE, Iceberg maintenance procedure Trino._
   - BANNED: `EXECUTE PROCEDURE iceberg.system.expire_snapshots(table => '...', retention_threshold => '7d')` — **INVALID Trino 467 syntax.** Trino's `EXECUTE` keyword is for **prepared statements only** (`EXECUTE <stmt_name> USING ...`); there is no `EXECUTE PROCEDURE` clause. Pasting this into Trino gives `mismatched input 'PROCEDURE'. Expecting: 'USING', <EOF>`. This form is a hallucinated third hybrid that exists in NEITHER Trino NOR Spark. There is no `EXECUTE PROCEDURE` anywhere in Trino's grammar.
   - BANNED as Trino: `CALL iceberg.system.expire_snapshots(...)` / `CALL iceberg.system.remove_orphan_files(...)` — that is the **SPARK** form. Trino does not implement `CALL` for the Iceberg connector; pasting it gives `Procedure not registered`. The same SQL works under `spark-sql` / `spark-shell` against Spark's Iceberg `SparkCatalog`.
   - THE ONE TRUE TRINO FORM (verbatim, copy these): `ALTER TABLE iceberg.analytics.tbl EXECUTE expire_snapshots(retention_threshold => '7d')` AND `ALTER TABLE iceberg.analytics.tbl EXECUTE remove_orphan_files(retention_threshold => '7d')`. Verified at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html): _"ALTER TABLE test_table EXECUTE expire_snapshots(retention_threshold => '7d');"_ and _"ALTER TABLE test_table EXECUTE remove_orphan_files(retention_threshold => '7d');"_

   **COMMENT ON TABLE / COLUMN — the standalone Trino DDL for adding/changing/removing a table or column description (iter542 Q4 findability gap-cure).** _Keyword anchors: add table comment Trino, COMMENT ON TABLE, table description Trino, update table comment, COMMENT ON COLUMN, remove table comment, set table description._
   Trino's full `COMMENT` grammar is `COMMENT ON ( TABLE | VIEW | COLUMN ) name IS 'string'` ([trino.io/docs/current/sql/comment.html](https://trino.io/docs/current/sql/comment.html), quoting: _"COMMENT ON TABLE users IS 'master table';"_, _"COMMENT ON COLUMN users.name IS 'full name';"_, _"COMMENT ON VIEW users IS 'master view';"_, and _"The comment can be removed by setting the comment to NULL."_).
   ```sql
   -- Set / replace a table comment (Trino 467, runs against the Iceberg connector natively).
   COMMENT ON TABLE iceberg.analytics.fct_orders IS 'Order facts, grain = one row per line item';

   -- Set / replace a column comment.
   COMMENT ON COLUMN iceberg.analytics.fct_orders.order_id IS 'Surrogate key; not the Postgres OLTP id';

   -- Remove a comment entirely (sets it back to NULL).
   COMMENT ON TABLE iceberg.analytics.fct_orders IS NULL;

   -- Verify: SHOW CREATE TABLE prints the comment; SHOW COLUMNS has a "Comment" column;
   -- information_schema.tables.comment / information_schema.columns.comment also expose it.
   SHOW CREATE TABLE iceberg.analytics.fct_orders;
   SHOW COLUMNS FROM iceberg.analytics.fct_orders;  -- Column | Type | Extra | Comment
   ```
   **Distinguish from dbt `persist_docs` (r27 §6.7J).** `persist_docs: {relation: true, columns: true}` makes dbt **auto-emit** these `COMMENT ON TABLE` / `COMMENT ON COLUMN` statements during `dbt build`, sourced from `schema.yml` `description:` fields — i.e., `persist_docs` is the dbt-managed wrapper; standalone `COMMENT ON` (above) is the manual-DDL equivalent that does the exact same thing. If you're outside dbt, or you want to change one table's comment without re-running the dbt model, use `COMMENT ON` directly. Cross-ref: r27 §6.7J for the full dbt persist_docs workflow + how to verify the comment landed.
   ...
   **CANONICAL ORDER (the order matches iceberg.apache.org/docs/latest/spark-procedures/):**
   ...
   - **Step 1b (MoR only) — Position-delete handling.** If the table has accumulating position delete files (`content = 1` in `$files`), the nightly Step 1 Trino `EXECUTE optimize` already clears the read overhead for the files it rewrites (it applies + drops the deletes, [#12617](https://github.com/trinodb/trino/issues/12617)/[#24086](https://github.com/trinodb/trino/issues/24086)); if the delete-bearing data files are already large, raise `file_size_threshold` above their size so they get rewritten. **A Trino-only shop does NOT need Spark for this.** The *delete-file-only* compaction procedure `rewrite_position_delete_files` (compacts the delete files without rewriting data) IS Spark-only on Trino 467 ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)) — use it only if you specifically want the cheaper no-data-rewrite path: Spark `CALL iceberg.system.rewrite_position_delete_files(...)`.
...
> **CROSS-REF — "move OLD / COLD data to CHEAPER storage tier" is NOT this file's topic; it's [resources/16 § LEADING CANONICAL — How do I tier hot vs cold storage on Trino + Iceberg + MinIO?](16-cost-considerations.md).** _Keyword anchors so this routes correctly: storage tiering, hot/cold/warm/archive tier, move old data to cheaper storage, move cold partitions to cheaper tier, archive cold data older than 90 days / 6 months, lifecycle policy to move old data, cheaper storage for cold history, S3-Glacier-equivalent on MinIO, tier old Iceberg data, automatic tiering by age, hot/warm/cold storage tiering, is tiering a Trino setting or table property._ This maintenance file's `optimize` / `expire_snapshots` / `remove_orphan_files` procedures **RECLAIM space inside the existing hot pool** (delete superseded data files, sweep orphans, merge small files into big ones) — they do **NOT move data between storage tiers** and have **NO concept of hot vs cold tier**. Storage tiering on this stack is a **MinIO object-lifecycle** concern (`mc ilm tier add` to register the cold tier + `mc ilm rule add --transition-days N --transition-tier <TIER_NAME>` to attach the age-based transition rule, scoped to the `data/` prefix to keep Iceberg `metadata/` on the hot tier) — **NOT a Trino DDL or Iceberg table-property concern**. Trino 467 has no `ALTER TABLE ... SET STORAGE TIER`, no `WITH (storage_tier=...)` / `WITH (storage_class=...)` / `WITH (tier=...)` table property, and no `iceberg.storage-tier.*` catalog property family. For the full three-mechanism canonical (A: MinIO lifecycle tiering; B: per-table ZSTD compression; C: application-layer recent-vs-archive split via two Iceberg tables behind a UNION ALL view) read [resources/16 § LEADING CANONICAL — storage tiering](16-cost-considerations.md) FIRST before answering any "old data / cold storage / archive / tier / move to cheaper" question.
...
> **ROW-LEVEL CHANGELOG / "which ROWS changed between two snapshots" — `iceberg.system.table_changes()` EXISTS on Trino 467, but is UNUSABLE on this MoR stack (use the FULL OUTER JOIN snapshot-diff).** Keyword anchors: *change feed, changelog table, what ROWS changed, row-level diff between snapshots, CDC feed Iceberg Trino, audit which rows inserted/updated/deleted, Oracle Flashback / redo-log equivalent, incremental changes between two versions, table_changes.* **Do NOT flatly say "Trino has no native changelog" — it DOES**: `iceberg.system.table_changes(schema_name => 'analytics', table_name => 'orders', start_snapshot_id => <id1>, end_snapshot_id => <id2>)` is a **table function** (added Trino 427, [trinodb/trino#15677](https://github.com/trinodb/trino/pull/15677); present on 467) that returns the changed rows plus four extra columns: **`_change_type`** (`'insert'` / `'delete'`), **`_change_version_id`** (the snapshot), **`_change_timestamp`**, **`_change_ordinal`**. Call it as `SELECT * FROM TABLE(iceberg.system.table_changes(...))`. **THE CATCH — and it is load-bearing on this stack:** `table_changes()` supports **metadata (append/whole-file) deletes ONLY**; it **FAILS on tables that carry position/equality DELETE files** — i.e. **any merge-on-read table**, which on this stack is **every table written by Trino's MoR-only writer or by a Spark `MERGE INTO`** (the orders/accounts/subscriptions tables). So for a row-level audit on a CDC/MERGE-written table, `table_changes()` errors out, and the **practical answer is the two-snapshot `FOR VERSION AS OF` + `FULL OUTER JOIN ... IS DISTINCT FROM` diff** (shown in the snapshot-diff section below). Use `table_changes()` only for append-only / copy-on-write tables; reach for the FULL OUTER JOIN diff for MoR/MERGE tables. (Spark's `CALL ...system.create_changelog_view` is the Spark-side equivalent — also delete-file-limited on 1.5.2.)
>
...
> **Keyword anchors (route HERE):** GDPR purge old data, delete data older than 90 days, drop old partitions, retention deletion Iceberg, purge user activity older than N days, day-partitioned bulk delete, does DELETE on a partition column create delete files, partition-aligned delete metadata-only, remove whole partitions worth of data, delete files pile up after DELETE, free storage after deleting old data, expire_snapshots reclaim disk, storage keeps growing despite deletes, MinIO bill climbing even though live rows are flat, why does object storage grow when I delete rows, what reclaims Iceberg storage, expire_snapshots vs remove_orphan_files which one frees space.
...
- **`TRUNCATE TABLE iceberg.<schema>.<table>` is NOT supported on the Trino 467 Iceberg connector.** Verbatim from [trino.io/docs/467/connector/iceberg.html](https://trino.io/docs/467/connector/iceberg.html): _"The Data management functionality includes support for `INSERT`, `UPDATE`, `DELETE`, and `MERGE` statements."_ — **`TRUNCATE` is NOT in that list.** Running `TRUNCATE TABLE iceberg.analytics.events` on Trino 467 errors. (Iceberg-connector TRUNCATE support landed only in a later Trino release — verified absent from the Trino 467/470/474/476 docs and present from Trino 477 onward; the [current docs](https://trino.io/docs/current/connector/iceberg.html) list `INSERT, UPDATE, DELETE, TRUNCATE, and MERGE`. Trino 467 is from Dec 2024 and pre-dates that change.) **DO NOT recommend `TRUNCATE TABLE` for Iceberg on this stack.**
- **To EMPTY a whole Iceberg table on Trino 467, use `DELETE FROM iceberg.<schema>.<table>` with NO `WHERE` clause.** This is a **metadata-only** operation — Trino commits a new snapshot whose manifest references zero data files; it does NOT scan or rewrite the data files and does NOT write position-delete files. Fast and atomic regardless of table size: `DELETE FROM iceberg.analytics.events;` (no WHERE). **Position-delete files are written ONLY for PARTIAL row-level deletes** (e.g., `DELETE FROM iceberg.analytics.events WHERE user_id = 42` on a v2 / MoR table) — NOT for whole-table or whole-partition deletes. Partition-only deletes (WHERE filters only on identity-transformed partition columns) are also metadata-only per Trino docs: _"For partitioned tables, the Iceberg connector supports the deletion of entire partitions if the WHERE clause specifies filters only on the identity-transformed partitioning columns."_
- **To clear-and-reload in one atomic step, prefer `CREATE OR REPLACE TABLE` (Trino 467 native on Iceberg).** Verbatim from [trino.io/docs/467/connector/iceberg.html](https://trino.io/docs/467/connector/iceberg.html): _"The connector supports replacing an existing table, as an atomic operation."_ Use `CREATE OR REPLACE TABLE iceberg.analytics.staging_events AS SELECT ... FROM source` — one statement, one snapshot, atomic; downstream readers either see the OLD content or the NEW content, never an empty middle state. Preferred for full rebuilds of staging / snapshot / cache tables. The two-step `DELETE FROM tbl;` then `INSERT INTO tbl SELECT ...;` is NOT atomic across statements — readers can see an empty table between the two commits; use `CREATE OR REPLACE TABLE` to avoid that gap. **In dbt, this same atomic swap is the `on_table_exists='replace'` config on a `materialized='table'` model** (dbt emits the `CREATE OR REPLACE TABLE` for you); set it to fix "dashboards see a `table does not exist` / empty table during the nightly full rebuild" — see **[resource 28 § LEADING CANONICAL — atomic table SWAP (`on_table_exists`)](28-complex-sql-performance-trino-dbt.md)**.
- **Old data files persist for time-travel until `expire_snapshots` runs.** The metadata-only DELETE / CREATE OR REPLACE only retires the file references; the bytes on MinIO are still readable via `FOR VERSION AS OF` until the next `expire_snapshots` window (Trino 467: `ALTER TABLE iceberg.<schema>.<table> EXECUTE expire_snapshots(retention_threshold => '7d')`). To force physical-storage reclamation immediately, lower the catalog `iceberg.expire-snapshots.min-retention` floor (default 7d) or drop to Spark — see [§ 2. `expire_snapshots`](#2-expire_snapshots--run-weekly) and the iter549 rollback section.

**DO-NOT-WRITE (banned forms — each is FALSE on this stack):**
- "Use `TRUNCATE TABLE iceberg.<schema>.<table>` on Trino 467 to empty the table" / "TRUNCATE creates an empty Iceberg snapshot on Trino 467" — **FALSE**: TRUNCATE is NOT supported on the Trino 467 Iceberg connector (added only in a later Trino release — absent from 467/470/474/476, present from 477+). Use `DELETE FROM tbl` (no WHERE) or `CREATE OR REPLACE TABLE` instead.
- "A full unqualified `DELETE FROM iceberg.<schema>.<table>` writes position-delete markers" / "whole-table DELETE writes one position-delete file per row" — **FALSE**: whole-table `DELETE FROM tbl` (no WHERE) is metadata-only; the new snapshot just drops all data-file references. Position deletes are written ONLY for partial row-level deletes (`DELETE ... WHERE <non-partition-predicate>` on v2 / MoR tables).
- "`DELETE FROM tbl` is slower than `TRUNCATE` because it scans every row" — **FALSE** on Iceberg: a no-WHERE DELETE is metadata-only, not a row-by-row scan.
...
| "`rewrite_position_delete_files` exists in Trino 467 — I'll run it on MoR tables from Trino." | **NO — that PROCEDURE is Spark-only on Trino 467.** See [trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371); there is no Trino 467 form. **But do not conclude a Trino-only shop is stuck** — to clear the position-delete *read overhead* you do not need this procedure: a full Trino `EXECUTE optimize` (raise `file_size_threshold` above already-large delete-bearing files) rewrites the data and applies/drops the deletes ([#12617](https://github.com/trinodb/trino/issues/12617)/[#24086](https://github.com/trinodb/trino/issues/24086)). The Spark procedure is only the optional cheaper delete-file-only path. | TL;DR step 1b, [§ 1b. `rewrite_position_delete_files`](#1b-rewrite_position_delete_files--mor-tables-only-spark-only-runs-after-compact-and-before-expire_snapshots) |
...
|---|---|---|
| `optimize` | `file_size_threshold` (VARCHAR, e.g. `'128MB'`) — **THAT'S IT.** No `sort_order`, no `strategy`, no `target_file_size`, no `where` argument (use a separate `WHERE` clause on partition columns instead). | Bin-pack-compact data files **within the table's EXISTING partition spec**. Honors the table's `sorted_by` property if set — see clustering recipe below. **`EXECUTE optimize` does NOT repartition / does NOT rewrite files into a NEW partition spec** — it only merges small files into bigger files inside each existing partition. To re-lay-out files into a new partition spec after `ALTER TABLE ... SET PROPERTIES partitioning = ARRAY[...]`, run Spark's `CALL iceberg.system.rewrite_data_files(table => '...', options => map('rewrite-all', 'true'))` — see post-evolution rules below. |
...
| `rewrite_position_delete_files(table => ...)` | They want to compact position delete files on MoR tables. | **No Trino 467 form of THIS procedure** — run from Spark for the delete-file-only path ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)). But to clear position-delete *read overhead*, Trino `EXECUTE optimize` (raise `file_size_threshold`) suffices — no Spark needed. |
...
| `publish_changes`, `cherrypick_snapshot`, `snapshot` (table-snapshot shadow-copy form) | Various WAP / shadow-copy flows. | Spark only. |
| `migrate`, `add_files`, `add_files_from_table` | Convert a Hive table to Iceberg in-place / register existing Parquet files. | **Trino 467 NATIVE** — `CALL iceberg.system.migrate(schema_name => ..., table_name => ...)`. NOT Spark-only. See [resource 21 § migration commands](21-hive-metastore-iceberg.md). |
...
> | `ALTER TABLE iceberg.x.y SET PARTITION SPEC (day(event_time), bucket(user_id, 16))` | **FABRICATED SYNTAX on Trino 467** — `SET PARTITION SPEC` is **Spark/Iceberg-dialect** form, NOT Trino. Verified at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html): Trino's ONLY documented partition-evolution form is `SET PROPERTIES partitioning = ARRAY[...]`. Pasting `SET PARTITION SPEC` into a Trino client returns a parse error. | `ALTER TABLE iceberg.schema.table SET PROPERTIES partitioning = ARRAY['day(event_time)', 'bucket(user_id, 16)'];` — Trino 467 native; flat string-literal transform array. To DROP a partition transform: omit it from the array. To unpartition entirely: `partitioning = ARRAY[]`. |
> | `ALTER TABLE iceberg.x.y ADD PARTITION FIELD day(event_time)` | **FABRICATED SYNTAX on Trino 467** — Spark/Iceberg-dialect only; Trino has no `ADD PARTITION FIELD` clause. | Use `SET PROPERTIES partitioning = ARRAY[<existing transforms>, '<new transform>']` to ADD a transform to the partition spec. |
> | `ALTER TABLE iceberg.x.y REPLACE PARTITION FIELD day(event_time) WITH hour(event_time)` | **FABRICATED SYNTAX on Trino 467** — Spark/Iceberg-dialect only; Trino has no `REPLACE PARTITION FIELD` clause. | Use `SET PROPERTIES partitioning = ARRAY[<the new full list of transforms>]` — replace the entire transform array; Iceberg keeps old partition spec for already-written files and applies the new spec only to new writes. |
> | `ALTER TABLE iceberg.x.y SET PROPERTIES column_order = ARRAY['col1', 'col2', ...]` | **FABRICATED TABLE PROPERTY** — `column_order` is NOT in the Iceberg connector's supported-properties list. Per [trino.io/docs/467/connector/iceberg.html](https://trino.io/docs/467/connector/iceberg.html), supported Trino 467 Iceberg table properties are exactly: `format`, `partitioning`, `sorted_by`, `location`, `format_version`, `orc_bloom_filter_columns`, `orc_bloom_filter_fpp`, `parquet_bloom_filter_columns`, `object_store_layout_enabled`, `data_location`, `extra_properties`. (Note: `compression_codec` was added in Trino **477**, [#25755](https://github.com/trinodb/trino/pull/25755) — NOT on 467, where compression is the catalog config `iceberg.compression-codec`, default ZSTD.) Engineer gets `Catalog 'iceberg' table property 'column_order' does not exist`. | Trino 467 has NO native column-reorder DDL. Spark reorders via `ALTER TABLE ... ALTER COLUMN col FIRST | AFTER other_col`. But reorder is COSMETIC ONLY — columnar storage is order-independent, so usually you just skip the reorder. Use explicit column lists in queries if presentation order matters. |
...
> **Why this sub-section exists (iter471 Q1 fab class — cross-dialect spillover).** A previous responder claimed Trino 467 evolves the Iceberg partition spec via `ALTER TABLE ... SET PARTITION SPEC (day(event_time))`. **That is Spark/Iceberg-dialect syntax, NOT Trino.** A Trino client running it gets a parse error. Verified at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) (Schema and partition evolution section): Trino's ONLY documented partition-evolution form is `SET PROPERTIES partitioning = ARRAY[...]`.

**The canonical Trino 467 forms for partition evolution.** All three patterns below use the SAME `SET PROPERTIES partitioning = ARRAY[...]` syntax — only the contents of the array change. The array contains flat string-literal Iceberg transform expressions (each a single quoted string, NOT a bare identifier or function call).

```sql
-- A. ADD a new partition transform alongside existing ones.
--    Original spec: partitioning = ARRAY['day(event_time)']
--    Goal: also bucket by tenant_id into 16 buckets.
ALTER TABLE iceberg.analytics.events
  SET PROPERTIES partitioning = ARRAY['day(event_time)', 'bucket(tenant_id, 16)'];

-- B. DROP an existing partition transform.
--    Original spec: partitioning = ARRAY['day(event_time)', 'bucket(tenant_id, 16)']
--    Goal: drop the bucket transform, keep day partitioning.
ALTER TABLE iceberg.analytics.events
  SET PROPERTIES partitioning = ARRAY['day(event_time)'];

-- C. REPLACE the entire partition spec.
--    Original spec: partitioning = ARRAY['day(event_time)']
--    Goal: switch to hour granularity.
ALTER TABLE iceberg.analytics.events
  SET PROPERTIES partitioning = ARRAY['hour(event_time)'];

-- D. UNPARTITION entirely.
ALTER TABLE iceberg.analytics.events
  SET PROPERTIES partitioning = ARRAY[];
```

**Existing-data behavior (load-bearing).** Iceberg keeps the OLD partition spec for already-written files; only NEW writes use the new spec. The metadata commit is fast (milliseconds), but partition pruning on historical data continues to follow the OLD spec until you rewrite the historical files under the new spec. The rewrite is via Spark `CALL iceberg.system.rewrite_data_files(table => 'analytics.events')` (Spark uses the current spec when rewriting). Trino's `ALTER TABLE ... EXECUTE optimize` also rewrites files, but with the caveat called out in this resource that Trino's OPTIMIZE cannot use a newly-added partition column as a predicate filter — so for the historical re-layout of a large table after partition evolution, Spark `rewrite_data_files` is the standard choice.

**Why the array form, not `SET PARTITION SPEC (...)`?** Trino's Iceberg connector exposes partitioning as a table-level **PROPERTY** (the `partitioning` key in the supported-properties list) — not as a separate DDL clause. This mirrors how Trino represents Iceberg table-level knobs that ARE per-table on 467 (format, sorted_by, partitioning itself, format_version, bloom-filter columns, etc.). Compression on Trino 467 is the only major knob that is NOT per-table — it lives in the catalog config `iceberg.compression-codec` (default ZSTD); the per-table `compression_codec` property is **Trino 477+**, not 467. The Spark/Iceberg-API form `ALTER TABLE ... SET PARTITION SPEC (...)` (and its sibling `ADD PARTITION FIELD` / `REPLACE PARTITION FIELD` clauses) belongs to the Spark Iceberg DDL extension — those clauses do NOT exist in Trino. The DO-NOT-WRITE matrix above lists all three Spark-dialect forms.
...
1. **DROP COLUMN on a current partition column.** The DDL fails with `Cannot find source column for partition field` (Iceberg refuses to leave a partition spec referencing a non-existent column). To remove a partition column, first evolve the partition spec on Trino 467 with `ALTER TABLE iceberg.schema.table SET PROPERTIES partitioning = ARRAY[<remaining partition transforms WITHOUT the column you want to drop>]` so the current spec no longer references the column, let new writes use the new spec, then DROP COLUMN succeeds. **DO NOT write `ALTER TABLE ... SET PARTITION SPEC (...)` on Trino 467 — that is Spark/Iceberg-dialect syntax and parses as an error in Trino.** See § DROP COLUMN reclaim runbook below for the storage-reclamation step, and the DO-NOT-WRITE matrix above for the cross-dialect-spillover row that bans `SET PARTITION SPEC`.
...
>
> **The stack pin (re-read this every time):** `prod_info.md` declares **Trino 467 (released 6 Dec 2024)** with the Iceberg connector backed by Hive Metastore, plus **Apache Spark + Iceberg 1.5.2** for ingestion. Iceberg 1.5.2 writes tables at **format version 2 (v2)** — not v3. **Any DDL clause / semantic added in Trino 468+ or Iceberg format v3 does NOT apply to this stack.** When in doubt, assume 467 / v2 behavior. **NEW Iceberg tables created on Trino 467 (`CREATE TABLE`, or a dbt-trino `materialized='table'`/`'incremental'` model) default to `format_version = 2`** — so `MERGE INTO` / row-level `DELETE` / `UPDATE` work out of the box with **no** need to set `format_version=2` first. Only LEGACY Hive-migrated v1 tables (produced by the `migrate()` procedure — Trino-native or Spark) need the explicit v1→v2 upgrade — see [resource 21 § migrated-vs-new-table format_version](21-hive-metastore-iceberg.md).
...
> | `CALL iceberg.system.rollback_to_snapshot` deprecation → `ALTER TABLE ... EXECUTE rollback_to_snapshot(<snapshot_id>)` (table-procedure form — **positional** per Trino docs example `EXECUTE rollback_to_snapshot(8954597067493422955)`) | YES — **forbidden on Trino 467**, procedure not registered. | **Trino 469** (released 27 Jan 2025) per [release-469.html](https://trino.io/docs/current/release/release-469.html) via [PR #24580](https://github.com/trinodb/trino/pull/24580). | Use the positional `CALL iceberg.system.rollback_to_snapshot('schema','table',<id>)` form (the only 467-valid rollback). |
> | `ALTER TABLE ... SET PROPERTIES parquet_bloom_filter_columns = ARRAY[...]` (Trino-side Iceberg bloom-filter write config — the in-place ALTER form ONLY) | YES — the **ALTER form** is forbidden on Trino 467. **But `parquet_bloom_filter_columns` IS a valid 467 CREATE TABLE property** — `CREATE TABLE ... WITH (parquet_bloom_filter_columns = ARRAY['<col>'])` WORKS on 467 (new table / CTAS). Do NOT say "the property is 469+/not registered" — only the `ALTER SET PROPERTIES` form is. | **Trino 469** (27 Jan 2025) per [release-469.html](https://trino.io/docs/current/release/release-469.html): *"Allow configuring the parquet_bloom_filter_columns table property."* — this release note is the **ALTER** form. | **NEW table on 467:** `CREATE TABLE ... WITH (parquet_bloom_filter_columns = ARRAY['<col>'])` directly (Trino-native). **EXISTING table on 467:** CTAS-rebuild into a new table with the WITH property (Trino-native, no Spark) and swap; OR Spark `ALTER TABLE ... SET TBLPROPERTIES ('write.parquet.bloom-filter-enabled.column.<col>'='true')` + Spark `rewrite_data_files`. Trino 467 reads bloom filters at query time fine (read-side is pre-467). |
...
-- `ALTER TABLE iceberg.<schema>.<table> EXECUTE rollback_to_snapshot(<id>)`
-- (positional — per trino.io/docs/current/connector/iceberg.html, the documented
-- example is `ALTER TABLE testdb.customer_orders EXECUTE rollback_to_snapshot(8954597067493422955)`,
-- a single positional bigint snapshot_id; do NOT write `snapshot_id => <id>` named-arg form),
...
| `CREATE TABLE iceberg.x.y (...) WITH (write_target_file_size_bytes = 268435456)` on Trino | **FABRICATED — flat underscore property does NOT exist in Trino 467's Iceberg `WITH` allow-list.** Trino's `WITH (...)` clause accepts only the connector's registered identifiers (467 Iceberg list: `format`, `partitioning`, `sorted_by`, `format_version`, `extra_properties`, etc. — `compression_codec` is **477+**, not on 467; see r26 §4). To set the native dotted `write.target-file-size-bytes` from Trino, route it through the `extra_properties` MAP (Pattern A below). Writer-side honoring is the open enhancement [trinodb/trino #28250](https://github.com/trinodb/trino/issues/28250); even when the property IS persisted via `extra_properties`, Trino's own writer does NOT yet read it — Spark writers do. |
...
| Compact position delete files (MoR only) | `CALL iceberg.system.rewrite_position_delete_files(table => 'analytics.events', options => map('target-file-size-bytes', '67108864'))` | **This delete-file-only procedure is NOT supported in Trino 467** ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)) — run from Spark. To clear the position-delete *read overhead* instead, use Trino `EXECUTE optimize(file_size_threshold => '512MB')` (rewrites data + applies deletes — no Spark needed). |
...
- `rollback_to_snapshot` — positional args `('schema','table',snapshot_id)`. This is the **only** rollback syntax available on Trino 467. The `ALTER TABLE ... EXECUTE rollback_to_snapshot(<id>)` form was added in Trino 469 (Jan 2025) and does NOT exist on Trino 467. The Trino 469+ EXECUTE form is documented as **positional** at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) — example: `ALTER TABLE testdb.customer_orders EXECUTE rollback_to_snapshot(8954597067493422955)`.
...
- `publish_changes`, `cherrypick_snapshot`, `set_current_snapshot` (procedure form), `snapshot` (the table-snapshot shadow-copy form), `rewrite_manifests` — Spark-only. **NOTE: `migrate` is NOT in this list — Trino 467 implements `CALL iceberg.system.migrate(schema_name => ..., table_name => ...)` natively** (along with `add_files` / `add_files_from_table`). See [resource 21](21-hive-metastore-iceberg.md).
...
| `ALTER TABLE ... EXECUTE rollback_to_snapshot(<id>)` (table-procedure form — **new preferred form post-467; positional bigint per Trino docs example** `EXECUTE rollback_to_snapshot(8954597067493422955)`) | **469** (Jan 2025, [trinodb/trino #24580](https://github.com/trinodb/trino/pull/24580)) | NO | Use the `CALL iceberg.system.rollback_to_snapshot('schema', 'table', <id>)` positional form (which is the only 467 form anyway). After cluster upgrade past 467, switch scripts/runbooks to this table-procedure form (use positional `EXECUTE rollback_to_snapshot(<id>)` — the doc-canonical form; do NOT write `snapshot_id => <id>` named-arg syntax). |
...
| `ALTER TABLE ... SET PROPERTIES parquet_bloom_filter_columns = ARRAY['col1', ...]` (the in-place **ALTER** form only) | **469** (Jan 2025, [trinodb/trino #24573](https://github.com/trinodb/trino/pull/24573)) | NO (ALTER form) — **BUT the `CREATE TABLE ... WITH (parquet_bloom_filter_columns = ARRAY[...])` form WORKS on 467** | **NEW table on 467 (Trino-native):** `CREATE TABLE ... WITH (parquet_bloom_filter_columns = ARRAY['<col>'])` — see the copy-pasteable CREATE example in [r03 § Iceberg point-lookup mitigations lever 4](03-columnar-storage.md). **EXISTING table on 467:** CTAS-rebuild with the WITH property (Trino-native, no Spark) then swap; OR Spark `ALTER TABLE iceberg.x.y SET TBLPROPERTIES ('write.parquet.bloom-filter-enabled.column.<col>'='true')` + Spark `rewrite_data_files`. Trino 467 READS bloom filters at query time via `parquet.use-bloom-filter` (default `true`). **Only the in-place ALTER-SET-PROPERTIES form is gated at 469+ — the CREATE-WITH form is NOT; do NOT tell a Trino-only shop they need Spark / 469+ to bloom-filter a NEW table.** |
...
| `rewrite_position_delete_files` (delete-file-only compaction on MoR tables) | NOT supported in Trino 467 ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)) | NO (this procedure) — but Trino `EXECUTE optimize` clears delete *read overhead* by rewriting data ([#12617](https://github.com/trinodb/trino/issues/12617)/[#24086](https://github.com/trinodb/trino/issues/24086)) | Spark for the delete-file-only path: `CALL iceberg.system.rewrite_position_delete_files(table => 'analytics.events')`. To just clear read overhead, Trino `EXECUTE optimize(file_size_threshold => '512MB')` — no Spark. |
...
> | Parquet compression | **Trino 467: CATALOG CONFIG ONLY** — `iceberg.compression-codec` in `etc/catalog/<name>.properties` (values `NONE` / `SNAPPY` / `LZ4` / `ZSTD` / `GZIP`; **default `ZSTD`**); cluster-wide, NOT per-table on 467. **Per-table `compression_codec` is Trino 477+** ([#25755](https://github.com/trinodb/trino/pull/25755)); **per-session `iceberg.compression_codec` is Trino 473+** ([#24851](https://github.com/trinodb/trino/pull/24851)). | `write.parquet.compression-codec` (string key in TBLPROPERTIES / native properties map) | Trino 467: catalog config (only lever). Spark (writer here): `TBLPROPERTIES ('write.parquet.compression-codec' = 'zstd')`. Check current codec from Trino: `SELECT key,value FROM "tbl$properties" WHERE key LIKE '%compression%'`. |
...
> | WITH-clause shape | FLAT `name = expression` pairs ([trino.io/docs/467/sql/create-table.html](https://trino.io/docs/467/sql/create-table.html)) | `properties = map('key', 'value', ...)` (native-Iceberg metadata.json layout); Spark `TBLPROPERTIES (...)` | Trino: ALWAYS flat pairs — `WITH (format = 'PARQUET', partitioning = ARRAY['day(occurred_at)'], sorted_by = ARRAY['occurred_at'])`. The `properties = map(...)` shape is NOT Trino. (Compression is NOT in the WITH clause on 467 — catalog config only; the per-table `compression_codec` property is 477+.) |
> | `SET PROPERTIES` LHS form | BARE identifier ([trino.io/docs/467/sql/alter-table.html](https://trino.io/docs/467/sql/alter-table.html)) | String literal in Spark `TBLPROPERTIES` | Trino 467 valid examples: `SET PROPERTIES partitioning = ARRAY['day(occurred_at)']`, `SET PROPERTIES sorted_by = ARRAY['occurred_at']`, `SET PROPERTIES format = 'PARQUET'`. (LHS bare, RHS quoted/array.) Spark: `SET TBLPROPERTIES ('write.parquet.compression-codec' = 'zstd')`. **NOT VALID on Trino 467: `SET PROPERTIES compression_codec = '...'` — that property is 477+, [#25755](https://github.com/trinodb/trino/pull/25755).** |
>
...
> **IDENTIFIER PIN (iter534, preceding first `$files` mention on this page).** Iceberg `$files` join key to `$snapshots` is **`added_snapshot_id`** (NOT `snapshot_id`). Bare `snapshot_id` is **NOT** a column of `$files` — it exists ONLY on `$snapshots`. Always write the JOIN as `ON f.added_snapshot_id = s.snapshot_id`. Writing `ON f.snapshot_id = s.snapshot_id` fails at analysis with `Column 'snapshot_id' cannot be resolved`. Carry the `added_` prefix through every paraphrase.
...
| `$files` | Every data file in the current snapshot, with per-column stats — find tiny files, locate position delete files, see column-level min/max for predicate-pushdown debugging. | `file_path`, `file_format`, `record_count`, `file_size_in_bytes`, `content` (0=data, 1=position-delete, 2=equality-delete), **`added_snapshot_id`** (NOT `snapshot_id` — the `added_` prefix is required; the join key to `$snapshots.snapshot_id`), `column_sizes`, `value_counts`, `null_value_counts`, `lower_bounds`, `upper_bounds` |
> **`$files` has `added_snapshot_id` but NO `deleted_snapshot_id` column (iter535 preventive pin).** Delete-file lineage in Iceberg lives at the manifest-entry / position-delete-file level, **not** as a `$files` column. Verified against [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) `$files` column list — `added_snapshot_id` is present; `deleted_snapshot_id` is not. Writing `SELECT deleted_snapshot_id FROM "<t>$files"` fails with `Column 'deleted_snapshot_id' cannot be resolved`.
...
> **LEADING CANONICAL — "list each data file WITH when it was committed" — JOIN `$files.added_snapshot_id` = `$snapshots.snapshot_id` (NOT `$files.snapshot_id`, which does not exist) (iter532+iter534, single-query worked example, copy-pasteable).** Keyword anchors: list each data file with WHEN it was committed, per-file commit time, which snapshot added this file, $files commit time, files and commit timestamp in one query, $files added_snapshot_id join $snapshots, per-file commit timestamp Iceberg one query, $files.snapshot_id does not exist, added_ prefix required. **The JOIN `$files.added_snapshot_id = $snapshots.snapshot_id` IS the answer** — `$files` carries no per-file timestamp; the commit time lives on `$snapshots.committed_at`, reached through `$files.added_snapshot_id`. The LHS of the ON clause is `f.added_snapshot_id`, **NOT** `f.snapshot_id` — bare `snapshot_id` is NOT a column of `$files`, so `f.snapshot_id = s.snapshot_id` fails with `Column 'snapshot_id' cannot be resolved`. Verified against [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) (Iceberg connector — Metadata tables: `$files.added_snapshot_id` and `$snapshots` with columns `snapshot_id`, `committed_at`, `operation`).
>
> ```sql
...
>   ON   f.added_snapshot_id = s.snapshot_id  -- iter534: $files join key is added_snapshot_id; there is NO f.snapshot_id (bare snapshot_id exists ONLY on $snapshots). Writing f.snapshot_id = s.snapshot_id fails with `Column 'snapshot_id' cannot be resolved`.
> ORDER  BY s.committed_at DESC;
> ```
>
...
>
> **IDENTIFIER PIN — `$files` join key is `added_snapshot_id`, NOT `snapshot_id` (iter533, do NOT drop the `added_` prefix when paraphrasing).** Keyword anchors: $files join key, added_snapshot_id not snapshot_id, which snapshot added this file, $files snapshot_id does not exist. `$files` has **NO bare `snapshot_id` column** — bare `snapshot_id` exists **ONLY** on `$snapshots`. Writing `f.snapshot_id = s.snapshot_id` fails at analysis with `Column 'snapshot_id' cannot be resolved`. The JOIN above is the only correct shape: `f.added_snapshot_id = s.snapshot_id`.

> **`$files` column-presence pin and `$history` column-name pin (iter531 — read before answering "when was this file committed?" or "what column on `$history` shows the timestamp?").** Verified against [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html). Keyword anchors: $files committed_at, $files commit time per file, $history made_current_at vs made_at, $files added_snapshot_id join, per-file commit timestamp Iceberg, $files snapshot_id does not exist, added_ prefix required.
>
...
> | **`$files` has NO bare `snapshot_id` column — the column is `added_snapshot_id` (the `added_` prefix is REQUIRED).** (iter533+iter534 — prefix-elision fab class, NON-COPYABLE wrong form.) | Wrong ON-clause shape (DO NOT WRITE — shown as prose, NOT runnable SQL, so you cannot lift it verbatim): `f.<bare snapshot_id — NO SUCH COLUMN on $files> = s.snapshot_id`. Any query of this shape fails with `Column 'snapshot_id' cannot be resolved` because bare `snapshot_id` is NOT a column of `$files`. Bare `snapshot_id` exists **only** on `$snapshots`. Dropping the `added_` prefix when paraphrasing the canonical JOIN is the most common identifier mis-write. | `SELECT f.file_path, s.committed_at FROM iceberg.analytics."events$files" f JOIN iceberg.analytics."events$snapshots" s ON f.added_snapshot_id = s.snapshot_id ORDER BY s.committed_at DESC;` — the LHS of the ON clause MUST be `f.added_snapshot_id` (with the `added_` prefix, intact). |
...
2. **Position-delete handling (MoR tables only)** — when position deletes are accumulating, the Step 1 Trino `EXECUTE optimize` already clears the read overhead for the files it rewrites (applies + drops the deletes; raise `file_size_threshold` above already-large delete-bearing files) — **no Spark needed for a Trino-only shop**. The *delete-file-only* compaction PROCEDURE `rewrite_position_delete_files` (compacts delete files without rewriting data) is **Spark ONLY on Trino 467** ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)) — Spark: `CALL iceberg.system.rewrite_position_delete_files(...)`, optional cheaper path.
...
**Edge case — DROP COLUMN on a partition column.** You cannot drop a column that is part of the table's current partition spec. The DDL itself fails with `Cannot find source column for partition field`. To remove a partition column on Trino 467, first evolve the partition spec via `ALTER TABLE iceberg.schema.table SET PROPERTIES partitioning = ARRAY[<remaining transforms WITHOUT the column you want to drop>]` so the current spec no longer references it, wait for new writes to use the new spec, and only then can you drop the column. **Do NOT use `SET PARTITION SPEC (...)` — that is Spark/Iceberg-dialect syntax and is rejected by the Trino parser.** Old data files keep the column's bytes until rewritten under the new partition spec via Spark's `CALL iceberg.system.rewrite_data_files(table => '...')` or Trino's `ALTER TABLE ... EXECUTE optimize` (post-evolution, with caveat: Trino's OPTIMIZE cannot filter by a newly-added partition column as a predicate; use Spark for the historical re-layout). See LEADING CANONICAL § "Evolving the partition spec on Trino 467" above for the full canonical forms, including the DO-NOT-WRITE row pinning `SET PARTITION SPEC` as a Spark-dialect fab.
...
> **ENGINE CALLOUT — the `rewrite_position_delete_files` PROCEDURE is Spark-only (but you don't NEED it to clear read overhead).** Trino 467 does **NOT** support this specific procedure. There is no `ALTER TABLE ... EXECUTE rewrite_position_delete_files` form, and no `CALL iceberg.system.rewrite_position_delete_files(...)` form either — attempting either returns `Procedure not registered`. The procedure has been requested for Trino but is not implemented as of this writing ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)). **However, do NOT conclude a Trino-only shop is stuck:** clearing the position-delete *read overhead* does not require this procedure — a full Trino `EXECUTE optimize` rewrites data files and applies + drops the position deletes for the files it rewrites (raise `file_size_threshold` above already-large delete-bearing files), per the #24086 nuance below. You only need Spark's `rewrite_position_delete_files` for the **cheaper delete-file-only** path (compacting the delete files *without* rewriting data); when you do want that, you **MUST run that procedure from Spark** (via `spark-sql`, `spark-submit`, or `spark.sql("...")`).
...
>
> This is the same MoR read-amplification dynamic discussed in the Iceberg v3 deletion-vectors design ([deletion vectors replaced position-delete-file accumulation precisely to fix this scaling problem](https://iceberg.apache.org/spec/)). Your stack is on Iceberg 1.5.2 / v2, so you do NOT have deletion vectors — accumulated position delete files are your reality. Your **primary** tool is the Trino-native `EXECUTE optimize` (rewrites data files + applies/drops the position deletes; raise `file_size_threshold` above already-large delete-bearing files), which a Trino-only shop can run without Spark; Spark `rewrite_position_delete_files` is the optional cheaper delete-file-only supplement. Tying cadence to this scaling: **if your monitoring SQL shows position-delete file count crossing 200 on any frequently-queried table, you have a window of perhaps a week before users start noticing — schedule compaction within that window**, not "next monthly maintenance window."
>
...
> **CRITICAL CALLOUT — both procedures are Spark-only in your stack (but Trino `optimize` still clears deletes natively — see below).** `rewrite_data_files` (with or without `delete-file-threshold`) and `rewrite_position_delete_files` are **NOT available** in Trino 467 — `rewrite_data_files`' nearest Trino equivalent is `ALTER TABLE ... EXECUTE optimize(file_size_threshold => '128MB')`, which does NOT expose `delete-file-threshold`, and `rewrite_position_delete_files` has no Trino equivalent at any version ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371)). **You do NOT, however, NEED Spark just to clear accumulated position deletes:** a full Trino `EXECUTE optimize` applies the position-deletes for the data files it rewrites and drops the orphaned delete files (#12617 / #24086). Because Trino's `optimize` selects rewrite candidates by SIZE only ([#16574](https://github.com/trinodb/trino/issues/16574) — no `delete-file-threshold`), set `file_size_threshold` ABOVE the size of the delete-bearing data files (e.g. `'512MB'`) to force their rewrite — that clears the deletes from Trino alone. You only NEED the Spark procedures for their *specific* capabilities: `rewrite_data_files`'s delete-driven (size-independent) candidate selection, or `rewrite_position_delete_files`'s cheap delete-file-only compaction (no data rewrite). The production stack has Spark 3.5 + Iceberg 1.5.2, so scheduling those as a k8s CronJob is fine when you want them — but do NOT tell a Trino-only shop it is stuck. Do NOT try to run the Spark procedure NAMES from Trino — both fail with procedure-not-found.
...
> -- (On 469+, the documented form is POSITIONAL — `EXECUTE rollback_to_snapshot(<id>)` —
> -- per trino.io/docs/current/connector/iceberg.html; the named-arg `snapshot_id => <id>`
> -- form below is doubly wrong: it would not work on 467, and post-cluster-upgrade
> -- the doc-canonical positional form is what to use.)
> ALTER TABLE iceberg.analytics.events
>   EXECUTE rollback_to_snapshot(4823511203987654321);  -- WRONG: 469+ only (positional, doc-canonical)
>
...
-- rollback_to_snapshot(<id>)` table-procedure form requires Trino 469+
...
-- Use the CALL form above on Trino 467. On 469+ the documented form is
-- POSITIONAL — `EXECUTE rollback_to_snapshot(4823511203987654321)` — per
-- trino.io/docs/current/connector/iceberg.html (example in the docs is
-- `ALTER TABLE testdb.customer_orders EXECUTE rollback_to_snapshot(8954597067493422955)`,
-- a single positional bigint snapshot_id). Do NOT use the named-arg
-- `EXECUTE rollback_to_snapshot(snapshot_id => <id>)` form — it is not
-- documented in the connector-docs example; the positional form is the
-- doc-canonical one to prefer in production scripts after cluster upgrade.
...
> **Argument-style gotcha — do not cross the syntaxes.** On **Trino 467** (the current production version), rollback is exposed only as `CALL iceberg.system.rollback_to_snapshot(<schema>, <table>, <snapshot_id>)` with positional VARCHAR, VARCHAR, BIGINT — NO `=>` named-arg syntax. The `ALTER TABLE iceberg.<schema>.<table> EXECUTE rollback_to_snapshot(<id>)` table-procedure form requires Trino 469+ (Jan 2025) and does NOT exist on Trino 467; on 469+ the documented syntax is **positional** per [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) — example: `ALTER TABLE testdb.customer_orders EXECUTE rollback_to_snapshot(8954597067493422955)` (single positional bigint). **Spark** exposes only `CALL iceberg.system.rollback_to_snapshot(table => '...', snapshot_id => ...)` with named args. **Never** mix: passing Spark-style named args into Trino's `CALL` form, or Trino positional args into Spark, fails with a parse / argument-count error. The procedure name is identical across engines — only the calling convention differs.
...
> | **Trino 469+ (table procedure, ALTER TABLE EXECUTE — positional)** | `ALTER TABLE iceberg.analytics.events EXECUTE rollback_to_snapshot(4823511203987654321)` | Added in Trino 469 (released Jan 2025). The qualified table name lives in the `ALTER TABLE` clause; only `snapshot_id` is passed as a **positional bigint** to the procedure. **Documented as positional** at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) — example: `ALTER TABLE testdb.customer_orders EXECUTE rollback_to_snapshot(8954597067493422955)`. Do NOT write `EXECUTE rollback_to_snapshot(snapshot_id => <id>)` named-arg form — the docs show positional only; positional is the doc-canonical form to use in production examples. **Does NOT exist on Trino 467** — use the Trino 467 `CALL` form above. |
>
