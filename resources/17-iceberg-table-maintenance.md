...
- "Use `TRUNCATE TABLE iceberg.<schema>.<table>` on Trino 467 to empty the table" / "TRUNCATE creates an empty Iceberg snapshot on Trino 467" — **FALSE**: TRUNCATE is NOT supported on the Trino 467 Iceberg connector (added in 481). Use `DELETE FROM tbl` (no WHERE) or `CREATE OR REPLACE TABLE` instead.
