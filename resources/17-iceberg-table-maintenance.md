...
4. **Each maintenance operation has a DIFFERENT API surface in Trino vs Spark — read the engine split FIRST, then the canonical order below. Do NOT conflate the two.**
   ...
   **Trino-EXECUTE-vs-Spark-CALL split (the load-bearing distinction — memorize this BEFORE writing any procedure call):**
   ...
   | Snapshot expiry | `ALTER TABLE iceberg.<schema>.<table> EXECUTE expire_snapshots(retention_threshold => '7d')`  <!-- Trino: ALTER TABLE ... EXECUTE; NOT `EXECUTE PROCEDURE` (prepared-statement clause only), NOT `CALL` (CALL is Spark) --> | `CALL iceberg.system.expire_snapshots(table => '<schema>.<table>', older_than => current_timestamp - interval '7' day, retain_last => 10)` |
   | Orphan-file sweep | `ALTER TABLE iceberg.<schema>.<table> EXECUTE remove_orphan_files(retention_threshold => '7d')`  <!-- Trino: ALTER TABLE ... EXECUTE; NOT `EXECUTE PROCEDURE`, NOT `CALL` (CALL is Spark) --> | `CALL iceberg.system.remove_orphan_files(table => '<schema>.<table>', older_than => current_timestamp - interval '3' day, dry_run => true)` |
