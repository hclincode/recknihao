...
## PARTITION EVOLUTION — the 12-line in-place-ALTER + historical-repartition nuance (iter537 PIN)

> **Keyword anchors this block answers:** change Iceberg partitioning, partition evolution, month to day partition, change partition spec from month to day, ALTER TABLE SET PROPERTIES partitioning, repartition existing data Iceberg, repartition historical files, rewrite_data_files partition evolution, optimize partition spec, Trino optimize repartition new spec, Spark rewrite_data_files rewrite-all.

1. **You CAN evolve the partition spec IN PLACE on Trino 467** — `ALTER TABLE cat.sch.tbl SET PROPERTIES partitioning = ARRAY['day(occurred_at)']` (verified at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html)). **Metadata-only**, milliseconds, no data rewrite. **No DROP+RECREATE required**, and old queries continue to work (Trino reads transparently across mixed specs).
2. **Affects NEW writes only.** Existing data files keep their **old `spec_id`** and their old partition layout. New writes land under the new spec. To see the split: `SELECT spec_id, COUNT(*) FROM cat.sch."tbl$files" GROUP BY spec_id`.
3. **To re-lay-out the OLD files into the new spec, the split-by-engine rule is:**
   - **Trino `ALTER TABLE ... EXECUTE optimize`** — Trino docs describe this as bin-pack compaction within the current spec; **the docs are silent on cross-spec re-layout of pre-existing files**, and open issues ([trinodb/trino #25279](https://github.com/trinodb/trino/issues/25279), [#12983](https://github.com/trinodb/trino/issues/12983), [#12362](https://github.com/trinodb/trino/issues/12362)) document REAL limitations post-evolution. Use Trino `optimize` for **go-forward compaction** within the current spec.
   - **Spark `CALL <cat>.system.rewrite_data_files(table => 'sch.tbl', options => map('rewrite-all','true','target-file-size-bytes','268435456'))`** — the **reliable, documented full-historical-repartition path** on this stack. `rewrite-all=true` forces every file to be rewritten regardless of size, restamping each with the new `spec_id`. Use this for the **guaranteed historical re-layout**.
4. **Framing rule (do NOT flatly say "Trino cannot"):** "Trino `EXECUTE optimize` for go-forward compaction within the current spec; Spark `rewrite_data_files` with `rewrite-all=true` for a guaranteed full historical re-layout after partition evolution." Do not claim DROP+RECREATE is required (false). Do not claim Trino `optimize` FULLY repartitions historical data to an arbitrary new spec (open issues show this is limited).
5. **What pruning the OLD-spec files actually does (do NOT say "old data can't prune / full-scans 400M") — TWO cases, very different:**
   - **TRANSFORM REFINEMENT on the SAME source column** (`month(event_date)` → `day(event_date)`, or `day`→`hour`, or identity→`bucket`): old-spec files **STILL prune — at the OLD, COARSER granularity.** Iceberg projects the predicate through the OLD transform: a query `WHERE event_date = DATE '2026-01-15'` projects to `month(event_date) = '2026-01'`, so the planner KEEPS only the old files in the **2026-01 month partition** and prunes every other month. So an old-day query reads **~one MONTH of old data, NOT all 400M rows.** Day-level pruning on the OLD files is what's missing (they only go to month granularity) — to get day-level pruning on old data, rewrite it under the new spec (STEP 2 below). **It is WRONG to tell the engineer the old data "can't prune at all / scans everything" — it prunes to the old coarser bucket.**
   - **TRANSFORM COARSENING on the SAME source column** (`day(event_date)` → `month(event_date)`, or `hour`→`day`, or `bucket(64,…)`→`bucket(16,…)`) — the REVERSE direction, e.g. switching a too-many-small-daily-partitions table to monthly: old-spec files prune at the OLD **FINER** granularity. The OLD files are `day`-partitioned, so `WHERE event_date = DATE '2026-01-15'` prunes the OLD files to the EXACT **DAY** (fine — the old data already prunes great); only the NEW `month`-spec files prune at the coarser MONTH granularity. **So in a coarsening, do NOT tell the engineer the old data "now only prunes to the month" — old daily files still prune to the day.** ⚠️ Coarsening to fix slow scans is mostly about reducing the FILE COUNT, not improving old-data pruning: the OLD daily files remain many small files (still pruning fine, but lots of them), so the actual speed-up for historical scans comes from `rewrite_data_files` compacting them into the new monthly layout (STEP 2), NOT from the `ALTER` alone.
   - **NEW PARTITION COLUMN added** (e.g. spec went from `day(event_date)` to `(day(event_date), bucket(16, tenant_id))` — a column that did NOT partition the old files): old files have NO partition value for the new column, so a predicate on the **new** column (`WHERE tenant_id = 'acme'`) **cannot prune the old files at all** — they're all scanned for that predicate until rewritten. (Predicates on the *retained* column still prune.) See the LEADING CANONICAL "date-only → (date, tenant_bucket)" block below for this case.
   - **Both cases share the same FIX:** Spark `rewrite_data_files(rewrite-all=true)` (STEP 2) re-stamps old files under the new spec, after which they prune at the new granularity / by the new column.

```sql
-- iter537 PIN — month-to-day evolution, the in-place ALTER (Trino 467, certain) followed by the Spark historical repartition (reliable).
-- STEP 1 (Trino 467, metadata-only, instant) — evolve the spec in place. Affects NEW writes; existing files keep old spec.
ALTER TABLE iceberg.analytics.events SET PROPERTIES partitioning = ARRAY['day(occurred_at)'];  -- was month(occurred_at); now day(occurred_at)
-- STEP 2 (Spark, full historical re-layout) — guaranteed repartition of existing files under the new spec.
-- (Run in spark-submit; CALL iceberg.system.* is Spark SQL only — Trino rejects with a parse error.)
CALL iceberg.system.rewrite_data_files(table => 'analytics.events', options => map('rewrite-all','true','target-file-size-bytes','268435456'));
-- STEP 3 (Trino 467) — reclaim storage from the now-superseded old-spec files.
ALTER TABLE iceberg.analytics.events EXECUTE expire_snapshots(retention_threshold => '7d');
```

> **The certainty levels (load-bearing).** The in-place ALTER (step 1) is **CERTAIN** — directly documented at trino.io. The "Spark `rewrite_data_files` with `rewrite-all=true` is the reliable full-historical-repartition path" is **the recommended pattern on this stack** based on Iceberg docs and the open Trino issues above. The claim that "Trino `EXECUTE optimize` flatly cannot repartition any pre-existing file to a new spec" is **conservative-worded**: Trino docs do not endorse it for that purpose, real Trino issues document gaps, so we don't recommend Trino `optimize` for the historical re-layout — but the open issues are limitations, not a categorical "Trino cannot." Do not paraphrase this nuance as "you MUST use Spark, Trino cannot do it at all." Paraphrase it as "Spark `rewrite_data_files` is the reliable, documented full-historical-repartition path; Trino `optimize` stays in its lane as go-forward compaction within the current spec."

For the deep walkthrough on a 3TB table (the full 5-step procedure with verification queries and per-tenant safe forms), see the canonical below.
...
> ```
>
> **Why Spark for the historical re-layout — the documented nuance.** The Trino 467 docs (verified at [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html)) describe `EXECUTE optimize` as a **bin-pack compaction** ("optimize the layout of the data by combining many small files into larger ones") and are **SILENT on whether it re-stamps existing files into a newly-evolved partition spec**. The framing on this stack is therefore: **Trino `EXECUTE optimize` is the go-forward compaction tool — it cleans up post-evolution small files written under the new spec; Spark `CALL iceberg.system.rewrite_data_files(..., options => map('rewrite-all','true', ...))` is the reliable, documented-by-Iceberg path for a guaranteed FULL HISTORICAL re-layout of pre-existing data files into the new partition spec.** This Spark-rewrite path is the recommended pattern for spec-migration because it (a) forces every file to be rewritten regardless of size, restamping each with the new `spec_id`, and (b) has known compatibility with Iceberg's cross-spec rewrite semantics. Confirmed Trino issues — [trinodb/trino #25279](https://github.com/trinodb/trino/issues/25279), [#12983](https://github.com/trinodb/trino/issues/12983), [#12362](https://github.com/trinodb/trino/issues/12362), [#26109](https://github.com/trinodb/trino/issues/26109), [#26503](https://github.com/trinodb/trino/issues/26503) — document REAL gaps in Trino's `EXECUTE optimize` behaviour around partition evolution (including reports of incorrect/NULL partition values for some scenarios after evolution). **Framing rule for this team: "Trino `EXECUTE optimize` for go-forward compaction within the current spec; Spark `rewrite_data_files` with `rewrite-all=true` for a guaranteed full historical re-layout after partition evolution."** Do **NOT** frame this as "Trino `EXECUTE optimize` flatly cannot do any post-evolution rewrite" (the docs are silent rather than negative) — frame it as "the Spark path is the reliable, documented full-historical-repartition path on this stack."
>
...
> | `ALTER TABLE iceberg.analytics.user_events EXECUTE optimize` as the **guaranteed** historical-repartition path after evolving the spec | Trino docs describe `EXECUTE optimize` as a bin-pack compaction and are **silent on cross-spec re-layout of pre-existing data**. Open Trino issues ([#25279](https://github.com/trinodb/trino/issues/25279), [#12983](https://github.com/trinodb/trino/issues/12983), [#12362](https://github.com/trinodb/trino/issues/12362), [#26109](https://github.com/trinodb/trino/issues/26109), [#26503](https://github.com/trinodb/trino/issues/26503)) document REAL limitations around post-evolution behaviour (including reports of incorrect/NULL partition values for some scenarios). It is not a documented/reliable full-historical-repartition tool on this stack. | For a guaranteed full re-layout of historical data under the new spec, use Spark `CALL iceberg.system.rewrite_data_files(..., options => map('rewrite-all','true', ...))` — the documented Iceberg path. Trino `EXECUTE optimize` is still the right tool for go-forward compaction WITHIN the current spec (small-file cleanup on new writes). Frame the split as "Trino `optimize` = go-forward compaction; Spark `rewrite_data_files` with `rewrite-all=true` = full historical repartition." |
...
### NULL values on a partition column — `WHERE col IS NULL` DOES prune (the slowness is SKEW, not a pruning failure)

> **Keyword anchors:** WHERE partition column IS NULL slow, IS NULL full scan partitioned table, does Iceberg create a partition for NULL, NULL partition Iceberg, IS NULL doesn't prune, NULL rows scattered across files, free-tier NULL plan_tier slow count.

**Iceberg puts ALL rows with a NULL partition value into ONE dedicated partition** — `identity(NULL)` (and every transform of NULL) returns NULL, so the partition tuple has a single `null` entry; NULLs are NOT scattered across every file. And **`WHERE plan_tier IS NULL` DOES prune to just that null partition's files** — the manifest field-summary carries a `contains_null` flag per column, so the planner skips files/partitions with no NULLs exactly like it skips on `col = literal`. **`IS NULL` on an identity-partitioned column is a first-class prunable predicate — do NOT say "IS NULL can't prune / engines conservatively scan all partitions" (that's a Postgres/Oracle sargability prior, FALSE here; cf. the temporal-unwrap pruning canonical in [r18 §90](18-query-performance-regression.md)).**

**So why is `count(*) WHERE plan_tier IS NULL` nearly as slow as a full scan?** Almost always **DATA SKEW, not a pruning miss**: on a SaaS events table, free-tier (= NULL `plan_tier`) is typically the **majority** of rows (often 70–95%). The pruner IS narrowing to the null partition — but that partition legitimately holds most of the table, so reading it ≈ reading everything. Verify with `SELECT partition, record_count, file_count FROM tbl$partitions` — if the null-partition `record_count` is the bulk of the table, pruning is working and the cost is inherent. **Fixes (without dropping partitioning):** (a) backfill a real sentinel value (`'free'`) instead of NULL so the tier is a normal partition you can also sub-bucket; (b) add a second partition transform (e.g. `day(occurred_at)`) so `IS NULL` queries also prune by date; (c) `EXECUTE optimize` to cut the null-partition file count if THAT (many small files) is the real cost; (d) accept it — a `count(*)` over the majority of the table is genuinely a large scan.

> ### POINTER — "How do I CREATE INDEX on my Iceberg table?" / "Can I add an index for fast filtering?"
>
> **The answer is: Trino + Iceberg has NO user-creatable secondary indexes — no `CREATE INDEX`, no `ADD INDEX`, and `PRIMARY KEY` is NOT EVEN GRAMMAR (writing `PRIMARY KEY` in a Trino CREATE TABLE fails at PARSE TIME with `mismatched input 'PRIMARY'` — it does NOT parse-and-ignore, it does NOT store as metadata).** Filter speed comes from the three pruning layers above (partition pruning + file-level min/max + row-group min/max), PLUS Puffin NDV stats for join planning, PLUS optional Parquet bloom filters for high-cardinality equality lookups.
>
> See the LEADING CANONICAL in **[resource 03 § Iceberg mitigations when you DO need point lookups → "Trino + Iceberg has NO user-creatable secondary indexes"](03-columnar-storage.md#iceberg-mitigations-when-you-do-need-point-lookups-on-a-fact-table)** for the full DO-NOT-WRITE matrix (including: `CREATE INDEX` syntax does not exist; `ADD INDEX` does not exist; `PRIMARY KEY` parse-errors — does NOT parse-and-ignore, does NOT store as metadata; you cannot index a view; you cannot index a materialized view) and the four-lever filter-speed table (partition transforms → `sorted_by` + `EXECUTE optimize` → `ANALYZE` → Parquet bloom filters). For the full CREATE TABLE constraint-grammar matrix (what IS vs IS NOT supported), see **[resource 23 § Trino 467 CREATE TABLE: what IS vs IS NOT supported](23-sql-best-practices-olap.md#trino-467-create-table-what-is-vs-is-not-supported--no-primary-key--foreign-key--unique--check--default)**.
...
>
> **The FILTERED single-value case is metadata-only too — `SELECT COUNT(*) FROM events WHERE tenant_id = 'acme'` on an `identity(tenant_id)` table is answered from manifests, NO Parquet data read.** The predicate is satisfied entirely by partition pruning (the manifest knows which files belong to `'acme'`), and the count is summed from those files' `record_count`s. So **do NOT say "a `WHERE` on the partition column forces Trino to read the data files"** — for an identity partition it does not. *(The only exception is merge-on-read position-delete files, which disable the fast path — but then the UNfiltered count is slow too.)* **Corollary diagnosis:** if `COUNT(*) WHERE <identity_partition_col> = X` is nonetheless SLOW (e.g. 30s) while the unfiltered count is fast (2s), the cause is **partition-count explosion / manifest-planning overhead** on a high-cardinality identity partition (millions of tiny partitions slow the *planning*, not a data scan) — diagnose with `SELECT COUNT(*) FROM <t>$partitions` and fix with `bucket()`. See [r18 §Check 2 — "fast unfiltered but slow filtered-by-partition COUNT"](18-query-performance-regression.md) for the full perf-regression decision branch.
>
