...
> | **3** | **Uncompacted MERGE/UPDATE/DELETE residue (MoR position-delete files)** | On Iceberg format-version 2, MERGE/UPDATE/DELETE write **position-delete files** instead of rewriting data files in two cases: (a) any row-level DML run from **Trino 467** (Trino's Iceberg writer is MoR-only regardless of `write.delete.mode` — see [trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272), "Today Iceberg writes only support merge-on-read mode"; full CoW in Trino is a roadmap item, NOT the 467 default), OR (b) DML run from Spark against tables explicitly set to `write.delete.mode = 'merge-on-read'`. On this stack (Trino is the primary DML engine), case (a) is the COMMON source of position-delete files. They accumulate alongside the original data files until `rewrite_position_delete_files` (Spark-only) runs. | Run the `$files` content-type query below. If `content = 1` (positional deletes) rows are >10% of file count, this is it. |
...
> ```
>
> **Verdict:** if `content = 1` rows exist AND `file_count` for content=1 is >10% of total — MoR position-delete files are accumulating. These were created by `MERGE`/`UPDATE`/`DELETE` statements in EITHER of two ways: (a) any row-level DML run from **Trino 467** (Trino's Iceberg writer is MoR-only regardless of property per [trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272) — "Today Iceberg writes only support merge-on-read mode"; full CoW in Trino is a roadmap item, NOT the 467 default), OR (b) DML run from Spark on a format-version-2 table with `write.delete.mode = 'merge-on-read'` explicitly set. **Do not assume the position-delete files imply someone flipped the table property** — on this stack, case (a) is the common source. They MUST be cleaned via Spark's `rewrite_position_delete_files` — Trino 467 does NOT have an EXECUTE procedure for this ([trinodb/trino #27371](https://github.com/trinodb/trino/issues/27371) tracks adding it). Trino's `EXECUTE optimize` does fold position deletes only when processing whole partitions without `file_modified_time` or path predicates ([trinodb/trino #24086](https://github.com/trinodb/trino/issues/24086)); per-tenant or partial-partition scoped rewrites need Spark.
>
> ```sql
...
> **ALSO the answer to CLUSTER-TRIAGE / PERFORMANCE narratives (not just "cost"):** "the cluster is **sluggish / slow during business hours**, which queries are **hammering it / eating resources** before we add hardware", "show me **what's running**, the **longest-running** queries, the ones **reading the most data**", "**top resource-consuming** / heaviest queries". Same recipe — these are about resource consumption, which IS what this `system.runtime.queries`/`tasks` recipe measures. (Deeper performance-triage version + cross-references: r18 §"Finding expensive queries on Trino 467".)
>
...
> | `EXECUTE optimize(rewrite_position_delete_files => true)` | **No such parameter exists in Trino 467** — `optimize`'s only parameter is `file_size_threshold`. But you do NOT need a special parameter: a full `EXECUTE optimize` already **applies the position-deletes for the data files it rewrites and drops the orphaned delete files** ([trinodb/trino#12617](https://github.com/trinodb/trino/issues/12617), PR #12704; whole-partition / no path-predicate per [#24086](https://github.com/trinodb/trino/issues/24086)). The Spark `rewrite_position_delete_files` procedure compacts the delete files *only* (no data rewrite) and is an **optional** alternative, not the only route. | A full `ALTER TABLE iceberg.analytics.events EXECUTE optimize(file_size_threshold => '512MB')` folds in the position deletes natively (raise `file_size_threshold` above the delete-bearing data files' size so they're rewrite candidates — Trino selects by size only, no `delete-file-threshold` on 467, [#16574](https://github.com/trinodb/trino/issues/16574)). Spark `CALL iceberg.system.rewrite_position_delete_files(table => 'analytics.events')` is the optional cheaper delete-file-only path. |
...
> | "Trino's `EXECUTE optimize` can NEVER touch position-delete files — only Spark can clear them, so a Trino-only shop is stuck" | **WRONG (over-broad).** A full Trino `EXECUTE optimize` rewrites data files and, for the files it rewrites, **applies the position-deletes and removes the orphaned delete files** ([trinodb/trino#12617](https://github.com/trinodb/trino/issues/12617), PR #12704; cleaned up when there are no path / file_modified_time predicates per [#24086](https://github.com/trinodb/trino/issues/24086)). So a **Trino-only shop CAN clear delete-file read overhead.** The one real limit: Trino `optimize` selects candidates by SIZE only (no `delete-file-threshold`, [#16574](https://github.com/trinodb/trino/issues/16574)), so if delete-bearing data files are already ≥ the 100MB default they're skipped. | If `"<table>$files" WHERE content = 1` shows many delete files, run `ALTER TABLE iceberg.analytics.events EXECUTE optimize(file_size_threshold => '512MB')` (raise the threshold above those data files' size to force the rewrite — clears the deletes from Trino, no Spark). Spark `CALL iceberg.system.rewrite_position_delete_files(...)` (delete-file-only) or `rewrite_data_files(options=>map('delete-file-threshold','1'))` (delete-driven selection) are OPTIONAL — use only if you want delete-driven candidate selection without raising the size threshold. |
...
> **This is the findable canonical answer for storage-tiering questions on the on-prem Trino 467 + Iceberg 1.5.2 + MinIO stack. Every capability claim below is verified against [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) (table-property list + EXECUTE registry) and [docs.min.io/enterprise/aistor-object-store/administration/object-lifecycle-management/](https://docs.min.io/enterprise/aistor-object-store/administration/object-lifecycle-management/) (object tiering). Keywords this block answers: "tier", "tiering", "hot", "cold", "warm", "archive", "storage class", "storage tier", "lifecycle", "move old partitions", "cheaper storage", "S3 Glacier equivalent", "move cold data to cheaper storage tier", "automatically tier old data", "archive cold data older than 90 days", "archive cold data older than 6 months", "move old data to cheaper storage", "move old data to slower storage", "hot/warm/cold storage tiering", "tier old Iceberg data on MinIO", "lifecycle policy to move old data", "cheaper storage for cold history", "S3-Glacier-equivalent on MinIO", "storage tiering Trino vs Iceberg vs storage-layer", "is tiering a Trino setting or table property or storage layer", "move cold partitions to cheaper tier", "automatic tiering for old data", "archive old data to cold tier", "tier data by age", "lifecycle rule to transition old objects", "does tiering track last-read / last-access / read recency", "is tiering based on how recently a file is queried", "move data that hasn't been read/accessed in N days", "keep frequently-accessed old data on the hot/warm tier", "access-aware tiering", "usage-based tiering", "access-pattern tiering", "S3 Intelligent-Tiering equivalent on MinIO", "tier by access frequency not age", "frequently-queried two-year-old data staying warm", "hot data that nobody reads vs cold data that gets queried". **NOTE — tiering on this stack is by OBJECT AGE (days since creation), NOT by read/query access recency; see the "Access-aware tiering" capability-bound row below and the CRITICAL age-not-access note in Mechanism A.** Read this BEFORE inventing DDL — there is no `SET STORAGE TIER` statement on Trino 467, no Iceberg table property for tier, no Trino catalog property for tier; **tiering lives at the MinIO object-lifecycle layer** (`mc ilm tier add` + `mc ilm rule add --transition-days N --transition-tier <TIER_NAME>`), NOT at the Trino or Iceberg layer.**
>
> **TL;DR — answer the tiering question in three lines (read these FIRST, before the detail below):**
> 1. **YES, age-based tiering exists AND keeps data queryable** — set it up at the MinIO object-lifecycle layer: `mc ilm tier add` registers a cheaper cold tier, then `mc ilm rule add --transition-days N --transition-tier <TIER>` transparently moves data files older than N days to it. **Trino keeps querying the tiered data with no SQL/config change** (cold reads are just slower). This is **Mechanism A** below — it is the real answer to "move old data to cheaper storage." (On-prem MinIO DOES support this; do NOT say "MinIO can't tier.")
> 2. **NO, it is NOT read-access / usage based** — the transition trigger is **OBJECT AGE (days since creation)**, never last-read / query-frequency. There is no "move files not read in N days" mode. If you truly need usage-driven tiering (S3 Intelligent-Tiering style), build it app-side (**Mechanism C** — split tables by age). So old-but-still-hot data WILL transition at its age threshold; if that matters, keep it in a separate hot table (Mechanism C).
> 3. **DO NOT just `DELETE` old data to "save storage"** — deletion DESTROYS the deep-history queries you still need. Tiering (Mechanism A) makes old data *cheaper but still queryable*; deletion makes it *gone*. These are different operations — never substitute `DELETE FROM ... WHERE created_at < ...` for a tiering request.
> 4. **Tiering fixes COST but makes recurring historical reports SLOW (cold reads). To keep those reports fast, pre-aggregate onto the HOT tier — DON'T un-tier the raw data.** *(Keyword anchors: year-over-year / YoY report slow on cold data, deep-history report 30 minutes after tiering, multi-year trend report scans cold storage, keep historical dashboards fast but data stays cheap, monthly executive dashboard slow on archived data.)* A query that SPANS the cold archive will pay the cold-read latency every time — neither Mechanism A nor a hot/archive UNION view changes that (the spanning query still reads the archive). The standard lakehouse fix is a **pre-aggregated ROLLUP/summary kept on the hot tier** that the recurring report reads INSTEAD of the cold detail: build it once and refresh on a schedule. Two forms — (a) a **dbt rollup model** (`materialized='table'` or `'incremental'`) that GROUPs the detail to month/quarter grain and lands on hot MinIO; the YoY dashboard queries the tiny rollup, never the cold raw. (b) a **Trino materialized view on Iceberg** (`CREATE MATERIALIZED VIEW ... GRACE PERIOD INTERVAL '...' WHEN STALE INLINE AS SELECT date_trunc('quarter', ...) ... GROUP BY ...` + scheduled `REFRESH MATERIALIZED VIEW`); the MV's storage table is on the hot tier and the base cold scan runs only once per refresh, not per dashboard hit. **See [resource 25 — Trino materialized views on Iceberg](25-trino-materialized-views-iceberg.md)** for the MV syntax/refresh/grace-period details, and [resource 07 §rollup-table patterns](07-analytical-query-patterns.md) / the cost-driver "rollup tables to skip repeated compute" note above for the dbt-model form.
>
...
> | **Access-aware tiering** — "move files that haven't been *read/queried* in N days to cold, and keep frequently-queried old data warm" | **NO — tiering is by OBJECT AGE (days since creation), NOT by read/access recency.** MinIO `mc ilm rule add --transition-days N` triggers purely on *days since object creation* (or days-since-noncurrent for versioned objects). It does NOT inspect last-read / last-access / GET-frequency — there is no `--transition-access-days` or `--last-accessed-days` flag. So a heavily-queried 2-year-old file STILL transitions to cold at its age threshold, and a recent file nobody reads stays hot until it ages out. If you genuinely need access-frequency-based tiering (what AWS S3 Intelligent-Tiering does), you must implement it yourself at the app layer (Mechanism C — split tables by age and move rows on a schedule). | docs.min.io object-lifecycle-management (transition is creation-age-based; see Mechanism A "CRITICAL — transitions are by OBJECT AGE" note) |
>
