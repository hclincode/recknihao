...
| "Trino's `LIKE` can't match a **literal** `%` or `_` — you have to drop to a regex function (`regexp_like`)." | **FALSE — `LIKE` has an `ESCAPE` clause for exactly this.** To match a literal percent sign (e.g. find rows containing `"30%"`), write `WHERE col LIKE '%\%%' ESCAPE '\'` — the `\%` is a literal `%`, the surrounding `%…%` stay wildcards. For a literal underscore: `LIKE '%\_%' ESCAPE '\'`. You may pick any escape char (`ESCAPE '!'` with `'%!%%'`). Do **NOT** bail to `regexp_like` for a simple literal-wildcard match — `ESCAPE` is the direct, standard-SQL answer. (Regex is the right tool for character classes / anchors / quantifiers, not for one literal `%`.) | [resource 23 § LEADING CANONICAL — match a LITERAL `%` or `_` (LIKE ESCAPE)](#leading-canonical--match-a-literal--or-_-escape-a-wildcard-in-like--like-pattern-escape-c-and-the-substring-test-is-strposs-sub--0-not-contains-which-is-array-only-iter751-pin--fix-a), [trino.io comparison.html](https://trino.io/docs/467/functions/comparison.html) |
...
> **Keyword anchors:** Trino format function, printf Trino, build display string, format string Trino, String.format Trino, format number with commas, format vs concat, format thousands separator, format percent, zero-pad integer Trino, formatted display string Trino, **format a number as CURRENCY / dollar amount, format money with commas and 2 decimals, $1,234,567.89, Oracle `TO_CHAR(amount, 'FM$999,999,999.00')` equivalent, Oracle FM number mask, number format mask in Trino, format a number with a thousands separator and currency symbol, format_number**. **The currency/number-format answer is `format('$%,.2f', amount)`** (`%,.2f` = thousands-grouped + 2 decimals; prefix the literal `$`) → `format('$%,.2f', 1234567.89)` = `'$1,234,567.89'`. Do NOT punt number-formatting to the app layer — `format()` does it in SQL. (NB: `format_number` IS a Trino 467 built-in but only does compact-units `'1.2M'`/`'500K'` magnitude — it does NOT accept an Oracle/Spark `'#,##0.00'` mask; do not write `format_number(x, '#,##0.00')`.) Verified at [trino.io/docs/467/functions/string.html](https://trino.io/docs/current/functions/string.html).
...
> ```
>
> **⚠️ CASE PARITY WITH POSTGRES/MySQL — Trino `to_hex` is UPPERCASE; Postgres/MySQL `md5()` is lowercase.** If you are migrating from Postgres (where `md5(text) -> text` returns a **32-char LOWERCASE** hex string, e.g. `'5d41402abc4b2a76b9719d911017c592'`) and you need the **same** value (so an existing key/join/dedup against Postgres-produced hashes still matches, or to match `dbt_utils.generate_surrogate_key`'s lowercase output), **wrap with `lower(...)`**: `lower(to_hex(md5(to_utf8(x))))`. Bare `to_hex(md5(...))` yields UPPERCASE (`'5D41402A...'`) and will **silently mismatch** lowercase Postgres hashes — a zero-row join / always-new surrogate key with NO error. The DIGEST bytes are identical across engines; only the hex rendering's case differs.
>
> **DO NOT WRITE:**
...
> **Safe-cast anchors (NULL-on-failure, not the `::` operator — route here too):** safe cast Trino, cast that returns NULL on invalid input, convert string to number ignore bad values, parse string to number return null on error, `TRY_CAST` vs `CAST`, cast bad/dirty values to NULL instead of erroring, `TRY_CAST(col AS DECIMAL)`, `TRY_CAST(col AS BIGINT)`, non-numeric value should become NULL not fail the query. **The fix is `TRY_CAST(x AS type)` — returns NULL on any cast failure instead of erroring the whole query** (plain `CAST` throws). For wrapping a whole expression (not just a cast), see `try(expression)` at [Resource 27 §4.4E](27-oracle-plsql-to-dbt-trino.md).
...
**Need to know what type a column actually is before you CAST it?** To **debug a type mismatch** — "what data type is this column", "is this column a string or a number", "column behaves as a string sometimes and a number sometimes" — ask Trino directly with `typeof(expr) -> varchar` (e.g. `SELECT typeof(order_total) FROM t LIMIT 1;` returns the runtime type name like `'varchar(20)'` or `'decimal(18,2)'`). See [Resource 27 §4.4B — INSPECT-THE-TYPE CANONICAL](27-oracle-plsql-to-dbt-trino.md) for worked examples (including why `typeof(json_extract(...))` is `'json'` not a number). Once `typeof` tells you the type, use `CAST` / `TRY_CAST` to change it.
...
> **DECISION — "first AND last status per ticket in ONE row" → `min_by` + `max_by` AGGREGATES, NOT window functions mixed with `GROUP BY` (iter656 PIN — landing-point lock).** *Keyword anchors:* first and last status per ticket, first and latest value per group in one row, earliest and most recent status per entity, opening and closing status, first_value and last_value per group, status at first change and status at last change, first product and most recent product per customer, opening status and closing status per ticket, first event status and last event status per ticket, first and last reading per device, first and last login status per user, first and latest login status per user, earliest and most recent value per group, first and last STATE per ID, status of first login and status of last login per user, opening and closing value per entity, first and current plan tier per subscriber, first and latest reading per sensor. **The shape:** *"give me both the **FIRST** and **LAST** value of a column (e.g. `status`) per group (e.g. `ticket_id`), ordered by a timestamp column (e.g. `changed_at`), in ONE row per group."* **The answer is two AGGREGATE calls in one GROUP BY pass:** `min_by(status, changed_at) AS first_status, max_by(status, changed_at) AS latest_status ... GROUP BY ticket_id`. `min_by` / `max_by` are **AGGREGATE functions** (per [trino.io/docs/467/functions/aggregate.html](https://trino.io/docs/467/functions/aggregate.html)) — they produce **one row per GROUP BY group**, which is exactly what the question asks for. **Do NOT mix `first_value()` / `last_value()` WINDOW functions with `GROUP BY` on this** — a window function in the SELECT list that references a column not in `GROUP BY` and not wrapped in an aggregate fails Trino's analyzer rule (verified at [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html): *"When a `GROUP BY` clause is used in a `SELECT` statement all output expressions must be either aggregate functions or columns present in the `GROUP BY` clause."*). The window form WITHOUT `GROUP BY` yields one row per **EVENT** (not per ticket) and would need `SELECT DISTINCT` to collapse — strictly worse than the aggregate form. Cross-link: see the [`min_by` / `max_by` worked example](#max_byx-y--min_byx-y--deterministic-pick-of-x-by-the-ordering-column-y) right above, the [first_value / last_value default-frame note in resource 07](07-analytical-query-patterns.md#first_value--last_value--window-value-functions), and the [Trino GROUP BY rules anchor in resource 07](07-analytical-query-patterns.md#trino-group-by-rules-anchor--apply-to-every-group-by-query).
>
> **PREFERRED ✅ — the canonical shape (one row per ticket, two columns):**
> ```sql
> -- Trino 467 — first AND latest status per ticket in ONE row, ordered by changed_at.
> SELECT ticket_id,
>        min_by(status, changed_at) AS first_status,
>        max_by(status, changed_at) AS latest_status
> FROM iceberg.support.ticket_events
> GROUP BY ticket_id;
> ```
> One pass, no self-join, no window function, no subquery. `min_by(status, changed_at)` returns the `status` from the row with the **minimum** `changed_at` per `ticket_id`; `max_by(status, changed_at)` returns the `status` from the row with the **maximum** `changed_at` per `ticket_id`.
>
> **DO NOT WRITE ❌ — window functions mixed with GROUP BY on the ungrouped column (iter655 Q3 bug, EXACT shape):**
> ```sql
> -- INVALID Trino 467 — analyzer error. The window functions reference `status` and
> -- `changed_at`, neither of which is in `GROUP BY ticket_id` nor wrapped in an
> -- aggregate. Per the SELECT-GROUP-BY rule (trino.io/docs/467/sql/select.html), all
> -- output expressions must be aggregates or grouping columns. Window functions are
> -- neither — they cannot reference ungrouped columns under GROUP BY.
> SELECT ticket_id,
>        first_value(status) OVER (PARTITION BY ticket_id ORDER BY changed_at) AS first_status,
>        last_value(status)  OVER (PARTITION BY ticket_id ORDER BY changed_at
>                                  ROWS BETWEEN UNBOUNDED PRECEDING
>                                           AND UNBOUNDED FOLLOWING)            AS last_status
> FROM iceberg.support.ticket_events
> GROUP BY ticket_id;
> ```
> **Two correct fixes** if you really want the window-function form (but `min_by`/`max_by` is shorter): (1) **drop `GROUP BY ticket_id`** so the windows operate per-row, then add **`SELECT DISTINCT`** to collapse to one row per ticket — the window form yields one row per **event**, so without `DISTINCT` you'd get N rows per ticket all with the same first/last values; or (2) use the **`min_by` / `max_by` aggregate form** above (preferred — one pass, no DISTINCT cost). The `last_value(...) ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING` frame is the **correct** frame override for "last value across the whole partition" (the responder got that detail right — see [first_value / last_value default-frame lock in resource 07](07-analytical-query-patterns.md#first_value--last_value--window-value-functions)) — **only the combination with `GROUP BY` broke it.** Mnemonic: **"first AND last in ONE row per group" = TWO aggregates (`min_by` + `max_by`) with ONE `GROUP BY`. Window functions are for keeping detail rows, not collapsing to one row per group.**
>
> **Tie / determinism note.** `min_by(x, y)` / `max_by(x, y)` pick **one** `x` at the extreme `y` — if multiple rows tie on `y`, the chosen `x` is **arbitrary** (Trino docs do not specify tie-breaking). If `changed_at` can tie within a ticket, add a deterministic tiebreaker by passing a `ROW(...)` as the ordering arg: `min_by(status, ROW(changed_at, event_id)) AS first_status` (lexicographic comparison on the row's fields).
...
| For the MODE, writing `COUNT(*) OVER (PARTITION BY group_key ...) AS freq` in the **same** query that has `GROUP BY group_key, value` (then `ROW_NUMBER() ... ORDER BY freq DESC`). | **BROKEN — returns the wrong value.** After `GROUP BY group_key, value` the rows are already one-per-`(group_key, value)` pair; a **window** `COUNT(*) OVER (PARTITION BY group_key)` then counts *how many distinct values that group has* (constant for every row of the group), NOT each value's frequency. So `freq` ties across the whole group and `ROW_NUMBER() ORDER BY freq DESC, value` just returns the **alphabetically-first** value — the same bug as `MAX(value)`. **The frequency must be a PLAIN AGGREGATE `COUNT(*)` produced by the `GROUP BY (group_key, value)` itself**, not a window `COUNT(*) OVER`. Use the `max_by(value, cnt)` over a `COUNT(*) GROUP BY (group_key, value)` subquery from the [LEADING CANONICAL above](#leading-canonical--mode--most-frequent-value-per-group-the-single-most-common-x-per-y) — do NOT mix a window function with `GROUP BY` here (cf. §3.1G the same window-vs-GROUP-BY rule). |
...
> **READ THIS FIRST if your question contains any of these keywords: `did any shipment arrive late`, `did any row in the group satisfy X`, `did all rows satisfy X`, `any true in a group`, `all true in a group`, `roll up a yes/no flag per group`, `bool_or`, `bool_and`, `group boolean any/all`, `at least one row is true`, `every row is true`, `aggregate a boolean per group`, `single true/false per group`, `did any order have a late shipment`, `did every shipment arrive on time`, `boolean aggregation`, `roll up bool`, `collapse boolean per group`, `has this user ever done X`, `has ever done X`, `has the account ever triggered X`, `did the customer ever buy X`, `ever-flag per user`, `a real true/false boolean column per user`, `give me a true/false per user`, `has the user ever logged in / purchased / failed`, `boolean flag per user from their history`.** Verified at [trino.io/docs/467/functions/aggregate.html](https://trino.io/docs/467/functions/aggregate.html) on 2026-06-07.

> **Note — the "has ever done X" / user-history framing routes here too.** "Has this user **ever** done X → give me a single TRUE/FALSE per user" is the SAME aggregate shape as "did ANY row in the group satisfy X" — the group is the user's history. The canonical answer is `bool_or(pred)` grouped by user: e.g. `SELECT user_id, bool_or(event_type = 'purchase') AS has_ever_purchased FROM events GROUP BY user_id` returns a real `boolean` column. Prefer this over `MAX(CASE WHEN pred THEN 1 ELSE 0 END)` + `CAST(... AS boolean)` (which returns 1/0 then casts) and over `count_if(pred) > 0` (which counts then compares) — `bool_or(pred)` is the docs-canonical, returns `boolean` directly, and reads as the intent.
...
| **The TOP-N ROWS per group as a RANGE, INCLUDING ties at the Nth-place boundary** (e.g. "top 5 accounts by revenue per `plan_tier`, and if the 5th and 6th tie show BOTH" → a tie at the cutoff yields MORE than N rows) | **`RANK() OVER (PARTITION BY g ORDER BY x DESC) <= N`** (or the ANSI `... FETCH FIRST N ROWS WITH TIES` form) — **see the dedicated [top-N-with-ties LEADING CANONICAL at §below](#leading-canonical--top-n-rows-per-group-with-ties-fetch-first--rank--n-not-dense_rank--n-iter714-pin--fix-a2)** | `RANK <= N` keeps every row whose rank is ≤ N, so boundary ties (both at rank N) BOTH pass — the gap *after* the tie is HARMLESS for a `<= N` **range** filter (the silent-empty gap problem only bites the `= N` **exact** case above). **DO NOT use `DENSE_RANK() <= N` for this** — `DENSE_RANK` numbers DISTINCT VALUES, so a tie ABOVE the boundary compresses the sequence and `<= N` reaches the (N+1)th-or-lower ACCOUNT, over-returning rows that are NOT in the true top-N-by-position. `DENSE_RANK` is for "top-N distinct VALUE-tiers", NOT "top-N rows/accounts". |
| **"The Nth" using `RANK()` = N** | **DO NOT — see DO NOT WRITE below** | `RANK` = 1, 1, 3 with ties (**GAP after ties**); if the top is tied, `WHERE rank = 2` matches **NOTHING** (the sequence skips past 2 to 3) and the query silently returns zero rows for those customers — no error, just wrong. (NOTE: this gap is harmless for the `<= N` **range** row above — it only breaks the `= N` **exact** match here.) |
...
> **READ THIS FIRST if your question contains any of these keywords:** `top 10 including ties`, `top N with ties at the cutoff`, `include everyone tied for 10th place`, `include all rows tied at position N`, `top 10 leaderboard including ties at the 10th spot`, `top 10 with ties`, `FETCH FIRST WITH TIES`, `FETCH FIRST n ROWS WITH TIES`, `leaderboard with ties`, `top 5 products with ties`, `RANK vs DENSE_RANK vs ROW_NUMBER for top-N`, `which ranking function for top-N with ties`, `does RANK <= N miss ties`, `does RANK<=10 miss the 10th row if it ties`, `top N keep ties`, `top N including ties at boundary`, **`top N PER GROUP with ties`, `top 5 accounts per plan_tier including both tied accounts`, `top N per partition / per category / per region including ties at the Nth spot`, `top-N-per-group keep boundary ties`, `RANK() OVER (PARTITION BY ...) <= N`, `both tied accounts should appear in the top 5`, `ROW_NUMBER dropped one of two tied rows at the cutoff`**. **Verified at [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html) on 2026-06-08.**
>
> **PER-GROUP vs WHOLE-RESULT — pick the right tool.** `FETCH FIRST n ROWS WITH TIES` applies to the **whole result set only** (it's a query-level clause — there is no per-partition `FETCH`). For **top-N WITHIN EACH GROUP including boundary ties** (e.g. "top 5 per `plan_tier`"), use **`RANK() OVER (PARTITION BY g ORDER BY x DESC) <= N`** in a subquery + outer `WHERE rnk <= N`. **NOT `DENSE_RANK() <= N`** (that returns top-N distinct VALUE-tiers per group, over-returning accounts when ties exist above the boundary — see the DECIDE-FIRST router at the [Nth-largest canonical above](#leading-canonical--second-largest--nth-largest-per-group-choose-dense_rank-vs-row_number-do-not-use-rank--it-leaves-gaps-after-ties-and-silently-returns-nothing-iter643-pin--fix-a)). NOT `ROW_NUMBER() <= N` (drops one of every tied pair at the cutoff — the exact bug being fixed).
>
...
>
> *Keyword anchors (READ THIS FIRST if your question contains any of these):* **days between two dates, days since signup, time to first purchase in days, tenure in days, account age in days, elapsed days, how many days from X to Y, days a ticket stays open, days from signup to first purchase, days to convert, days until first event, days between event A and event B, lag in days, gap in days, minutes between two timestamps, seconds between two timestamps, hours between events, time between events, gap between consecutive events, duration between timestamps, dwell time, time to first response, response time in minutes, ticket age in hours, session gap, 30 minute gap, new session when gap exceeds 30 minutes, sessionization gap test, time-between-events filter, ts2 minus ts1, timestamp minus timestamp.** Verified at [trino.io/docs/467/functions/datetime.html](https://trino.io/docs/467/functions/datetime.html) on 2026-06-08 (operator table + `date_diff` signature).
>
> **THE ONE FACT — Trino has NO temporal-minus-temporal-yields-interval operator. NO `date − date`. NO `timestamp − timestamp`. NO `time − time`.** The Trino 467 operator table at [trino.io/docs/467/functions/datetime.html](https://trino.io/docs/467/functions/datetime.html) lists EXACTLY these subtraction operators on temporal types: `date - interval -> date`, `time - interval -> time`, `timestamp - interval -> timestamp`, and `interval - interval -> interval`. **All four take an INTERVAL on the right side, and ALL four return a temporal value (or interval), not a number.** There is **NO `date - date -> integer`**, **NO `date - date -> bigint`**, **NO `date - date -> interval`**, AND **NO `timestamp - timestamp -> interval`**. Whether you are counting **days between two dates** or **minutes between two timestamps** (sessionization, time-to-first-response, dwell time, gap between consecutive events), the integer count is computed exclusively with **`date_diff('unit', earlier, later) -> bigint`**. Per the docs verbatim: `date_diff(unit, timestamp1, timestamp2)` *"Returns `timestamp2 - timestamp1` expressed in terms of `unit`"* — so the result is **POSITIVE when `timestamp2` is later than `timestamp1`** (i.e. the **later** date/timestamp goes in the **third** argument). Valid `unit` values include `'second'`, `'minute'`, `'hour'`, `'day'`, `'week'`, `'month'`, `'quarter'`, `'year'`.
>
> **What IS valid arithmetic — don't over-ban this.** `timestamp + INTERVAL '30' MINUTE` IS valid (timestamp + interval -> timestamp). `timestamp - INTERVAL '30' MINUTE` IS valid (the right side is an **INTERVAL LITERAL**, not another timestamp — interval on the right means "shift this timestamp by that interval"). `current_date - INTERVAL '7' DAY` IS valid. **Only the subtraction-of-TWO-temporal-columns form is invalid.** A quick discriminator: if both sides of the `-` are temporal **values** (dates or timestamps), it's invalid; if the right side is an INTERVAL literal, it's valid.
>
...
> |---|---|---|
> | `CAST(first_purchase_date - signup_date AS bigint) AS days_to_purchase` | **INVALID Trino 467** — there is no `date - date -> integer` operator (and NO `date - date -> interval`, NO `date - date -> bigint`). Per [trino.io/docs/467/functions/datetime.html](https://trino.io/docs/467/functions/datetime.html) verified 2026-06-08: the operator table documents only `date - interval -> date`, `time - interval -> time`, `timestamp - interval -> timestamp`, and `interval - interval -> interval`. **There is also NO `timestamp - timestamp -> interval` operator** — Trino has no temporal-minus-temporal-yields-interval operator at all (date or timestamp). The `CAST` does not rescue the expression because the inner subtraction never parses at all. | `date_diff('day', signup_date, first_purchase_date) AS days_to_purchase` |
...
> | `EXTRACT(DAY FROM (first_order_date - signup_date))` | Same root problem — `first_order_date - signup_date` does not produce a Trino value at all (no `date - date` operator). **The same is true for `EXTRACT(MINUTE FROM (ts2 - ts1))`**: there is NO `timestamp - timestamp -> interval` operator either, so `ts2 - ts1` itself fails before `EXTRACT` ever runs. | `date_diff('day', earlier, later)` (or `date_diff('minute', earlier, later)` for timestamps) — single function, returns total whole units as `bigint`, no operator-existence surprises. |
...
> | `event_time - LAG(event_time) OVER (PARTITION BY user_id ORDER BY event_time) > INTERVAL '30' MINUTE` (the iter670 Q3 bug — sessionization gap test) | **INVALID Trino 467** — there is NO `timestamp - timestamp -> interval` operator. The left side (a subtraction of two timestamp values) does not parse at all, regardless of what the right side is. The presence of `INTERVAL '30' MINUTE` on the right does NOT make the left-side `timestamp - timestamp` legal — interval-on-the-right rescues `timestamp - INTERVAL '30' MINUTE` (one timestamp, one interval) but does NOT rescue `timestamp - timestamp`. | `date_diff('minute', LAG(event_time) OVER (PARTITION BY user_id ORDER BY event_time), event_time) > 30` — `date_diff` returns `bigint`, so compare against a **plain number** (`> 30`), NOT an `INTERVAL` literal. See the [sessionization 30-min-gap canonical in resource 07 §Pattern B-Session](07-analytical-query-patterns.md#leading-canonical--sessionization--30-minute-gap--gaps-and-islands--new-session-when-gap-exceeds-threshold-iter671-pin) for the full worked example. |
> | `ts2 - ts1 > INTERVAL '5' SECOND` (any "time between events" / "duration > N" gap test) | **INVALID Trino 467 — same reason as the row above.** Even when the comparison is `> INTERVAL`, the LEFT side `ts2 - ts1` is a `timestamp - timestamp` subtraction that does not parse. | `date_diff('second', ts1, ts2) > 5` (or `'minute'`, `'hour'` as the unit needs). |
> | `closed_at - opened_at` (ticket age in hours; expecting an interval) | **INVALID Trino 467** — no `timestamp - timestamp -> interval` operator exists. Even though Postgres and Spark support this form, Trino does not. | `date_diff('hour', opened_at, closed_at)` (returns `bigint` hours). |
> | `first_reply_at - created_at` (time-to-first-response duration) | **INVALID Trino 467** — same reason. | `date_diff('minute', created_at, first_reply_at)` (returns `bigint` minutes; pair with `format('%dh %dm', d / 60, d % 60)` for a display string per [§3.1A `concat`-with-`date_diff` canonical](#31a-format-vs-concat-with-date_diff-bigint-results)). |
>
> **One-line rule.** *Any* "days between X and Y" / "tenure in days" / "elapsed days" / "time to event in days" / **"minutes between two timestamps"** / **"seconds between two events"** / **"time gap between consecutive events for sessionization"** / **"duration between two timestamps"** question is **`date_diff('<unit>', earlier, later) -> bigint`** — never `date - date`, never `timestamp - timestamp`, never `CAST(date - date AS bigint)`, never `EXTRACT(DAY FROM date - date)`, never `DATEDIFF`. Compare the result against a **plain number** (`> 30`, `>= 5`, `BETWEEN 1 AND 90`), NOT against an `INTERVAL` literal. And whatever expression you reference in the outer query (whether inside `approx_percentile`, `WHERE`, or `ORDER BY`) **must already be in the CTE's `SELECT` list under that name**.
>
...
| **`LIMIT n OFFSET m`** (LIMIT placed BEFORE OFFSET — pagination "page N, M rows per page") | PostgreSQL, MySQL, SQLite | **PARSE ERROR.** Trino's clause order is FIXED: `ORDER BY → OFFSET → LIMIT`/`FETCH`. Per [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html) the synopsis lists `[ OFFSET … ] [ LIMIT … ] [ FETCH … ]` and the documented example is `ORDER BY x OFFSET 2 LIMIT 2`. Writing `… ORDER BY created_at DESC LIMIT 25 OFFSET 75` fails with `mismatched input 'OFFSET'` — **OFFSET must come BEFORE LIMIT.** This clause order is the ONE genuine PostgreSQL/MySQL→Trino pagination syntax difference (do NOT assume "same as Postgres"). | **Write `OFFSET m LIMIT n` (OFFSET first).** Page 4 at 25 rows/page (skip first 75, return next 25): `SELECT * FROM orders ORDER BY created_at DESC OFFSET 75 LIMIT 25`. `FETCH FIRST 25 ROWS ONLY` also works *after* OFFSET (it is SUPPORTED — see the `FETCH FIRST` row below; do NOT claim Trino lacks FETCH). For DEEP pages prefer **keyset / cursor pagination** — `WHERE id < :last_seen_id ORDER BY id DESC LIMIT 25` — because `OFFSET` still scans and discards the skipped rows (it does not reduce scan cost). (keyword anchors: pagination Trino, paginate Trino, page N Trino, page 3 of results, M rows per page, LIMIT OFFSET order Trino, OFFSET before LIMIT, OFFSET after LIMIT, skip the first N rows Trino, next page Trino, LIMIT 25 OFFSET 75, mismatched input OFFSET, Postgres pagination in Trino, is Trino pagination different from Postgres, offset limit syntax Trino.) |
...
| **`NOW() AT TIME ZONE 'UTC'`** | PostgreSQL syntax | **Different semantics.** Trino's `current_timestamp AT TIME ZONE 'UTC'` works on `TIMESTAMP WITH TIME ZONE`. | Use `current_timestamp AT TIME ZONE 'UTC'`, or `at_timezone(ts, 'UTC')`. For a per-row zone held in a COLUMN (not a literal), `at_timezone(ts, zone_column)` accepts a varchar column/expression — see [resource 07 § Fact 3b — dynamic per-row timezone](07-analytical-query-patterns.md). |
...
- **HyperLogLog (HLL)**: the probabilistic sketch behind **`approx_distinct(x)`** (approximate COUNT DISTINCT / cardinality). **2.3% standard error by default** per the Trino 467 aggregate-functions docs. **NOT** the algorithm behind `approx_percentile`.
- **Quantile-digest / T-Digest (qdigest)**: the probabilistic sketch behind **`approx_percentile(x, p)`** (approximate quantile — p50/p95/p99/etc). **NOT** HyperLogLog. The Trino docs do not publish a single fixed-percent error for this overload; there is no `approx_percentile(x, p, accuracy)` overload — accuracy lives on the separate `qdigest_agg(x, w, accuracy)` function. **Mnemonic:** HLL = cardinality (= `approx_distinct`, 2.3%); qdigest/T-digest = quantile (= `approx_percentile`, no fixed-% error). Do NOT swap them.
