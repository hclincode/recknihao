...
> **`approx_set` precision is FIXED at ~2.3% — there is NO `approx_set(x, e)` overload** (verified at [trino.io/docs/current/functions/hyperloglog.html](https://trino.io/docs/current/functions/hyperloglog.html); the only signature is `approx_set(x) -> HyperLogLog`). **Keyword anchors:** approx_set precision, tune HLL sketch error, approx_set no second argument, approx_distinct vs approx_set precision, tighter than 2.3% distinct sketch. Every sketch you persist and later `merge()` shares the one fixed precision — you cannot store a tighter sketch by passing a precision argument. To get tighter than 2.3%: use the scalar **`approx_distinct(x, e)`** form (e.g. `approx_distinct(user_id, 0.01)` ≈ 1% standard error — valid range `[0.0040625, 0.26000]`; NOT a stored / mergeable sketch) OR fall back to exact `COUNT(DISTINCT)`. **DO NOT WRITE:** `approx_set(x, e)` with a 2nd precision arg (does NOT exist — only `approx_distinct` takes `e`).
