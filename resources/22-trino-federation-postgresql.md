...
| "Aggregate pushdown (GROUP BY → Postgres) works regardless of what's in the WHERE clause." | **NO — there is a hard ordering dependency.** All WHERE predicates must push down FIRST for the aggregate to push. If any WHERE predicate stays on Trino, the aggregate also stays on Trino. EXPLAIN smoking gun: a `Project` or `Filter` node sitting between the `Aggregation` and the `TableScan` means aggregation did NOT push — Trino is doing the GROUP BY in worker memory after pulling raw rows. | [§ 13.2 PUSHDOWN ORDERING DEPENDENCY](#132-pushdown-ordering-dependency--all-where-predicates-must-push-for-aggregate-to-push) |
