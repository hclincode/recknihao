...
> | **`ILIKE`** (case-insensitive) | **CONDITIONAL** — added by Trino PR #11045; pushes only when `enable_string_pushdown_with_collate=true` AND column collation is compatible. Defaults to NOT pushing. **Always verify with EXPLAIN.** For predictable behavior independent of flags/collation, denormalize on Postgres with a `lower()` generated column. |
>
...
> **ILIKE pushdown to the PostgreSQL connector was ADDED to Trino in [PR #11045](https://github.com/trinodb/trino/pull/11045) and is available in Trino 467, but it does NOT push unconditionally.** Whether a given `ILIKE` predicate actually reaches Postgres depends on **two** runtime conditions:
>
...
>
> **The correct operational stance**: do not assume `ILIKE` pushes, and do not assume it never pushes. **Run `EXPLAIN (TYPE DISTRIBUTED)` for your specific query and confirm directly** — see Section 3.4 for the EXPLAIN signature. A `ScanFilterProject` / `Filter` operator carrying the `ILIKE` predicate ABOVE the Postgres `TableScan` means the filter ran in Trino memory; an `ILIKE` clause appearing inside the TableScan's `constraint = [...]` block (and visible in Postgres `pg_stat_activity` as a `WHERE col ILIKE 'pattern'` clause) means it pushed.
>
> **If your ILIKE predicate is NOT pushing and you want to force it down**, in order of preference:
>
> 1. **Session-level test first** — no restart needed: `SET SESSION app_pg.enable_string_pushdown_with_collate = true;` then re-run your query with `EXPLAIN`. If the predicate now appears inside the TableScan, the flag fixed it.
...
-- - ILIKE pushdown (added by Trino PR #11045) is gated by this flag PLUS a compatible column collation; always verify with EXPLAIN.
SET SESSION billing_pg.enable_string_pushdown_with_collate = true;
```
