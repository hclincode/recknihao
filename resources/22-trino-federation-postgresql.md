...
| "VARCHAR equality and LIKE predicates always push down to PostgreSQL." | **CONDITIONAL.** Default OSS Trino 467 behavior pushes equality on VARCHAR columns that use the connection's default collation. For columns with non-default per-column COLLATE clauses, or ICU collations, or any case where Trino's bytewise comparison may disagree with Postgres's locale-aware comparison, pushdown is BLOCKED by default to preserve correctness. The opt-in flag is `postgresql.experimental.enable-string-pushdown-with-collate=true` (catalog) + `enable_string_pushdown_with_collate=true` (session). Equality-comparison performance can REGRESS when this flag is on. Measure both before committing. | § 3.3, § 13.5 failure shape #3 |
...
> **Both `LIKE` and `ILIKE` MAY push down on PostgreSQL, but the conditions differ.** Older blog posts state "simple LIKE pushes down by default" and "ILIKE never pushes" — **both are too strong**. The correct, defensive rule is that LIKE and ILIKE pushdown are **conditional on (a) Trino session/catalog config and (b) the Postgres column's collation** — and the only authoritative answer for your specific query is `EXPLAIN`.
>
