...
| Multi-dimensional arrays (e.g., `INTEGER[][]`) | Trino arrays are flat | `system.query()` to fetch with Postgres-side flattening (`unnest()`); no good catalog-property fix |
...
| `<catalog>.array_mapping` | `DISABLED` | How to map Postgres array columns: `DISABLED` (arrays silently omitted from results — no error), `AS_ARRAY` (map to Trino ARRAY type, e.g. `TEXT[]` → `ARRAY<VARCHAR>`), `AS_JSON` (map to JSON string). **Session property form**: `array_mapping` (underscores). **Catalog file property form**: `postgresql.array-mapping` (prefixed, hyphens). These are different names for the same setting in different contexts. | Enable `AS_ARRAY` when your Postgres tables have array columns you need to query with Trino array functions (`element_at()`, `cardinality()`, `contains()`) |
...
| `TEXT[]`, `INTEGER[]`, `BOOLEAN[]` (1-D arrays) | Trino `ARRAY<VARCHAR>`, `ARRAY<BIGINT>`, `ARRAY<BOOLEAN>` | **Silently omitted from results by default** (no error — columns just vanish from `SELECT *`) | Set `postgresql.array-mapping=AS_ARRAY` in catalog file |
...
| Multi-dimensional arrays (e.g. `INTEGER[][]`) | Not supported | Query fails or silently drops | No workaround — use `system.query()` to fetch |
