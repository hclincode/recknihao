...
- **THE THRESHOLD-PUSH REFERENCE CARD — Section 13.** Nine-question paste-ready answer template covering: (13.1) connector pushdown LIMITATION MATRIX — predicate / projection / aggregate / join / limit / topN / cross-join supported-vs-not per JDBC connector; (13.2) **PUSHDOWN ORDERING DEPENDENCY** — predicate must push FIRST for aggregate to push (the "all WHERE predicates must push for aggregate to push" rule) + literal EXPLAIN ANALYZE 50M-row smoking-gun snippet + `aggregation_pushdown_enabled` session-property worked example + `jdbc-types-mapped-to-varchar` foot-gun callout + collation equality-perf-regression caveat; (13.3) **dynamic filtering for federated joins** — build-side-to-probe-side direction + `join_distribution_type='BROADCAST'` + `enable-dynamic-filtering` + EXPLAIN ANALYZE `dynamicFilterSplitsProcessed` verification + INNER/RIGHT vs LEFT/FULL OUTER join-type support; (13.4) **federated-join cost & data-movement mental model** — why a 50M-row table without pushdown is a full network transfer + remediation tree; (13.5) **TopN / LIMIT pushdown — RECONCILED single source of truth** (TopN IS supported in OSS Trino 467 since release 353/354 in 2021; canonical pushed case + EXPLAIN-signature absence-of-TopN-operator + failure-shape catalog with the GROUP-BY-aggregate / multi-source-federated-join / non-default-collation / issue-#25138 shapes called out; myth-buster table making explicit that "OSS Trino can't push TopN" is wrong); **(13.5A) THE LEAST-EXPLORED FEDERATION ANGLES — bulletproof EXPLAIN signatures + verbatim doc citations**: (13.5A.1) **aggregation pushdown** — verbatim trino.io rule *"the explain plan does not show that Aggregate operator"* + supported function list (16 functions) + `aggregation_pushdown_enabled` worked example + `count(1)` vs `count(*)` PR #6667/issue #7251 caveat; (13.5A.2) **HAVING-clause pushdown** — when HAVING pushes (only when the aggregate also pushes), EXPLAIN signature (HAVING embedded inside the TableScan synthetic query), what blocks HAVING pushdown (unsupported aggregate, ROLLUP/CUBE, collation issues), PR #6667 sourcing note; (13.5A.3) **3-way cross-catalog JOIN execution location** — all joins execute on Trino workers (cross-catalog joins NEVER push), how to read `RemoteExchange[REPLICATE]` vs `RemoteExchange[REPARTITION]` in EXPLAIN, the BROADCAST/PARTITIONED-per-pairwise-join mental model, the "which side dominates wall-clock" operational rule; (13.5A.4) **OR-with-mixed-types pushdown** — when the OR pushes (every disjunct individually pushable, e.g., numeric `=` OR VARCHAR `=` default collation) vs when it doesn't (leading-wildcard LIKE / VARCHAR range / function-wrapped column as any disjunct = entire OR stays on Trino, full table over JDBC), 4-shape table + EXPLAIN signature + `UNION ALL + DISTINCT` workaround with `system.query()` passthrough; the AND-vs-OR pushability-composition asymmetry rule; (13.6) **WHEN TO MATERIALIZE a federated lookup locally** — Iceberg CTAS / refreshed snapshot instead of live federation + the 6-signal decision table; (13.7) the nine-question answer template; (13.8) verifiable trino.io URLs.
...
## Common myths about Trino federation — read FIRST (the load-bearing wrong claims)

These are the absolutes most often stated incorrectly about what OSS Trino 467's PostgreSQL connector "can't do." Each TRUTH below has been verified against the official Trino docs and release notes. Lead with the TRUTH; state the nuance explicitly.

| MYTH (commonly said wrong) | TRUTH (correct framing) | Where in this doc |
|---|---|---|
| "OSS Trino 467 can't push `ORDER BY ... LIMIT N` (TopN pushdown) to PostgreSQL — that's a Starburst Enterprise/commercial-fork feature." | **TopN pushdown is in OSS Trino since release 353 (March 2021) and enabled-by-default since release 354.** Session property `topn_pushdown_enabled = true` is the OSS Trino 467 default. EXPLAIN signature when pushed: `sortOrder=[col DESC NULLS LAST] limit=N` INSIDE the `TableScan`, with NO `TopN` operator above. The absence of the `TopN` operator IS the success signature. Specific plan shapes (ORDER BY on a computed aggregate, ORDER BY spanning multiple sources, non-default collation, non-identity projection between TopN and TableScan — [#25138](https://github.com/trinodb/trino/issues/25138)) prevent pushdown for THAT query, but the feature is available. | § 3.3A (intro), [§ 13.5](#135-topn--limit-pushdown--under-known-high-impact-reconciled--single-source-of-truth) (canonical) |
| "Aggregate pushdown (GROUP BY → Postgres) works regardless of what's in the WHERE clause." | **NO — there is an EXPLAIN-observable ordering dependency.** All WHERE predicates must push down FIRST for the aggregate to push. If any WHERE predicate stays on Trino (a `Filter` or `ScanFilterProject` sits between `Aggregate` and `TableScan`), the aggregate-pushdown rewrite rule does not match and the aggregate also stays on Trino. **This is an empirical plan-tree behavior — not an explicit doc-quoted rule — but it is consistently reproducible on Trino 467; the trino.io docs treat the two pushdowns as independent and document only the success signature ("If an aggregate function is successfully pushed down to the connector, the explain plan does not show that Aggregate operator"). EXPLAIN smoking gun: a `Project`, `Filter`, or `ScanFilterProject` node sitting between the `Aggregate` and the `TableScan` means aggregation did NOT push — Trino is doing the GROUP BY in worker memory after pulling raw rows.** | [§ 13.2 PUSHDOWN ORDERING DEPENDENCY](#132-pushdown-ordering-dependency--all-where-predicates-must-push-for-aggregate-to-push) |
| "Connection pooling in OSS Trino's PostgreSQL connector is configured via `connection-pool.enabled` / `connection-pool.max-size`." | **NO — those properties belong to Starburst Enterprise, NOT OSS Trino 467.** OSS Trino 467 has NO native PostgreSQL connection pooling ([#15888](https://github.com/trinodb/trino/issues/15888) open since Jan 2023). Pasting `connection-pool.*` into a catalog properties file is silently ignored. The correct OSS mitigation for connection pressure is PgBouncer in front of Postgres, Postgres role-level `CONNECTION LIMIT`, Trino resource groups, and `statement_timeout` on the replica. | § 0 (this section), § 8.2 |
| "Trino can push a JOIN between an Iceberg table and a PostgreSQL table down to one of the engines (join pushdown)." | **NO — cross-catalog join pushdown does NOT exist.** The join itself always executes on Trino workers. Only each side's OWN predicates push down to its own engine. The mechanism that makes "small dim × huge fact" cross-catalog joins efficient is **dynamic filtering** (build-side keys become a runtime IN-list/min-max predicate sent to the probe side's scan), NOT join pushdown. | § 1 (mental model), § 5 (dynamic filtering) |
| "If TopN doesn't push for one query, the whole TopN-pushdown feature is broken / unavailable on this connector version." | **NO — TopN pushdown is per-query plan-shape-dependent.** A specific plan shape can block the push (see [§ 13.5 failure shapes](#failure-shapes--when-topn-does-not-push-and-why)), but other queries on the same connector still push. Each failure shape has its own workaround (the most common: `system.query()` passthrough). Do NOT generalize one failed-pushdown query into "OSS Trino can't push TopN." | [§ 13.5](#135-topn--limit-pushdown--under-known-high-impact-reconciled--single-source-of-truth) |
| "VARCHAR equality and LIKE predicates always push down to PostgreSQL." | **CONDITIONAL — and the four VARCHAR predicate shapes have FOUR DIFFERENT answers (do not conflate them):** (1) **VARCHAR equality** (`col = 'x'`, `col != 'x'`, `IN`) — **PUSHES by default** (collation-sensitive: default Postgres collation pushes cleanly; non-default per-column COLLATE / ICU collations are blocked unless you enable the experimental `postgresql.experimental.enable-string-pushdown-with-collate=true` flag — equality performance can REGRESS when this flag is on). (2) **Anchored / prefix LIKE** (`col LIKE 'foo%'`) — **CAN push** (collation-sensitive; maps to a range scan on the prefix; verify with EXPLAIN). (3) **Leading-wildcard LIKE** (`col LIKE '%foo'`, `col LIKE '%foo%'`) — **does NOT push usefully regardless of collation, regardless of the experimental flag** (no anchored prefix means no B-tree index can satisfy it; Trino strips it from the JDBC SQL or Postgres falls back to a sequential scan; Trino re-filters in memory after pulling rows). (4) **VARCHAR range** (`col < 'x'`, `col > 'x'`, `BETWEEN 'a' AND 'z'`) — **does NOT push by default**; opt-in via `postgresql.experimental.enable-string-pushdown-with-collate=true` (correctness-risk flag — emits `COLLATE "C"`). **The experimental flag gates VARCHAR RANGE pushdown and ILIKE pushdown — it does NOT make leading-wildcard LIKE useful** (leading-wildcard LIKE is a separate index-shape problem, fix with `pg_trgm` GIN index or full-text search). | § 3.3 (LIKE-vs-ILIKE), § 3.3 PUSHDOWN TABLE (4-row VARCHAR/LIKE matrix), § 13.5 failure shape #3 |

> **Why these specific myths matter.** Each is a load-bearing topic-specific claim about what OSS Trino 467 "can't do." Stated as an absolute, it causes engineers to design around limitations that don't exist (manually pulling 50M rows when TopN would have pushed) or to design over limitations that DO exist (assuming aggregate pushdown will fire when a WHERE predicate is blocking it). Always check the EXPLAIN signature before claiming a feature isn't available.
...
- Saturate connection slots — OSS Trino 467 has no JDBC connection pool, so every running query opens fresh Postgres connection(s) for the duration of that query. **JDBC connection model — the precise rule**: **each Postgres `TableScan` operator in the query plan opens exactly one JDBC connection**, because the PostgreSQL connector creates **one split per non-partitioned table scan** and one split = one JDBC connection. The implications:
  - A query that scans **one** Postgres table → **1 JDBC connection** (one TableScan, one split, one connection).
  - A query that **joins two different Postgres tables** (e.g., `app_pg.public.users JOIN app_pg.public.orders`) → **2 JDBC connections** — one per TableScan operator, even though they belong to the same Trino query and the same catalog. Each TableScan independently opens its own connection to run its own `SELECT ... FROM <table>` against Postgres.
  - A query that joins three Postgres tables → 3 connections, and so on. **The connection count tracks the number of Postgres TableScans, not the number of queries.**
  - A query that joins a Postgres table to an Iceberg table → **1 Postgres JDBC connection** (Iceberg uses S3/MinIO reads, not JDBC, so it does not consume Postgres connections).
  - Per-table self-joins also open one connection per TableScan — e.g., `users u1 JOIN users u2 ON ...` opens **2** connections to Postgres, not 1.

  **Capacity planning formula**: `peak_postgres_connections ≈ max_concurrent_federation_queries × avg_postgres_TableScans_per_query`. This matters directly when sizing PgBouncer `default_pool_size` and Trino resource group `hardConcurrencyLimit` — if your typical query joins 3 Postgres tables and you allow 20 concurrent queries via resource groups, you need `default_pool_size ≥ 60` in PgBouncer (plus a margin for short-lived metadata-fetch connections from coordinator planning). For typical single-table federation with a PgBouncer ceiling of 50, you can support roughly 50 concurrent federation queries (each reading one Postgres table) before saturating the pool. **Worker count does NOT multiply this number**: one split = one connection, regardless of how many Trino workers are available — the connection is opened by the single worker assigned the split, not by every worker in the cluster. Worker count would multiply connections only if (a) Postgres-side table partitioning (the `partition-column` property — **Starburst Enterprise only, NOT available in OSS Trino 467**; see Section 4.4) is configured so Trino opens N parallel range-scan connections per scan, or (b) a custom split strategy is used. The mitigation when you DO push past the ceiling is **PgBouncer + Postgres role-level CONNECTION LIMIT + Trino resource groups** (see Section 8.2), NOT a Trino-side pool property — that property does not exist in OSS Trino 467.
...
> 2. **String-range pushdown caveat — applies to the JDBC side, NOT to Iceberg.** On the **PostgreSQL connector**: equality (`=`), `IN`-lists, `IS NULL`, `IS NOT NULL` on `CHAR`/`VARCHAR` columns push down by default in Trino 467. **Anchored LIKE patterns (`LIKE 'foo%'`) may push down on PostgreSQL for standard-collation columns — but behavior is collation-dependent and more conservative than equality pushdown** (non-default collations, ICU collations, or `COLLATE` clauses can suppress it). **Always verify with EXPLAIN before relying on LIKE pushdown** — it is not the unconditional push that equality is. On the **MySQL connector**: VARCHAR equality and ALL LIKE patterns do NOT push down (Section 2A.2). What is **conditional / does not push by default** on the **PostgreSQL connector**: **`ILIKE`** (LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation — **only pushes when both the `enable_string_pushdown_with_collate` session/catalog property is enabled AND the column collation is compatible** — verify with EXPLAIN) AND **byte-range predicates** (`>`, `<`, `BETWEEN` on VARCHAR/CHAR) — those also require the experimental `postgresql.experimental.enable-string-pushdown-with-collate=true` flag (PostgreSQL only; the flag does NOT exist for MySQL; see 3.3). On the **MySQL connector**, ILIKE does NOT push at all (the connector has no ILIKE pushdown rule, and the string-collation flag does not exist). On the **Iceberg connector**: string range predicates in WHERE work fine for partition pruning if the partition column is a string. The Iceberg-specific caveat is different — use **TIMESTAMP/DATE literals that align with the partition transform** (`day`, `hour`, `month`) so partition pruning fires, not `LIKE` or string-based date matching against a timestamp column. See section 3.7 below for the Iceberg version. If your join key or filter is a VARCHAR range on the JDBC side, plan for this.
...
| **Anchored / prefix `LIKE`** (`col LIKE 'foo%'`) | **CAN push** — pushes down on PostgreSQL for **standard-collation columns**; behavior is **collation-dependent** (anchored prefix maps to a range scan on the prefix; non-default ICU collations or explicit `COLLATE` clauses block it). **Always verify with EXPLAIN.** | `WHERE email LIKE 'alice@%'` — see 3.3 |
| **Leading-wildcard `LIKE`** (`col LIKE '%foo'` or `col LIKE '%foo%'`) | **NO — does NOT push usefully regardless of collation, regardless of the experimental flag.** Without an anchored prefix, no B-tree index can satisfy it. Trino either strips the LIKE before issuing the JDBC query OR Postgres receives it and falls back to a sequential scan; either way, the filter ultimately runs in Trino memory after pulling candidate rows. **The `enable-string-pushdown-with-collate` flag does NOT help — it gates VARCHAR range and ILIKE, NOT leading-wildcard LIKE.** Fix at the schema level: `pg_trgm` GIN index, full-text search (`tsvector` + GIN), or denormalize the substring you actually filter on into its own indexed column. | `WHERE email LIKE '%@bigcorp.com'`, `WHERE name LIKE '%foo%'` — see 3.3 leading-wildcard subsection |
| **`ILIKE`** (case-insensitive LIKE) | **CONDITIONAL** — requires session/catalog `enable_string_pushdown_with_collate=true` AND a compatible column collation; verify with EXPLAIN | `WHERE email ILIKE 'A%'` — see ILIKE subsection in 3.3 |
| **String RANGE** predicates (`>`, `<`, `<=`, `>=`, `BETWEEN` on VARCHAR/text/char) | **NO by default** (collation-correctness concern); opt-in via experimental `postgresql.experimental.enable-string-pushdown-with-collate=true` — gates RANGE predicates (and ILIKE), NOT leading-wildcard LIKE | `WHERE name BETWEEN 'a' AND 'm'` — see 3.3 |
...
>
> - `=`, `IN`, `!=` on VARCHAR/CHAR columns -> **PUSH DOWN to PostgreSQL** by default (collation-sensitive: default collation pushes cleanly; non-default ICU / explicit COLLATE blocks unless you enable the experimental flag — enabling that flag can REGRESS equality performance by disabling B-tree index usage)
> - `<`, `>`, `<=`, `>=`, `BETWEEN` on VARCHAR/CHAR columns -> **do NOT push by default**
>   (opt-in with `postgresql.experimental.enable-string-pushdown-with-collate=true` for C/POSIX locale DBs only — see Section 3.3 for the correctness caveat)
...
> - **Anchored / prefix LIKE** (`col LIKE 'foo%'`, e.g., `email LIKE 'alice@%'`) on VARCHAR -> **CAN push — collation-sensitive** (anchored prefix maps to a range scan; on standard collations it often pushes, on non-default ICU collations it doesn't — verify with EXPLAIN)
> - **Leading-wildcard LIKE** (`col LIKE '%foo'` or `col LIKE '%foo%'`, e.g., `email LIKE '%@bigcorp.com'`) on VARCHAR -> **does NOT push usefully regardless of collation, regardless of the experimental flag.** No anchored prefix = no B-tree index can satisfy it. Trino either strips the LIKE or Postgres scans sequentially; the filter runs in Trino memory either way. **The `enable-string-pushdown-with-collate` flag does NOT make this case useful — that flag gates RANGE predicates and ILIKE, NOT leading-wildcard LIKE.** Fix at the schema level: `pg_trgm` GIN index, full-text search (`tsvector` + GIN), or denormalize the substring you actually filter on into its own indexed column.
> - `ILIKE` on VARCHAR -> **CONDITIONAL** — LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation. Pushes only when `enable_string_pushdown_with_collate=true` AND the column collation is compatible. **Default = does NOT push** (flag off). Verify with EXPLAIN for your specific query. For predictable, collation-independent case-insensitive matching, prefer denormalizing with a `lower()` generated column (always pushes, never depends on flags).
>
...
> |---|---|
> | **Equality** (`col = 'x'`, `col != 'x'`) | **YES — pushes by default** (collation-sensitive: default Postgres collation pushes cleanly; non-default per-column COLLATE / ICU collations are blocked unless you enable the experimental `enable-string-pushdown-with-collate` flag — and enabling that flag can REGRESS equality performance by disabling Postgres B-tree index usage on the column) |
> | **Membership** (`col IN ('a', 'b', ...)`) | **YES — pushes by default** (same collation rules as equality) |
> | **Dynamic filter IN-list** (runtime, from cross-catalog join build side) | **YES — pushes by default** (ships as `WHERE col IN (...)` in the JDBC query at probe time) |
> | **`IS NULL` / `IS NOT NULL`** | **YES — pushes by default** (text columns push the same as numeric/date — collation-independent) |
> | **Anchored / prefix `LIKE`** (`col LIKE 'foo%'`) | **CAN push — collation-sensitive** (anchored prefix maps to a range scan on the prefix; on standard-collation columns it often pushes, but on non-default ICU collations or with an explicit `COLLATE` clause Trino refuses the push). **Verify with EXPLAIN.** |
> | **Leading-wildcard `LIKE`** (`col LIKE '%foo'` or `col LIKE '%foo%'`) | **NO — does NOT push usefully regardless of collation, regardless of the experimental flag.** Without an anchored prefix, no B-tree index can satisfy the predicate. Trino either strips the LIKE before issuing the JDBC query OR Postgres receives it and falls back to a sequential scan; either way, the filter ultimately runs in Trino worker memory after pulling candidate rows. **The `enable-string-pushdown-with-collate` flag does NOT help** — that flag gates RANGE predicates and ILIKE, NOT leading-wildcard LIKE. Fix at the schema level: `pg_trgm` GIN index for substring search, or `tsvector` + GIN for full-text search, or denormalize the substring you actually filter on into its own indexed column. |
> | **Range** (`col < 'x'`, `col > 'x'`, `<=`, `>=`, `BETWEEN`) | **NO — does NOT push by default** (collation-correctness concern — Trino's bytewise sort may disagree with Postgres's locale-aware collation). **Opt-in via experimental `postgresql.experimental.enable-string-pushdown-with-collate=true`** (catalog) / `enable_string_pushdown_with_collate=true` (session). Correctness-risk flag — emits `COLLATE "C"` on pushed predicates which is only correct for C/POSIX-locale DBs; can also disable Postgres index usage. Test on a non-prod replica first. |
> | **`ILIKE`** (case-insensitive `col ILIKE 'A%'`) | **CONDITIONAL** — LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation. Pushes only when `enable_string_pushdown_with_collate=true` AND column collation is compatible. Defaults to NOT pushing. **Always verify with EXPLAIN.** For predictable behavior independent of flags/collation, denormalize on Postgres with a `lower()` generated column. |
>
...
|---|---|---|---|
| **ILIKE** (case-insensitive LIKE) | `WHERE email ILIKE 'A%'` | **Conditional** — LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation. ILIKE is available in Trino 467, but it pushes ONLY when (a) the session/catalog property `enable_string_pushdown_with_collate` is enabled AND (b) the column's Postgres collation is compatible (standard C / en_US.UTF-8 collations typically allow it; custom or ICU collations may not). With the flag at its default `false`, ILIKE is not pushed. **Always verify with EXPLAIN for your specific query.** (Anchored `LIKE` prefix patterns are a separate, also-collation-dependent path — see the LIKE-vs-ILIKE clarification below.) | **Production-stable fix (no flags, no collation dependency)**: denormalize on the Postgres side — add a `lower_email` generated column indexed on `lower(email)`, then push down equality on it from Trino. **Short-term fix when you cannot change the schema**: enable the session property `SET SESSION app_pg.enable_string_pushdown_with_collate = true;` and re-run EXPLAIN to confirm the ILIKE now pushes (the `ScanFilterProject`/`Filter` node above the `TableScan` should disappear). **Read the correctness-risk caveat** in the LIKE-vs-ILIKE subsection below before promoting the flag cluster-wide. |
...
The three rows above use overlapping workarounds — read carefully which one applies to your case. **`enable_string_pushdown_with_collate=true` gates BOTH string-range pushdown AND ILIKE pushdown** (ILIKE pushdown behavior on PostgreSQL is conditional on the same flag plus column collation compatibility), so enabling it can help either category — but for ILIKE the flag is necessary, not sufficient (the column collation must also be compatible). It does NOT help function calls (`LOWER(col)`, `CAST(col AS ...)`) — those need predicate rewriting onto a stored/generated column. Denormalizing a column does NOT help unbounded byte-range string predicates. Use `EXPLAIN` to confirm which workaround actually fixed pushdown for your specific query.

#### CANONICAL — the four VARCHAR / LIKE predicate shapes on the PG connector (4 separate rows, do NOT conflate)

> **TOP-OF-SECTION GUARDRAIL — read this BEFORE answering any question that mentions VARCHAR pushdown or any form of LIKE on the PostgreSQL connector.**
>
> The most common iter421-class failure mode on this resource is collapsing **anchored prefix LIKE** (`col LIKE 'foo%'`), **leading-wildcard LIKE** (`col LIKE '%foo'` or `col LIKE '%foo%'`), **VARCHAR equality** (`col = 'x'`), and **VARCHAR range** (`col < 'x'`, BETWEEN) into a single answer. They have **FOUR DIFFERENT answers**. Keep them visually distinct.
>
> | VARCHAR shape | Example | Pushes by default? | Why / what the `enable-string-pushdown-with-collate` flag does for this case |
> |---|---|---|---|
> | **1. VARCHAR equality** (`=`, `!=`, `IN`) | `WHERE status = 'active'`, `WHERE email = 'alice@example.com'` | **YES — pushes by default.** Collation-sensitive: default Postgres collation pushes cleanly; non-default per-column COLLATE / ICU collations are BLOCKED unless you enable the experimental flag. | **Equality already pushes by default**, so you do NOT normally need the flag for equality. Enabling the flag to fix something else (range, ILIKE) can REGRESS equality performance by emitting `COLLATE "C"` on equality predicates — that can disable existing Postgres B-tree indexes built under the default collation. Trade-off, not a free win. |
> | **2. Anchored / prefix LIKE** (`LIKE 'foo%'`) | `WHERE email LIKE 'alice@%'`, `WHERE name LIKE 'Acme%'` | **CAN push — collation-sensitive.** Maps to a range scan on the prefix. On standard-collation columns it usually pushes; on non-default ICU collations or with an explicit `COLLATE` clause it doesn't. **Verify with EXPLAIN.** | If anchored LIKE refuses to push on a non-default-collation column, the flag may enable it (same correctness/index caveat as range). For standard-collation columns the flag is typically not needed. |
> | **3. Leading-wildcard LIKE** (`LIKE '%foo'`, `LIKE '%foo%'`) | `WHERE email LIKE '%@bigcorp.com'`, `WHERE name LIKE '%foo%'` | **NO — does NOT push usefully regardless of collation, regardless of the experimental flag.** Without an anchored prefix, no B-tree index can satisfy the predicate. Trino either strips the LIKE before issuing the JDBC SQL OR Postgres receives it and falls back to a sequential scan; either way, the filter ultimately runs in Trino worker memory after pulling candidate rows. | **The `enable-string-pushdown-with-collate` flag does NOT help here.** The flag is about RANGE predicates (and ILIKE) where the issue is bytewise-vs-locale-aware *comparison correctness*. Leading-wildcard LIKE has a different problem entirely: an *index-shape* problem (no prefix = no B-tree). **Fix at the schema level**: `pg_trgm` extension + GIN index for substring search, OR `tsvector` + GIN for full-text search, OR denormalize the substring you actually filter on into its own indexed column. **Common mistake**: assuming "if I enable the flag, leading-wildcard LIKE will push." It won't. |
> | **4. VARCHAR range** (`<`, `>`, `<=`, `>=`, `BETWEEN`) | `WHERE name BETWEEN 'a' AND 'm'`, `WHERE status > 'paid'` | **NO — does NOT push by default.** Doc-quoted from trino.io/docs/current/connector/postgresql.html: *"The connector does not support pushdown of range predicates, such as `>`, `<`, or `BETWEEN`, on columns with character string types."* | **Opt-in via** `postgresql.experimental.enable-string-pushdown-with-collate=true` (catalog) / session `enable_string_pushdown_with_collate=true`. **This is what the flag was actually designed for.** Correctness-risk: the flag emits `COLLATE "C"` on pushed predicates, which is only correct for C/POSIX-locale DBs (byte-wise sort = locale sort); on ICU collations it can silently return different rows than Trino's in-memory evaluation would. Can also disable Postgres index usage on the column. Test on a non-prod replica with `EXPLAIN ANALYZE` on the Postgres side before promoting cluster-wide. |
>
> **One-line summary of the flag**: `postgresql.experimental.enable-string-pushdown-with-collate` is about **VARCHAR RANGE predicates and ILIKE** (collation-correctness gate); it is **NOT** about leading-wildcard LIKE (which is an index-shape problem fixed by `pg_trgm` GIN). For VARCHAR equality the flag is normally unnecessary and can hurt performance.
>
> **Mental model — when you see a VARCHAR/LIKE predicate, FIRST ask: which of the FOUR shapes is it?**
>
> 1. **Equality / IN / IS NULL on VARCHAR?** → Pushes by default. Done. (No flag needed except for non-default-collation edge cases.)
> 2. **Anchored prefix LIKE (`LIKE 'foo%'`)?** → CAN push, verify with EXPLAIN. Collation-sensitive.
> 3. **Leading-wildcard LIKE (`LIKE '%foo'` or `LIKE '%foo%'`)?** → DOES NOT push usefully. Flag does NOT help. Schema-level fix only (`pg_trgm` GIN).
> 4. **Range (`<`, `>`, `BETWEEN`) on VARCHAR?** → Does NOT push by default. Opt-in via experimental flag (correctness-risk).
>
> Then describe the EXPLAIN signature for that specific shape (Section 3.4). Do not collapse the four cases into a single "VARCHAR LIKE pushdown" answer.
>
> **DO-NOT-WRITE table — sentences that would produce the iter421 leading-wildcard LIKE imprecision (factually wrong on Trino 467 / PG connector):**
>
> | DO NOT WRITE | THE FACTUAL FIX |
> |---|---|
> | "`email LIKE '%@bigcorp.com'` pushes if the column has default collation." | Leading-wildcard LIKE does NOT push usefully regardless of collation. The default-collation rule applies to **anchored** LIKE (`LIKE 'alice@%'`), not to leading-wildcard. |
> | "Enabling `enable-string-pushdown-with-collate` will make `LIKE '%foo'` push to Postgres." | False. The flag gates VARCHAR RANGE and ILIKE — both are collation-correctness issues. Leading-wildcard LIKE has an index-shape problem (no anchored prefix = no B-tree usable); no Trino-side flag fixes it. Schema-level fix only (`pg_trgm` GIN). |
> | "All LIKE predicates push the same way on the PG connector." | False — at least 3 different shapes: anchored (`LIKE 'foo%'`) CAN push, exact-match (`LIKE 'foo'`) degenerates into equality and pushes, leading-wildcard (`LIKE '%foo'`/`LIKE '%foo%'`) does NOT push usefully. |
> | "VARCHAR range and leading-wildcard LIKE both need the experimental flag to push." | Only VARCHAR range needs the flag (correctness gate). Leading-wildcard LIKE cannot push usefully even WITH the flag — it's a different problem (index shape, not comparison correctness). |
> | "Anchored prefix LIKE never pushes on Trino 467; LIKE pushdown is a Starburst feature." | False. LIKE pushdown landed in OSS Trino release 365 (Dec 2021) and ships in OSS Trino 467 for anchored-prefix patterns; collation-sensitive but not Starburst-only. |
>
> **Verified against**: trino.io/docs/current/connector/postgresql.html (range-predicate pushdown quote + experimental flag doc) and trino.io/docs/current/optimizer/pushdown.html.

#### LIKE vs ILIKE in Trino 467 — the distinction matters

> **Anchored `LIKE` and `ILIKE` MAY push down on PostgreSQL, but the conditions differ. Leading-wildcard `LIKE` does NOT push usefully regardless of conditions** (see the leading-wildcard bullet below). Older blog posts state "simple LIKE pushes down by default" and "ILIKE never pushes" — **both are too strong**. The correct, defensive rule for ANCHORED LIKE/ILIKE is that pushdown is **conditional on (a) Trino session/catalog config and (b) the Postgres column's collation** — and the only authoritative answer for your specific query is `EXPLAIN`. The leading-wildcard case is separate and not gated by either condition.
>
...
> - **`ILIKE` pushdown**: LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation. ILIKE is available in Trino 467. **Whether a specific ILIKE actually pushes depends on (1) the session/catalog property `enable_string_pushdown_with_collate` being enabled and (2) the Postgres column's collation being compatible (standard C/en_US.UTF-8 collations typically allow it; custom collations may not).** With the flag at its default `false`, ILIKE will NOT push and Trino evaluates the filter in-memory on its workers. With the flag on, behavior depends on the column collation — **always verify with `EXPLAIN` rather than assuming either way**. See the dedicated ILIKE subsection below for the full force-pushdown procedure and the correctness caveat.
...
>
> **Always verify with `EXPLAIN (TYPE DISTRIBUTED)` for your exact query** — see Section 3.4. Predicate-shape edge cases (escape characters, multi-byte characters, custom collations, ICU vs. libc collations) routinely affect whether a specific `LIKE` or `ILIKE` is rewritten cleanly. The defensive rule **"anchored LIKE MAY push down on PostgreSQL for standard-collation columns; ILIKE MAY push down when `enable_string_pushdown_with_collate` is enabled AND the column collation is compatible — but with the flag at its default `false`, ILIKE does NOT push"** is the right starting point; the EXPLAIN plan is the authoritative answer for any given query.
>
...
#### ILIKE pushdown — conditional, depends on session config and column collation

> **LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation. ILIKE is available in Trino 467, but it does NOT push unconditionally.** Whether a given `ILIKE` predicate actually reaches Postgres depends on **two** runtime conditions:
>
> 1. **The string-pushdown session/catalog property must be enabled**: `SET SESSION app_pg.enable_string_pushdown_with_collate = true;` (per-query) **or** `postgresql.experimental.enable-string-pushdown-with-collate=true` in the catalog `.properties` file (cluster-wide, requires coordinator restart). With the flag at its default `false`, `ILIKE` is **NOT pushed** — Trino pulls candidate rows over JDBC and evaluates the predicate in-memory on its workers.
> 2. **The Postgres column's collation must be compatible**. Columns using the standard `C` / `POSIX` / `en_US.UTF-8` collations typically allow the pushdown to fire. Columns using **custom ICU collations**, an explicit `COLLATE` clause in the query, or non-default locale-aware collations may suppress it even with the flag on.
>
> **The correct operational stance**: do not assume `ILIKE` pushes, and do not assume it never pushes. **Run `EXPLAIN (TYPE DISTRIBUTED)` for your specific query and confirm directly** — see Section 3.4 for the EXPLAIN signature. A `ScanFilterProject` / `Filter` operator carrying the `ILIKE` predicate ABOVE the Postgres `TableScan` means the filter ran in Trino memory; when pushdown succeeds the `ScanFilterProject` / `Filter` node disappears from the plan and the predicate is absorbed into the `TableScan`. **The exact textual format of pushed predicates in the TableScan output varies by Trino version and connector** — the official Trino docs only commit to "the `ScanFilterProject` disappears" as the success signal, not to any specific format of the `constraint = [...]` block inside the TableScan. What matters is that the `ScanFilterProject` node disappears from the plan; the ground truth that the predicate actually reached Postgres is visible in Postgres `pg_stat_activity` as a `WHERE col ILIKE 'pattern'` clause in the running JDBC query.
>
> **If your ILIKE predicate is NOT pushing and you want to force it down**, in order of preference:
>
> 1. **Session-level test first** — no restart needed: `SET SESSION app_pg.enable_string_pushdown_with_collate = true;` then re-run your query with `EXPLAIN`. If the `ScanFilterProject` / `Filter` node above the `TableScan` disappears, the flag fixed it. (The exact textual format of pushed predicates inside the TableScan output varies by Trino version and connector; what matters is that the `ScanFilterProject` node disappears from the plan.)
> 2. **Cluster-wide promotion** — once verified safe for your specific Postgres locale, add `postgresql.experimental.enable-string-pushdown-with-collate=true` to `etc/catalog/app_pg.properties` and restart the coordinator. **Warning**: this is a **correctness-risk flag** — see the detailed caveat in the LIKE-vs-ILIKE subsection above. The flag emits a `COLLATE "C"` clause on pushed string predicates, which is only correct for byte-wise locales. On non-default collation columns it can **return different rows than Trino's in-memory evaluation would** and can **disable Postgres index usage** built under the default collation.
> 3. **Bypass the connector entirely with `system.query()`** if you need server-side ILIKE on a column that refuses to push for collation reasons: `SELECT * FROM TABLE(app_pg.system.query(query => 'SELECT * FROM users WHERE email ILIKE ''%@example.com'''))`. The query string is sent verbatim to Postgres, so Postgres evaluates the ILIKE with its native semantics.

For predictable, collation-independent case-insensitive matching that ALWAYS pushes regardless of session flags and never depends on which Postgres collation the column uses, **the safer alternatives, in preference order**, are:
...
|---|---|
| PostgreSQL (and MySQL, SQL Server) | Equality, IN-lists, and **simple `LIKE 'prefix%'` patterns push down** (LIKE pushdown was added in Trino 365 and is supported in Trino 467). **`ILIKE` is CONDITIONAL on PostgreSQL** — LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; behavior depends on the `enable_string_pushdown_with_collate` session property and the column's collation. ILIKE pushes only when the `enable_string_pushdown_with_collate` session/catalog property is enabled AND the column collation is compatible; **always verify with EXPLAIN** rather than assuming either way. On MySQL, ILIKE does not push at all. String **range** predicates (`>`, `<`, `BETWEEN` on VARCHAR/CHAR) also do NOT push down by default on PostgreSQL; enable them only with the experimental `postgresql.experimental.enable-string-pushdown-with-collate=true` flag (same flag that gates ILIKE). |
...
> **Equivalent restatement — "JDBC source stages are single-task by default."** In Trino plan / Web UI terminology, the JDBC source stage for a non-partitioned Postgres (or MySQL) table runs as **exactly one task** on one worker. The PostgreSQL connector does **not implement the SPI methods that would let Trino split a single table scan into multiple tasks** — splits-per-table is hardcoded to 1 for non-partitioned tables. This is by design in the base JDBC connector framework; there is no per-catalog property to change it in OSS Trino 467.
>
> **Properties that DO NOT EXIST in OSS Trino 467 PostgreSQL connector** (engineers frequently search for these and copy them from Stack Overflow answers that refer to either Starburst Enterprise or to Spark JDBC):
>
> - `postgresql.max-splits-per-second` — **does not exist**. Pasting it into `etc/catalog/app_pg.properties` causes catalog startup to fail with "Configuration property 'postgresql.max-splits-per-second' was not used." Setting it via session likewise errors: "Session property does not exist."
> - `postgresql.max-splits-per-scan` — **does not exist**. Same failure mode.
> - `postgresql.parallel-reads` / `postgresql.parallel-splits` — **do not exist**. There is no OSS property to enable parallel JDBC reads of a single table.
> - `partition-column` / `partition-count` — **Starburst Enterprise only**, not available in OSS Trino 467 (already noted above and in Section 4.4's main body).
>
> **The Iceberg contrast — why federated joins serialize on the Postgres side:** when a query joins `iceberg.fact` (parallel multi-task source stage, one task per file split, fanning out across all 20 workers) to `app_pg.dim` (single-task source stage, one task on one worker pulling all rows over one JDBC connection), the Iceberg side finishes its parallel scan in seconds while the Postgres side is still streaming rows over its single connection. The whole federated query is bounded by the JDBC throughput on that one worker thread. This is why **a large unfiltered Postgres scan inside a federated join is almost always the bottleneck** even on a 20-worker cluster.
>
> **What you can actually do about it** (already covered in the "Parallelism for Postgres Table Scans" subsection below, but restated here so engineers find it from the JDBC single-task angle): (1) push down a selective WHERE clause so Postgres returns fewer rows over the single connection (Section 3 on predicate pushdown); (2) let dynamic filtering trim the build side (Section 5); (3) materialize the Postgres table into Iceberg if the same scan runs repeatedly (Section 6 on federate-vs-ingest). The single-task ceiling is not configurable away — it is a property of the JDBC connector framework.
...
| `<catalog>.enable_string_pushdown_with_collate` | `false` | Enables string-predicate pushdown (experimental — collation risk). **Gates two predicate families**: (1) **range predicates** (`>`, `<`, `BETWEEN`) on VARCHAR/CHAR columns, and (2) **`ILIKE` predicates** (LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; ILIKE behavior depends on this flag plus column collation compatibility). Does NOT affect simple anchored `LIKE` (already pushes for prefix patterns in Trino 467 without the flag, though collation-dependent in its own way). | Enable to test whether string range predicates OR ILIKE predicates reach Postgres; verify with EXPLAIN first — ILIKE pushdown also requires the column to use a compatible collation (standard C / en_US.UTF-8 typically works; custom ICU collations may suppress it) |
...
-- enable string RANGE pushdown AND ILIKE pushdown (experimental — collation-risk flag):
-- - Simple anchored LIKE prefix patterns push down WITHOUT this flag in Trino 467 (but are themselves collation-dependent).
-- - String RANGE predicates (>, <, BETWEEN on VARCHAR/CHAR) push only with this flag.
-- - ILIKE pushdown: LIKE and ILIKE pushdown support was added to the PostgreSQL connector in Trino; ILIKE behavior depends on this flag PLUS a compatible column collation; always verify with EXPLAIN.
SET SESSION billing_pg.enable_string_pushdown_with_collate = true;
```
...
**There is NO per-session form for the Iceberg connector — you cannot `SET SESSION iceberg.dynamic_filtering_wait_timeout = '20s'`.** That statement fails with `Session property 'iceberg.dynamic_filtering_wait_timeout' does not exist`, because the Iceberg connector simply does not register `dynamic_filtering_wait_timeout` as a session property (verified against trino.io/docs/current/connector/iceberg.html for Trino 481). The only way to change Iceberg's wait-timeout is to edit `etc/catalog/iceberg.properties` (shown above) and **restart the coordinator**. By contrast, the Hive, Delta, MySQL, PostgreSQL, and SQL Server connectors all DO expose `<catalog>.dynamic_filtering_wait_timeout` as a session property — so for those probe types you can tune per-query without a restart (`SET SESSION billing_mysql.dynamic_filtering_wait_timeout = '20s'`).
...
> ```
>
> **There is NO per-session form for the Iceberg connector — `SET SESSION <iceberg_catalog>.dynamic_filtering_wait_timeout = '20s'` does NOT work** (returns `Session property '...' does not exist`, regardless of whether you use the catalog name or the connector name as the prefix). The Iceberg connector does not register `dynamic_filtering_wait_timeout` as a session property at all — this is a verified asymmetry against trino.io/docs/current/connector/iceberg.html (Trino 481). Your **only** option to raise the Iceberg probe's wait-timeout is to edit `etc/catalog/iceberg.properties` as shown above and **restart the coordinator**. If you need a per-query lever, the workaround is to engineer the join so the probe is the JDBC side instead (then `SET SESSION billing_mysql.dynamic_filtering_wait_timeout = '20s'` works), but that's almost always a worse plan. For Hive and Delta probes, per-session DOES work (`SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '20s'`).
>
...
>
> **Catalog name vs connector name for `SET SESSION` (for the connectors that DO support a session form).** For Hive / Delta / MySQL / PostgreSQL / SQL Server, the prefix in `SET SESSION` is your **catalog name** (the filename in `etc/catalog/`, without `.properties`), **not** the connector type. If your Hive catalog file is `etc/catalog/hive_warehouse.properties`, use:
> ```sql
> SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '20s';
> ```
> Not `hive.dynamic_filtering_wait_timeout` unless the file is literally named `hive.properties`. **The Iceberg connector does not have a session form at all, so this prefix discussion is moot for Iceberg — neither `SET SESSION iceberg.xxx` nor `SET SESSION iceberg_catalog.xxx` will work; both return `does not exist`.** Verify supported session properties with `SHOW SESSION` (look for entries prefixed with your catalog name).
...
> | **How to raise it (per-query)** | **NOT POSSIBLE for the Iceberg connector.** The Iceberg connector does not expose `dynamic_filtering_wait_timeout` as a session property — `SET SESSION iceberg.dynamic_filtering_wait_timeout = '15s'` fails with `does not exist`. You must use the config-file route above and restart the coordinator. (Hive, Delta, MySQL, PostgreSQL, and SQL Server probes DO support `SET SESSION <catalog>.dynamic_filtering_wait_timeout = '15s'` — but not Iceberg.) If per-query tuning is operationally required, the workaround options are: (a) deploy two Iceberg catalogs with different wait-timeout configs (e.g., `iceberg_batch.properties` with 30s and `iceberg_interactive.properties` with 1s) and use the appropriate catalog name in the SELECT; or (b) raise the catalog config to a value that works for the slowest expected build, accepting added latency on queries where DF wouldn't have helped anyway. |
...
> 2. Is the probe Iceberg / Hive / Delta? If YES → the 1s wait-timeout almost certainly fired. Run `EXPLAIN ANALYZE VERBOSE` to confirm wait time, then either (a) add a more selective build-side WHERE, or (b) raise the wait-timeout — **method depends on connector**: for Hive (`SET SESSION <hive_catalog>.dynamic_filtering_wait_timeout = '15s'`) and Delta (`SET SESSION <delta_catalog>.dynamic_filtering_wait_timeout = '15s'`) you can do it per-query. **For Iceberg you CANNOT — there is no session property; you must edit `etc/catalog/iceberg.properties` (`iceberg.dynamic-filtering.wait-timeout=15s`) and restart the coordinator.**
...
- Raise the per-catalog wait-timeout if the build side is consistently slow. **For the Iceberg-probe case, this requires editing `etc/catalog/iceberg.properties` (`iceberg.dynamic-filtering.wait-timeout=15s`) and restarting the coordinator — there is NO session-property form for the Iceberg connector.** For Hive/Delta/JDBC probes the session form works: `SET SESSION <catalog>.dynamic_filtering_wait_timeout = '15s'` (catalog-name prefix, NOT the connector name).
...
>
> Per-session form (run before your query) — **applies only to Hive, Delta, MySQL, PostgreSQL, SQL Server. The Iceberg connector has no session-property form.**
> ```sql
> -- For Iceberg-probe topology (most common): there is NO per-session option.
> --   SET SESSION iceberg.dynamic_filtering_wait_timeout = '20s';   -- DOES NOT WORK
> -- (returns "Session property '...' does not exist")
> -- The ONLY way to raise the Iceberg probe's wait-timeout is to edit
> -- etc/catalog/iceberg.properties:  iceberg.dynamic-filtering.wait-timeout=20s
> -- and RESTART the coordinator.
>
> -- For MySQL-probe / Postgres-probe / Hive-probe / Delta-probe (session form supported):
> SET SESSION billing_mysql.dynamic_filtering_wait_timeout = '20s';
> SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '20s';
>
> -- NOT: SET SESSION app_pg.dynamic_filtering_wait_timeout = '20s'
> -- when PG is the BUILD side — PG isn't waiting for anything, it's producing values.
> -- (Only the PROBE-side catalog's timeout matters, regardless of connector type.)
> ```
>
...
|---|---|---|
| **MySQL** ([trino.io/docs/current/connector/mysql.html](https://trino.io/docs/current/connector/mysql.html)) | `<catalog>.dynamic_filtering_wait_timeout` (session property exists) | **20 seconds** |
| **PostgreSQL** ([trino.io/docs/current/connector/postgresql.html](https://trino.io/docs/current/connector/postgresql.html)) | `<catalog>.dynamic_filtering_wait_timeout` (session property exists) | **20 seconds** |
| **SQL Server** | `<catalog>.dynamic_filtering_wait_timeout` (session property exists) | **20 seconds** |
| **Iceberg** ([trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html)) | **NO session property** — Iceberg connector does NOT expose `dynamic_filtering_wait_timeout` as a session property. **Catalog-level config only**: set `iceberg.dynamic-filtering.wait-timeout=15s` in `etc/catalog/iceberg.properties` (the `iceberg.` prefix IS REQUIRED; bare form is silently ignored) and **restart the coordinator**. Attempting `SET SESSION iceberg.dynamic_filtering_wait_timeout = '...'` fails with `Session property '...' does not exist`. | **1 second** |
| **Hive** ([trino.io/docs/current/connector/hive.html](https://trino.io/docs/current/connector/hive.html)) | `<hive_catalog_name>.dynamic_filtering_wait_timeout` (session property exists) | **1 second** |
| **Delta Lake** ([trino.io/docs/current/connector/delta-lake.html](https://trino.io/docs/current/connector/delta-lake.html)) | `<delta_catalog_name>.dynamic_filtering_wait_timeout` (session property exists) | **1 second** |

> **CRITICAL — Iceberg vs Hive/Delta asymmetry on session-property support** (verified against trino.io/docs/current for Trino 481):
> - **Hive and Delta Lake connectors DO expose `<catalog>.dynamic_filtering_wait_timeout` as a catalog session property** — you can tune them per-query without a coordinator restart.
> - **The Iceberg connector does NOT expose any session property** for `dynamic_filtering_wait_timeout`. The only way to change Iceberg's wait-timeout is via the catalog config file `etc/catalog/iceberg.properties` line `iceberg.dynamic-filtering.wait-timeout=15s`, followed by a **coordinator restart**. This is a frequent footgun: copy-pasting `SET SESSION iceberg.dynamic_filtering_wait_timeout = '20s'` from a blog post or another connector's docs will return `Session property 'iceberg.dynamic_filtering_wait_timeout' does not exist`. The Iceberg connector simply does not register this session property — there is no SQL-level workaround.
> - **Practical implication for the "Iceberg fact × Postgres dim" pattern** (the most common federation join): if you need to raise the Iceberg probe's 1s wait-timeout, you must do it as a deploy step (edit catalog properties file + restart coordinator), not as a per-query session set. Plan capacity / change-management accordingly. The probe-side wait-timeout for the Postgres or MySQL side, by contrast, CAN be tuned per-session via `SET SESSION <jdbc_catalog>.dynamic_filtering_wait_timeout = '30s'`.
  ...
  - **What this means in practice for the "Iceberg fact x Postgres dim" pattern (the most common federation join)**: Iceberg is the probe with a **1s** default — meaning Trino will only wait 1 second for the Postgres-derived DF before launching the Iceberg scan. If your Postgres build side takes longer than 1s to deliver its values, the Iceberg scan starts without the DF and reads more files than necessary. **For batch jobs where DF is the difference between a 5-second query and a 5-minute query, raise it explicitly in the Iceberg catalog config file (`iceberg.dynamic-filtering.wait-timeout=15s` in `etc/catalog/iceberg.properties`) and restart the coordinator.** Unlike Hive/Delta/JDBC connectors, the Iceberg connector does NOT expose `dynamic_filtering_wait_timeout` as a session property — there is no `SET SESSION` equivalent for Iceberg's wait-timeout. The probe-side JDBC catalog (Postgres/MySQL/SQL Server) DOES support `SET SESSION <catalog>.dynamic_filtering_wait_timeout`, so when the probe is JDBC you have a per-query lever; when the probe is Iceberg you do not.
...
> **To tune the wait-timeout without a Trino coordinator restart, use the catalog session property form — BUT only the Hive, Delta, and JDBC (MySQL/PostgreSQL/SQL Server) connectors expose it. The Iceberg connector does NOT.**
>
> ```sql
> -- WORKS: MySQL / PostgreSQL / SQL Server / Hive / Delta connectors
> -- (replace billing_mysql with your actual catalog filename).
> SET SESSION billing_mysql.dynamic_filtering_wait_timeout = '5s';
> SET SESSION app_pg.dynamic_filtering_wait_timeout = '30s';
>
> -- DOES NOT WORK: Iceberg connector — no session property exists.
> -- The following statement returns: "Session property '...' does not exist"
> --   SET SESSION iceberg.dynamic_filtering_wait_timeout = '5s';   -- REJECTED
> -- To change Iceberg's wait-timeout you MUST edit etc/catalog/iceberg.properties
> -- (line: iceberg.dynamic-filtering.wait-timeout=15s) and RESTART the coordinator.
> ```
>
> Notes: for connectors that DO support the session form, the catalog-prefixed form is required — the bare `SET SESSION dynamic_filtering_wait_timeout` will error. **And the prefix is your catalog name (filename in `etc/catalog/`), not the connector type.** For Iceberg there is no session form at all, so the catalog config file is the only option.
>
...
> - If **Iceberg is the probe side** (e.g., 500M-row `iceberg_catalog.analytics.events` filtered by a Postgres-derived user-id list — the most common case), you CANNOT tune the wait-timeout per-session — the Iceberg connector does not expose a `dynamic_filtering_wait_timeout` session property. You must edit `etc/catalog/iceberg.properties` and add `iceberg.dynamic-filtering.wait-timeout=30s`, then **restart the coordinator**. The Postgres build-side wait-timeout has no effect on whether Iceberg waits. Plan changes through your deploy pipeline, not as ad-hoc per-query SETs.
...
How to set it — depends on which connector is the probe side:

```sql
-- ICEBERG probe side (most common — Postgres dimension building DF for Iceberg fact scan).
-- The Iceberg connector does NOT expose a session property for this. The following
-- statement would fail with "Session property 'iceberg.dynamic_filtering_wait_timeout'
-- does not exist":
--   SET SESSION iceberg.dynamic_filtering_wait_timeout = '15s';   -- DOES NOT WORK
--
-- Instead, edit etc/catalog/iceberg.properties on the coordinator and add:
--   iceberg.dynamic-filtering.wait-timeout=15s
-- then RESTART the coordinator. This is the only way to change the Iceberg wait-timeout.

-- PostgreSQL probe side (rarer — when Iceberg/another source is building DF for a Postgres scan).
-- JDBC connectors DO support the session property form, so per-query tuning works:
SET SESSION app_pg.dynamic_filtering_wait_timeout = '30s';

-- MySQL probe side — also supports the session property:
SET SESSION billing_mysql.dynamic_filtering_wait_timeout = '30s';

-- Hive probe side — session property supported (catalog-prefixed, underscores):
SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '15s';

-- Delta Lake probe side — session property supported (catalog-prefixed, underscores):
SET SESSION delta_lake.dynamic_filtering_wait_timeout = '15s';
```

> **Bare `SET SESSION dynamic_filtering_wait_timeout = '15s'` does NOT work for any connector.** Without the catalog prefix, Trino returns `Session property 'dynamic_filtering_wait_timeout' does not exist` because this is a per-catalog property, not a system property. For Hive / Delta / JDBC connectors, prefix with the **actual catalog name** (`app_pg.`, `billing_mysql.`, `hive_warehouse.`, etc.) — that is the filename in `etc/catalog/` without `.properties`, NOT the connector type. Verify with `SHOW CATALOGS;`. **For the Iceberg connector there is no session property at all — even with the correct catalog prefix, `SET SESSION iceberg.dynamic_filtering_wait_timeout = '15s'` returns `does not exist`. Iceberg's wait-timeout is catalog-config-only.**

> **Session property uses underscores; config file property uses hyphens — AND the Iceberg connector has no session form at all.** For connectors that DO support the session form (Hive, Delta, MySQL, PostgreSQL, SQL Server), the session property is `<catalog_name>.dynamic_filtering_wait_timeout` (underscores in the property name after the **catalog-name** prefix — e.g., `hive_warehouse.dynamic_filtering_wait_timeout` if your Hive catalog file is named `hive_warehouse.properties`). The config-file form inside the catalog's properties file uses hyphens. **For Iceberg/Hive/Delta the config-file line must include the connector-name prefix**: `iceberg.dynamic-filtering.wait-timeout=15s` inside `etc/catalog/iceberg.properties` (the `iceberg.` prefix is required; bare `dynamic-filtering.wait-timeout=15s` is silently ignored by the Iceberg connector). For PostgreSQL/MySQL JDBC connectors the config-file line is the **bare** form `dynamic-filtering.wait-timeout=30s` (JDBC base config — no prefix). Mixing forms is a common footgun. **And for Iceberg specifically: there is NO session-property equivalent — the only way to change the wait-timeout is via the catalog config file plus a coordinator restart.** Hive and Delta DO have session-property equivalents even though they share the lakehouse 1s default; Iceberg is the odd one out.
...
> **Catalog `.properties` file prefix rules — depends on the connector.** Connector-specific properties in `etc/catalog/<name>.properties` require the **connector-name prefix** for lakehouse connectors (Iceberg, Hive, Delta), but JDBC connectors (PostgreSQL, MySQL, SQL Server) expose `dynamic-filtering.wait-timeout` as a **bare** property because it comes from the general JDBC base config. **Without the required prefix, the property is silently ignored** — that is the #1 reason a `dynamic-filtering.wait-timeout` setting "had no effect." Inside a SQL session you use the **catalog-name-prefixed** session-property form with underscores (`SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '15s';` if your Hive catalog file is `hive_warehouse.properties`) — **but ONLY for Hive, Delta, MySQL, PostgreSQL, SQL Server. The Iceberg connector has no session-property form for this setting.** Side-by-side:
>
> | Where it goes | Correct line | Reason |
> |---|---|---|
> | `etc/catalog/iceberg.properties` (lakehouse) | `iceberg.dynamic-filtering.wait-timeout=15s` (requires coordinator restart) | Iceberg connector property table requires the `iceberg.` prefix. Bare form silently ignored. **This is the ONLY way to change Iceberg's wait-timeout — no session form exists.** |
> | `etc/catalog/hive.properties` (lakehouse) | `hive.dynamic-filtering.wait-timeout=15s` | Same prefix rule — `hive.` required. Also tunable via session: `SET SESSION hive.dynamic_filtering_wait_timeout = '15s'`. |
> | `etc/catalog/delta.properties` (lakehouse) | `delta.dynamic-filtering.wait-timeout=15s` | Same prefix rule — `delta.` required. Also tunable via session: `SET SESSION delta.dynamic_filtering_wait_timeout = '15s'`. |
> | `etc/catalog/app_pg.properties` (PostgreSQL JDBC) | `dynamic-filtering.wait-timeout=30s` | JDBC connectors use the **bare** form (comes from JDBC base config). Also tunable via `SET SESSION app_pg.dynamic_filtering_wait_timeout = '30s'`. |
> | `etc/catalog/billing_mysql.properties` (MySQL JDBC) | `dynamic-filtering.wait-timeout=30s` | Same as PostgreSQL — bare form, no `mysql.` prefix. Also tunable via session. |
> | SQL session (per query) — Hive / Delta / JDBC | `SET SESSION <catalog>.dynamic_filtering_wait_timeout = '15s';` | Catalog-name prefix (filename without `.properties`) + **underscores** in the property name. |
> | SQL session (per query) — **Iceberg** | **NOT SUPPORTED** — no such session property exists. Edit `etc/catalog/iceberg.properties` and restart the coordinator. | The Iceberg connector does not register `dynamic_filtering_wait_timeout` as a session property; only Hive, Delta, and JDBC connectors do. |
>
...
> **CRITICAL prefix rule — depends on the connector.** For **Iceberg / Hive / Delta** (lakehouse connectors), the `dynamic-filtering.wait-timeout` property requires the connector-name prefix in `etc/catalog/<name>.properties` — `iceberg.dynamic-filtering.wait-timeout`, `hive.dynamic-filtering.wait-timeout`, `delta.dynamic-filtering.wait-timeout`. **The bare form `dynamic-filtering.wait-timeout=15s` inside an Iceberg catalog file is SILENTLY IGNORED** — Trino does not error, it simply does not apply the setting. For **PostgreSQL / MySQL / SQL Server JDBC** connectors, the property is expressed as the **bare** `dynamic-filtering.wait-timeout=30s` (no `postgresql.` / `mysql.` prefix) because it comes from the JDBC base configuration that all JDBC connectors inherit. (Inside a SQL session — for the connectors that support it — `SET SESSION <catalog_name>.dynamic_filtering_wait_timeout = '15s';` works for Hive, Delta, MySQL, PostgreSQL, SQL Server: catalog-name prefix, underscores in the property name. **The Iceberg connector does NOT expose this as a session property — there is no `SET SESSION iceberg.dynamic_filtering_wait_timeout` equivalent; the catalog file + coordinator restart is the only route for Iceberg.**)

> **These are per-connector properties set in each catalog's properties file.** The defaults are NOT uniform: Iceberg / Hive / Delta = **1 second** (lakehouse style — go fast), and PostgreSQL / MySQL / SQL Server JDBC = **20 seconds** (give the slow JDBC build side a chance). **The Iceberg 1-second default is the most common reason a federated "Iceberg fact × Postgres dim" join shows `dynamicFilterSplitsProcessed=0`** — the Postgres build side takes more than 1s to deliver, and Iceberg has already started its scan unfiltered. The fix is to raise the wait-timeout explicitly: in `etc/catalog/iceberg.properties` use the prefixed form `iceberg.dynamic-filtering.wait-timeout=15s` (the prefix is REQUIRED — the bare form is silently ignored) and **restart the coordinator**. **For Iceberg there is NO per-session alternative** — `SET SESSION iceberg.dynamic_filtering_wait_timeout = '15s'` fails with `does not exist`. For Hive/Delta/JDBC probes the per-session form is supported and is the preferred lever when only some queries need a different timeout.
...
> **ANTI-PATTERN — BROADCAST is NOT the OOM fix; it is the OOM CAUSE on large tables. Do not invert these.**
>
> Engineers under pressure sometimes reach for `SET SESSION join_distribution_type = 'BROADCAST'` thinking it will "make the join smaller / faster." On a 50M-row Postgres table (~10–20 GB), BROADCAST is exactly what causes the OOM:
>
> | Distribution | What Trino actually does | Effect on a 50M-row build side |
> |---|---|---|
> | **BROADCAST** | Replicates the **entire** build side to **every** worker. Every worker holds a complete copy of the build hash table. | Every worker tries to hold 10–20 GB. Default `join_max_broadcast_table_size = 100MB` is the safety cap — exceeding it via stale stats or a forced override causes worker OOM. **This is the OOM trigger, not the fix.** |
> | **PARTITIONED** | Hash-redistributes **both** sides across workers by the join key. Each worker sees only its `1/N` slice of both sides. | Each worker holds ~`(10–20 GB) / N` of the build. Memory pressure is divided across the cluster. Trade-off: an extra network shuffle adds latency, but workers stay alive. **This is the OOM remedy.** |
>
> Mnemonic: **B**ROADCAST = **B**ig copy to every worker (dangerous on big tables). **P**ARTITIONED = **P**artition / split across workers (safe on big tables). For OOM-fixing on tables larger than ~10M rows (or anything that would not fit comfortably in `join_max_broadcast_table_size` after column pruning and filter pushdown), use **PARTITIONED**. BROADCAST is only correct for **small** build sides — typically under 100MB after filtering, never multi-GB Postgres dimension tables. The BROADCAST workaround discussed elsewhere in this doc (Section 5.5.1) is for the OPPOSITE shape — a tiny (e.g., 5K-row) Postgres dimension joined to a huge Iceberg fact, where the small side fits comfortably in worker memory. Do not transplant that advice to the 50M-row OOM case.
...
- The `source` field is matched against the **client-supplied** `X-Trino-Source` header / JDBC `source=` URL parameter (also surfaced as the JDBC `ApplicationName` connection property) / CLI `--source` flag. **`source` does NOT match the catalog name.** A selector like `{ "source": ".*postgresql.*", "group": "federation.queries" }` will **NOT** route queries that touch the `postgresql` catalog — because `source` is whatever string the **client process** sets when it opens the connection (e.g., `"tableau-dashboard"`, `"airflow-etl"`, `"metabase"`), independent of which catalogs the query later references. To route by catalog, you would have to (a) train every client to set `source` to a value that identifies the workload (e.g., `dashboard.source=federation-queries`), or (b) match on `user` / `userGroup` / `clientTags` instead. Clients MUST set this value (see "How to make the `source` regex match your queries" below).
...
> **Note**: `hardConcurrencyLimit` caps the number of **concurrent Trino queries** in the group, not JDBC connections directly. **JDBC connection model — the precise rule**: **each Postgres `TableScan` operator in the query plan = exactly 1 JDBC connection**. One split per non-partitioned TableScan, one worker task per split, one connection per task. It is NOT one connection per worker — the connection is opened by the single worker assigned the split, regardless of cluster size. The implications for sizing:
> - A query scanning **one** Postgres table → **1** JDBC connection.
> - A query that **joins two different Postgres tables** (e.g., `users JOIN orders` in the same Postgres catalog) → **2** JDBC connections (one per TableScan operator).
> - A query joining three Postgres tables → **3** connections, and so on.
> - A query joining a Postgres table to an Iceberg table → **1** Postgres connection (Iceberg uses S3/MinIO, not JDBC).
>
> Connection count scales with `concurrent_queries × Postgres_TableScans_per_query × splits_per_TableScan`. On OSS Trino 467 `splits_per_TableScan` is always 1 for non-partitioned tables. **Sizing rule of thumb for `default_pool_size`**: take the worst-case `hardConcurrencyLimit` for any federation resource group, multiply by the worst-case number of Postgres tables joined in a single query for that group, then add a small margin (10–20%) for short-lived metadata-fetch connections from coordinator planning. Example: `hardConcurrencyLimit=10` × 3 Postgres tables per query = 30 backend connections worst-case → `default_pool_size ≥ 35`. The exceptions where one TableScan opens multiple connections are: (a) the `partition-column` property — **Starburst Enterprise only, NOT available in OSS Trino 467** — is set so Trino splits the table into N parallel range-scans (N connections per TableScan), or (b) a custom split strategy is in use. For typical single-table federation with PgBouncer `default_pool_size=50`, that ceiling supports roughly 50 concurrent federation queries before connections saturate. For tight connection-count control, combine resource groups with PgBouncer (Section 8.2A) and a Postgres role-level `CONNECTION LIMIT` (Section 8.2B). Resource groups are the Trino-side lever; PgBouncer is the network-side lever.
...
- **Configure per session**: supported only for Hive, Delta, MySQL, PostgreSQL, SQL Server — `SET SESSION <catalog_name>.dynamic_filtering_wait_timeout = '15s'` (e.g., `SET SESSION billing_mysql.dynamic_filtering_wait_timeout = '30s'` or `SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '15s'`). Catalog-prefixed (using the **catalog name** = filename in `etc/catalog/` without `.properties`), NOT system-level. **NOT supported for the Iceberg connector** — `SET SESSION iceberg.dynamic_filtering_wait_timeout = '15s'` returns `Session property '...' does not exist`. For Iceberg the only route is the catalog config file (above) plus a coordinator restart.
...
#### `START TRANSACTION` / `BEGIN` in Trino does NOT give cross-catalog atomicity — common misconception

> **CRITICAL — read before writing any federated write code.** Engineers often see that Trino supports `START TRANSACTION` / `COMMIT` / `ROLLBACK` and assume this gives them cross-catalog atomicity, the same way `BEGIN ... COMMIT` works in PostgreSQL. **It does not.** Trino's transaction block **does NOT coordinate commits across catalogs**. Each connector commits to its own backend independently the moment its DML statement finishes, regardless of whether you're inside a `START TRANSACTION` block.
>
> **The misconception in code form — this is NOT atomic across catalogs:**
>
> ```sql
> -- ANTI-PATTERN — looks atomic, is NOT atomic across catalogs:
> START TRANSACTION;
>   INSERT INTO iceberg.analytics.audit_log SELECT ...;   -- commits to Iceberg/HMS immediately when done
>   UPDATE app_pg.public.users SET status = 'archived' WHERE id = 42;  -- commits to Postgres immediately when done
> COMMIT;  -- this COMMIT is a no-op for cross-catalog coordination — both writes already landed in their own systems
> ```
>
> If the `UPDATE` against Postgres fails, **the Iceberg `INSERT` is NOT rolled back** — the new rows are already visible in Iceberg, the HMS metadata pointer was already swapped, and there is no mechanism in Trino to undo them. Conversely, if the Iceberg INSERT succeeds and then the coordinator dies before the UPDATE runs, the Iceberg side committed but the Postgres side never ran at all. Either failure mode leaves the two systems **permanently inconsistent** with no automatic recovery.
>
> **What `START TRANSACTION` actually controls in Trino:** it only changes the autocommit behavior **within a single connector**, for connectors that implement their own per-statement transaction semantics. It allows multiple statements against the **same catalog** to be grouped into one connector-level transaction (where the underlying connector supports it). It is **not a two-phase commit (2PC) coordinator** and Trino does **not implement XA / 2PC across catalogs** at all. There is no Trino setting, no session property, and no future-roadmap feature in OSS Trino 467 that turns cross-catalog DML into an atomic unit.
>
> **The three correct patterns for "I need a write to two systems to either both succeed or both fail" — pick one:**
>
> 1. **App-level coordination with idempotent retries.** Do the writes from your application code, not from Trino. Use an outbox table + a worker that retries each side until both succeed; record a transaction ID so retries are idempotent. The simplest pattern for SaaS apps.
> 2. **CDC with Debezium + Kafka.** Write only to Postgres (the system of record); let Debezium tail the WAL and publish to Kafka; a Kafka consumer materializes the changes into Iceberg. Failures buffer in Kafka and retry automatically; eventually consistent within seconds. The right answer for "Postgres is the source of truth and Iceberg is a derived analytical store."
> 3. **Batch MERGE INTO from a single source.** If both targets can be rebuilt from a single Iceberg source, run a nightly `MERGE INTO` job that reconciles each target separately from the same source. Each `MERGE` is its own retryable unit; the source is the durable record. Simpler to operate than CDC, higher latency.
>
> **Restating the rule once more, plainly:** treat every Trino DML statement as committing to its own catalog independently the moment it finishes, even inside a `START TRANSACTION` block. If two writes must be atomic, they cannot both happen through Trino — one of the three patterns above is required.
...
|---|---|---|---|
| **Predicate pushdown** (WHERE clauses) | **PUSHES by default**: Numeric `=`, `!=`, `IN`, `BETWEEN`, range (`<`, `>`); UUID `=`; DATE/TIMESTAMP `=` and range; `IS NULL`/`IS NOT NULL` (all types); **VARCHAR equality and inequality** (`=`, `!=`, `IN`) — **doc-quoted**: "Equality predicates, such as `IN` or `=`, and inequality predicates, such as `!=` on columns with textual types are pushed down." **DOES NOT push by default**: **VARCHAR range** (`<`, `>`, `BETWEEN` on text) — **doc-quoted**: "The connector does not support pushdown of range predicates, such as `>`, `<`, or `BETWEEN`, on columns with character string types"; **`LIKE 'foo%'` anchored** is collation-dependent and conservative (verify with EXPLAIN); **`LIKE '%foo'`** (leading-wildcard) NEVER pushes usefully; **`ILIKE`** gated by `enable_string_pushdown_with_collate`; **function calls in predicate** (`LOWER(col) = ...`, `CAST(col AS ...)`) never push. | `TableScan[...constraint=(predicate here)]` — predicate is INSIDE the scan node; no `Filter`/`ScanFilterProject` above. | trino.io/docs/current/optimizer/pushdown.html, trino.io/docs/current/connector/postgresql.html |
...
> **Sourcing note — empirically observable, not an explicit doc-quoted rule.** The trino.io optimizer/pushdown.html and connector/postgresql.html docs describe predicate pushdown and aggregate pushdown as **independent optimizations** and **do not explicitly state** an ordering dependency between them. The ordering dependency below is the **empirically observed plan-tree behavior**: when a `ScanFilterProject` or `Filter` node sits between an `Aggregate` and a `TableScan`, the rewrite rule that fuses aggregate into the connector scan no longer matches (the aggregate is not a direct parent of the TableScan), so the aggregate pushdown rule does not fire. The behavior is **EXPLAIN-observable on any Trino 467 cluster** and is the practical mental model engineers need; treat it as "this is what the plan tree does," not "this is what the docs say." If you want a doc-level confirmation, the only canonical claim trino.io commits to is the success signature — "If an aggregate function is successfully pushed down to the connector, the explain plan does **not** show that `Aggregate` operator" (trino.io/docs/current/optimizer/pushdown.html). The cause/effect relationship between unpushed predicates and unpushed aggregates is derived from observing the plan tree, not from a doc-asserted rule.
...
**How to CONFIRM DF actually fired at runtime (the only definitive check)** — **doc-quoted source**: trino.io/docs/current/admin/dynamic-filtering.html says `dynamicFilterSplitsProcessed` "records the number of splits processed after a dynamic filter is pushed down." Non-zero = DF fired; zero = DF did NOT fire.
...
**The wait-timeout property — doc-quoted catalog property names and defaults:**

| Probe connector | Catalog property | Default | Session-property form available? |
|---|---|---|---|
| Iceberg (the canonical SaaS shape — Iceberg fact is the probe) | `iceberg.dynamic-filtering.wait-timeout` | **1s** (per trino.io/docs/current/connector/iceberg.html: "Maximum duration to wait for completion of dynamic filters during split generation.") | **NO** — Iceberg connector does not register a session-property form. Edit `etc/catalog/iceberg.properties` + restart coordinator. |
| PostgreSQL JDBC (reverse case — Postgres is probe, rarer) | `dynamic-filtering.wait-timeout` (no `postgresql.` prefix) | **20s** (per trino.io/docs/current/connector/postgresql.html: "Maximum duration for which Trino waits for dynamic filters to be collected from the build side of joins before starting a JDBC query.") | **YES** — `SET SESSION app_pg.dynamic_filtering_wait_timeout = '30s'` works. |
| MySQL JDBC | same shape as PostgreSQL | 20s | YES — same shape as PostgreSQL. |
| Hive / Delta Lake | same shape as Iceberg property name but session-form DOES work | 1s | YES — `SET SESSION hive_warehouse.dynamic_filtering_wait_timeout = '15s'` works. |

> **Iceberg-probe is the asymmetric case** — the catalog property exists (doc-quoted), but the session-property form is missing from the Iceberg connector's session-property registry in Trino 467/481. Verify with `SHOW SESSION LIKE '%dynamic_filtering%'` on a live cluster — entries appear for hive/delta/jdbc catalogs but NOT for iceberg.

**Which join TYPES and PREDICATES support DF** (verified verbatim against trino.io/docs/current/admin/dynamic-filtering.html: "inner and right joins with `=`, `<`, `<=`, `>`, `>=` or `IS NOT DISTINCT FROM` join conditions, and semi-joins with `IN` conditions are supported"):

| Join shape | DF? | Note |
|---|---|---|
| `INNER JOIN ... ON a.id = b.id` | **YES** | The canonical case; IN-list derived. **Doc-quoted support.** |
| `INNER JOIN ... ON a.x < b.y` (inequality on join key — `<`, `<=`, `>`, `>=`) | **YES** for INNER and RIGHT joins | Min/max range derived. **Doc-quoted support** ("inner and right joins with `=`, `<`, `<=`, `>`, `>=`"). The "DF only works on equality joins" claim is a common misconception. |
| `RIGHT JOIN ... ON a.id = b.id` | **YES** | Same as INNER for DF purposes. **Doc-quoted.** |
| `ON a.id IS NOT DISTINCT FROM b.id` (null-safe equality) | **YES** | **Doc-quoted** — listed alongside the regular equality and inequality operators. |
| `LEFT JOIN ... ON a.id = b.id` | **NO** | LEFT must return every left-side row regardless of match; pruning would drop required rows. **Doc-quoted exclusion** (only inner and right joins are listed as supported). |
| `FULL OUTER JOIN ... ON a.id = b.id` | **NO** | Same reason as LEFT. **Doc-quoted exclusion.** |
| `ON LOWER(a.name) = b.name` (function in ON clause) | **NO** | Derived expressions block clean predicate pushdown — function in ON disables DF even on an INNER join. |
| `WHERE x IN (SELECT ...)` semi-join | **YES** | **Doc-quoted support** — "semi-joins with `IN` conditions are supported." |

> **WRONG — do NOT rewrite `INNER JOIN` to `LEFT JOIN` to "enable" DF.** Rewriting in that direction DISABLES DF entirely (LEFT and FULL OUTER are NOT in the supported list per trino.io/docs/current/admin/dynamic-filtering.html). If DF is not firing on an INNER join, the cause is (a) wait-timeout, (b) build-side row cap exceeded (raise `enable_large_dynamic_filters`), (c) IN-list compacted to BETWEEN by `domain-compaction-threshold`, or (d) join key is VARCHAR and probe is MySQL (Section 2A.2). Fix the actual cause; do not break the join shape.

> **Default-on in OSS Trino 467 — doc-quoted.** trino.io/docs/current/admin/dynamic-filtering.html states: "Dynamic filtering is enabled by default. It can be disabled by setting either the `enable-dynamic-filtering` configuration property, or the session property `enable_dynamic_filtering` to `false`." Engineers do NOT need to opt-in. The verification path is to RUN the query (not just EXPLAIN — EXPLAIN ANALYZE) and check that `dynamicFilterSplitsProcessed > 0` on the probe-side TableScan.
...
> **SCOPE of this section.** This section is about **TopN pushdown for a single-source query against a PostgreSQL table** — `SELECT ... FROM pg.t ORDER BY col LIMIT N` and its failure-shape variants. It is **NOT** about MERGE-broadcast-join behavior (that's an Iceberg MERGE topic — see `resources/13-postgres-to-iceberg-ingestion.md` and `resources/17-iceberg-table-maintenance.md`). When explaining why an aggregate `GROUP BY ... ORDER BY agg DESC LIMIT N` doesn't push, the mechanism is **"the connector cannot sort on a value Postgres has not produced yet"** — NOT "the staging table contains all today's events" / "the MERGE is joining every source row." Do not mix MERGE-target-table-scan vocabulary into a Postgres GROUP BY answer; the two are unrelated.
...
> **trino.io/docs/current/optimizer/pushdown.html (verbatim):** *"If an aggregate function is successfully pushed down to the connector, the explain plan does not show that `Aggregate` operator."*

So the success signature is: **the `Aggregate` Trino operator is ABSENT** from the plan above the `TableScan`. Concretely:

```
-- SUCCESS — aggregate pushed to Postgres:
Output[customer_id, _col1, _col2, _col3]
└── TableScan[app_pg:Query[SELECT customer_id, sum(amount), count(*), avg(amount)
                            FROM public.orders WHERE status = 'paid'
                            GROUP BY customer_id]]
    -- No `Aggregate` operator. No `Filter`. Postgres did the GROUP BY + WHERE.

-- FAILURE — aggregate stayed on Trino (e.g., a WHERE predicate failed to push):
Output[customer_id, _col1, _col2, _col3]
└── Aggregate[GROUP BY customer_id] sum(amount), count(*), avg(amount)
    └── ScanFilterProject[filter = LOWER(email) = 'foo@bar.com']
        └── TableScan[app_pg.orders, constraint=(status = 'paid')]
    -- The `Aggregate` operator is PRESENT. Postgres returned post-`status` rows;
    -- Trino applied the `LOWER(email)` residual filter, then aggregated on workers.
```

**EXPLAIN ANALYZE — the runtime smoking gun:**

```
-- SUCCESS: only ~K rows (one per group) cross the wire from Postgres:
TableScan[app_pg:Query[SELECT customer_id, sum(amount), ... GROUP BY customer_id]]
    CPU: 95ms, Input: 1,200 rows (84 kB), Output: 1,200 rows (84 kB)
    -- 1,200 groups came back. Trivial network egress.

-- FAILURE: the full unaggregated table crossed the wire:
TableScan[app_pg.orders]
    CPU: 14.8s, Input: 50,000,000 rows (4.0 GB), Output: 50,000,000 rows (4.0 GB)
    -- 50M rows on JDBC. The aggregate ran on Trino workers AFTER the residual filter.
```

**Supported aggregate functions (trino.io/docs/current/connector/postgresql.html — verbatim list):** `avg()`, `count()`, `max()`, `min()`, `sum()`, `stddev()`, `stddev_pop()`, `stddev_samp()`, `variance()`, `var_pop()`, `var_samp()`, `covar_pop()`, `covar_samp()`, `corr()`, `regr_intercept()`, `regr_slope()`.

**NOT supported for pushdown (by trino.io optimizer/pushdown.html):**

- Aggregations on **expressions inside the aggregate call** — e.g., `SUM(a * b)`, `SUM(CASE WHEN ... END)`, `COUNT(DISTINCT col)` on most JDBC connectors. The connector falls back to in-Trino aggregation; the `Aggregate` operator appears.
- Complex grouping operations — `ROLLUP`, `CUBE`, `GROUPING SETS`. These never push to PostgreSQL; Trino does the rollup itself.
- `COUNT(DISTINCT col)` — does NOT push on the PostgreSQL connector by default (it's not one of the simple "five-and-related" supported list above for distinct counts). For exact distinct counts, run inside `system.query()`; for an approximate count, switch to `approx_distinct(col)` which pushes as part of the supported `count`/`sum`/etc family in some shapes — verify with EXPLAIN.
- `count(1)` — the `count(*)` form pushes cleanly; the `count(1)` form has historically been blocked when used with `GROUP BY` ([trinodb/trino issue #7251](https://github.com/trinodb/trino/issues/7251)). Prefer `count(*)` in federated queries.

**Session property — `aggregation_pushdown_enabled` (default `true`):**
...
-- Disable for A/B debugging (force aggregation onto Trino workers):
SET SESSION app_pg.aggregation_pushdown_enabled = false;
-- Re-run EXPLAIN; the `Aggregate` operator should now reliably appear above the scan.
RESET SESSION app_pg.aggregation_pushdown_enabled;
```
...
#### 13.5A.2 HAVING-clause pushdown — what pushes, what doesn't, and the EXPLAIN signature

**The single canonical statement.** `SELECT customer_id, SUM(amount) FROM app_pg.public.orders GROUP BY customer_id HAVING SUM(amount) > 1000` pushes the **HAVING predicate together with the aggregation** to PostgreSQL **when the aggregation itself pushes** — i.e., when every WHERE predicate (if any) already pushes AND the aggregate functions are in the supported list (§13.5A.1). In that case Postgres runs `SELECT customer_id, SUM(amount) FROM public.orders GROUP BY customer_id HAVING SUM(amount) > 1000` end-to-end and returns only the groups above the threshold.

**EXPLAIN signature — the success case (HAVING pushed alongside the aggregate):**

```
Output[customer_id, _col1]
└── TableScan[app_pg:Query[SELECT customer_id, sum(amount) FROM public.orders
                            GROUP BY customer_id HAVING sum(amount) > 1000]]
    -- The TableScan's synthetic query embeds the HAVING.
    -- No `Aggregate` operator. No `Filter` operator above the TableScan.
    -- Postgres did the GROUP BY + HAVING; Trino just streams the result rows.
```

**EXPLAIN signature — the failure case (HAVING stayed on Trino):**

```
Output[customer_id, _col1]
└── Filter[sum_amount > 1000]                          <-- HAVING ran on Trino
    └── Aggregate[GROUP BY customer_id] sum(amount)    <-- Aggregate ran on Trino
        └── TableScan[app_pg.orders]
    -- Either the aggregate didn't push (a WHERE predicate didn't push),
    -- OR a connector limitation prevented HAVING pushdown for that shape.
```

**The pushdown ordering, in one sentence.** HAVING pushdown follows aggregation pushdown — if the aggregation isn't pushed (because a WHERE predicate stayed on Trino, or the aggregate function isn't supported, or the GROUP BY uses ROLLUP/CUBE), then HAVING also stays on Trino, because Trino must apply the HAVING to the rows it just produced via its own aggregate. **No HAVING push without an aggregate push first.**

**Sourcing note — empirical vs doc-quoted.** The trino.io/docs/current/optimizer/pushdown.html page covers aggregation pushdown but does NOT explicitly enumerate a separate "HAVING pushdown" rule. The mechanism comes from [trinodb/trino PR #6667 — "HAVING pushdown and more advanced aggregation pushdown in JDBC connectors"](https://github.com/trinodb/trino/pull/6667), which restructured the JDBC `JdbcTableHandle` to convert aggregation directly into a query (instead of separate `groupingSets` + `expression` components), enabling subsequent pushdowns such as predicate-after-aggregation — i.e., HAVING. **Treat HAVING pushdown as "an EXPLAIN-observable consequence of aggregation pushdown" rather than a doc-asserted separate optimization.** Verify on your specific query with EXPLAIN.

**What does NOT push (HAVING-side):**

- HAVING on **expressions inside the aggregate** — e.g., `HAVING SUM(a * b) > 1000`. The underlying aggregate `SUM(a * b)` itself doesn't push (expression inside the aggregate is unsupported), so HAVING cannot push either.
- HAVING that references columns not in the GROUP BY — Trino-side parse error before pushdown even runs.
- HAVING after a `ROLLUP`/`CUBE`/`GROUPING SETS` — those complex grouping shapes don't push, so neither does HAVING above them.
- HAVING on a column with a non-default collation in the comparison — same correctness concern as VARCHAR range pushdown (§3.3); blocked unless `enable_string_pushdown_with_collate=true` is set.

**Common mistake.** Engineers see "HAVING didn't push" in EXPLAIN and conclude HAVING pushdown is "not supported in Trino." It IS supported; the cause for THIS query is almost always upstream — either the aggregate didn't push, or the aggregate function isn't in the supported list. Fix the aggregate-pushdown problem and HAVING follows.

---

#### 13.5A.3 3-way cross-catalog JOIN — where does each join execute, and how to read EXPLAIN

**The single canonical statement.** A 3-way join like `iceberg.analytics.events e JOIN app_pg.public.accounts a ON e.account_id = a.id JOIN iceberg.analytics.regions r ON a.region_id = r.id` **never pushes any of the joins to a connector** — joins across catalogs are structurally impossible to push (one side knows about Iceberg, the other doesn't know about PostgreSQL, etc.). **All three joins execute on Trino workers.** Each catalog's `TableScan` is performed in parallel against its respective source; the join results are exchanged across worker stages.

**The execution-location rule, in three sentences.** (1) Each `TableScan` runs on Trino workers, **fetching from the respective connector** (Iceberg from MinIO via the Iceberg connector; PostgreSQL over JDBC). (2) Each pairwise `Join` operator runs on Trino workers — there is no "Postgres does the join" or "Iceberg does the join" option when sides live in different catalogs. (3) Trino's CBO picks the **join order and distribution strategy** (BROADCAST vs PARTITIONED) for each pairwise join based on table statistics — run `ANALYZE TABLE` on each source so the CBO has reliable row-count estimates.

**The EXPLAIN signature — three `TableScan` nodes under two `InnerJoin` operators:**

```
Output[...]
└── InnerJoin[a.region_id = r.id]                           <-- 2nd join: events+accounts JOIN regions
    │   distribution: REPLICATED (regions broadcast as build)
    ├── InnerJoin[e.account_id = a.id]                      <-- 1st join: events JOIN accounts
    │   │   distribution: REPLICATED (accounts broadcast as build)
    │   ├── TableScan[iceberg.analytics.events]             <-- Iceberg scan (probe of 1st join)
    │   │       constraint = (event_date >= DATE '2026-05-01')
    │   │       dynamicFilters = {account_id = (build-side IN-list)}
    │   └── RemoteExchange[REPLICATE]                       <-- broadcast accounts to every worker
    │       └── TableScan[app_pg:public.accounts]           <-- Postgres scan over JDBC
    │               constraint = (region_id IN (...))       (build-side filter pushed to PG)
    └── RemoteExchange[REPLICATE]                           <-- broadcast regions to every worker
        └── TableScan[iceberg.analytics.regions]            <-- Iceberg scan of dim
```

**How to read the plan in five bullets:**

1. **Three `TableScan` nodes — one per catalog.** Each runs against its own connector in parallel. Iceberg scans pull files from MinIO; PostgreSQL scans send a JDBC SQL with the pushed predicates.
2. **Two `InnerJoin` operators — both on Trino workers.** Trino's CBO orders them; the leaf join runs first, then the result feeds the upper join.
3. **`RemoteExchange[REPLICATE]` = BROADCAST distribution.** The build side (the smaller, the dimension) is replicated to every worker; the probe side (the larger fact, or the join result) stays partitioned where it was scanned.
4. **`RemoteExchange[REPARTITION]` = PARTITIONED distribution.** Both sides hash-shuffled by join key. The CBO picks this when the build side is bigger than `join-max-broadcast-table-size` (default 100MB) — see §13.3 / §5.
5. **Dynamic filtering crosses catalogs.** The build-side scan's distinct join-key values are turned into an IN-list and pushed INTO the probe-side scan's connector — including JDBC. On the EXPLAIN, look for `dynamicFilters = {col = #df_N}` on the probe `TableScan`. On Iceberg-probe, this prunes data files at split-generation time; on Postgres-probe, the JDBC SQL gets `AND col IN (...)` appended.

**Reading "which side dominates" — the operational rule of thumb:**

- The **biggest probe scan dominates wall-clock time.** Optimize THAT scan first: partition pruning on Iceberg (§4 in resource 28), pushdown on Postgres (§13.1), narrow projection (only the columns the join needs).
...
**Forcing a strategy — when CBO picks wrong:**

```sql
-- Force BROADCAST (only when you know all build sides fit in worker memory):
SET SESSION join_distribution_type = 'BROADCAST';

-- Force PARTITIONED (when a build side would OOM via BROADCAST):
SET SESSION join_distribution_type = 'PARTITIONED';

-- Reset to CBO-driven choice:
SET SESSION join_distribution_type = 'AUTOMATIC';
```

These properties are SYSTEM-level (no catalog prefix), and they apply to ALL joins in the session — including the federated ones.
...
| `WHERE` shape | Pushes? | Why |
|---|---|---|
| `(user_id = 123 OR email = 'a@b.com')` — numeric `=` OR VARCHAR `=`, default collation | **YES — entire OR pushes.** Both disjuncts individually push (numeric equality always; VARCHAR equality with default collation). | Trino sends `WHERE (user_id = 123 OR email = 'a@b.com')` to Postgres as a single SQL clause. |
| `(user_id = 123 OR email LIKE '%bigcorp.com')` — numeric `=` OR leading-wildcard LIKE | **NO — entire OR stays on Trino.** The leading-wildcard LIKE doesn't push regardless of collation (no anchored prefix = no B-tree range scan; §3.3). So the OR cannot push as a unit. Trino pulls **all rows matching the OR via no predicate at all** (or only what the AND-side predicates push) and applies the OR in worker memory. | The connector cannot safely omit the LIKE disjunct — that would return fewer rows than the OR requires. |
| `(user_id = 123 OR name BETWEEN 'a' AND 'm')` — numeric `=` OR VARCHAR range | **NO by default.** VARCHAR range doesn't push by default (§3.3). The OR fails to push as a unit. **MAY push** if `enable_string_pushdown_with_collate=true` is set AND the column has a `text_pattern_ops` index — verify with EXPLAIN, and measure equality regression (§3.3). | Same correctness rule as above — one disjunct is not pushable, so the whole OR stays. |
| `(user_id = 123 OR LOWER(email) = 'a@b.com')` — numeric `=` OR function-wrapped column | **NO — entire OR stays on Trino.** The function wraps the column on the right disjunct; that disjunct never pushes. The OR therefore never pushes. | Connector cannot push `LOWER(email) = ...` for any column. **Fix:** denormalize a generated column `email_lower` on Postgres (indexed via `LOWER(email)`), then `(user_id = 123 OR email_lower = 'a@b.com')` pushes cleanly. |

**EXPLAIN signature — OR-pushed vs OR-not-pushed:**

```
-- OR PUSHED — both disjuncts pushable:
TableScan[app_pg.users, constraint=((user_id = 123) OR (email = 'a@b.com'))]
    -- Constraint is INSIDE the TableScan. The OR is part of the JDBC SQL sent to Postgres.

-- OR NOT PUSHED — at least one disjunct unpushable:
Filter[(user_id = 123) OR (email LIKE '%bigcorp.com')]
    └── TableScan[app_pg.users]
        -- TableScan has NO constraint on user_id or email.
        -- Trino pulled the FULL table over JDBC (no WHERE clause at all),
        -- then evaluated the OR in worker memory. This is a serious anti-pattern.
```

**Performance consequence — read carefully.** When an OR fails to push, Trino does NOT push the pushable side either — because doing so would return only the rows matching the pushable disjunct and SILENTLY MISS the rows matching the un-pushable disjunct. So **the entire WHERE clause becomes a no-op as far as JDBC is concerned**, and the full table crosses the wire. **A single un-pushable disjunct in an OR is enough to convert a fast targeted query into a full-table JDBC pull.** This is the most catastrophic pushdown failure mode on the PostgreSQL connector. The fix is always to make every disjunct individually pushable — usually via denormalization (generated columns indexed on the un-pushable predicate's underlying expression).

**The two-step workaround for an un-pushable OR — `UNION ALL` of two pushable queries:**

```sql
-- Original (entire OR stays on Trino, full-table pull):
SELECT * FROM app_pg.public.users
WHERE (user_id = 123 OR email LIKE '%bigcorp.com');

-- Workaround — split into two queries, each with a pushable predicate, then UNION ALL + DISTINCT:
SELECT DISTINCT * FROM (
  SELECT * FROM app_pg.public.users WHERE user_id = 123                  -- numeric equality, pushes
  UNION ALL
  SELECT * FROM TABLE(app_pg.system.query(query =>                       -- raw SQL passthrough for LIKE '%...'
    'SELECT * FROM public.users WHERE email LIKE ''%bigcorp.com'''))
) u;
-- Each subquery pushes its WHERE individually; the UNION ALL + DISTINCT
-- replicates the OR semantics. Total bytes over JDBC drop by orders of magnitude
...
2. **"Why is my COUNT(*)/GROUP BY query still slow against Postgres even though aggregate pushdown is enabled?"** → Section 13.2 + Section 13.5A.1. Almost always because a WHERE predicate failed to push; the aggregate then has to run on Trino because it needs post-filter rows. Check `EXPLAIN` for a `ScanFilterProject` above the `TableScan` — that's the unpushed predicate. Rewrite it (denormalize, generated column, etc.) so it pushes; the aggregate will follow. **The EXPLAIN success signature is the ABSENCE of the `Aggregate` operator** — doc-quoted: *"If an aggregate function is successfully pushed down to the connector, the explain plan does not show that `Aggregate` operator."*

3. **"My cross-catalog join is pulling 50M Postgres rows even though I have a WHERE on the Postgres side — why?"** → Section 13.4 + Section 13.5A.3. Check (a) is the WHERE actually pushing — `EXPLAIN` for the `TableScan` constraint; (b) is dynamic filtering firing — `EXPLAIN ANALYZE` for `dynamicFilterSplitsProcessed > 0` on the Iceberg-side probe; (c) is the build small enough for BROADCAST — force with `SET SESSION join_distribution_type = 'BROADCAST'`. If the table is fundamentally 50M+ rows without a selective predicate, the answer is "stop federating, ingest into Iceberg" — Section 13.6.
...
7. **"Does `HAVING SUM(amount) > 1000` push to Postgres?"** → Section 13.5A.2. **YES — but only when the underlying aggregate also pushes.** HAVING pushdown is a consequence of aggregation pushdown — if the aggregate stays on Trino (because a WHERE predicate didn't push, or the aggregate function isn't one of the supported ones, or the GROUP BY uses ROLLUP/CUBE), HAVING also stays on Trino. EXPLAIN success signature: HAVING embedded inside the `TableScan` synthetic query; no separate `Filter` or `Aggregate` operator above the scan.

8. **"In a 3-way join Postgres-dim + Iceberg-fact + Iceberg-dim, where does each join execute?"** → Section 13.5A.3. **All joins execute on Trino workers** — cross-catalog joins NEVER push to a connector. Each `TableScan` runs against its own connector in parallel; the pairwise `Join` operators run on Trino workers. CBO picks BROADCAST vs PARTITIONED per join based on stats; EXPLAIN shows `RemoteExchange[REPLICATE]` for BROADCAST and `RemoteExchange[REPARTITION]` for PARTITIONED. Run `ANALYZE TABLE` on every source so the CBO has reliable estimates.

9. **"Does `WHERE (user_id = 123 OR email LIKE '%bigcorp.com')` push to Postgres?"** → Section 13.5A.4. **NO — entire OR stays on Trino.** OR-of-mixed-pushability has different semantics than AND: if **any** disjunct is un-pushable, the **entire OR** stays on Trino (because skipping any disjunct would change the result set). With a leading-wildcard LIKE as one disjunct, the OR never pushes regardless of collation. The full table crosses the JDBC wire. The fix is `UNION ALL` of two individually-pushable queries (one for the numeric equality, one via `system.query()` passthrough for the LIKE) with an outer `DISTINCT`.

### 13.8 One-line citations — verify any claim above

| Claim | Source | Doc-quoted vs empirical |
|---|---|---|
| Pushdown kinds enumerated; `Aggregate operator absent = pushed` quote ("If an aggregate function is successfully pushed down to the connector, the explain plan does not show that Aggregate operator") | trino.io/docs/current/optimizer/pushdown.html | **Doc-quoted.** |
| TopN-pushdown success signature ("The absence of the TopN Trino operator in the Fragment 0 from the query plan demonstrates that the query benefits of the Top-N pushdown optimization") + canonical PostgreSQL example | trino.io/docs/current/optimizer/pushdown.html | **Doc-quoted.** |
| PostgreSQL connector supported aggregates (16 functions including `count`, `min`, `max`, `sum`, `avg`, `variance`, `stddev`, covariance, correlation); VARCHAR equality and `IN` push by default; VARCHAR range (`>`, `<`, `BETWEEN`) does NOT push by default; `postgresql.experimental.enable-string-pushdown-with-collate` enables string range pushdown; `jdbc-types-mapped-to-varchar` foot-gun | trino.io/docs/current/connector/postgresql.html | **Doc-quoted.** |
| `topn_pushdown_enabled` / `aggregation_pushdown_enabled` session property NAMES (and default true) | These session properties are observable via `SHOW SESSION LIKE` on any Trino 467 PG catalog; the names appear in the JDBC connector implementation and in release notes; the official connector page references the pushdowns but does NOT explicitly enumerate the session-property names. | **Empirical/release-notes-derived** — the names are real and verifiable via `SHOW SESSION` on a live cluster. |
| Dynamic filtering supports `inner and right joins with =, <, <=, >, >=, IS NOT DISTINCT FROM` and `semi-joins with IN`; `enable-dynamic-filtering` default true; INNER/RIGHT support DF, LEFT/FULL OUTER do NOT | trino.io/docs/current/admin/dynamic-filtering.html | **Doc-quoted** (exact phrasing). |
| `dynamicFilterSplitsProcessed` operator-stats field as runtime DF verification ("records the number of splits processed after a dynamic filter is pushed down") | trino.io/docs/current/admin/dynamic-filtering.html | **Doc-quoted.** |
| `iceberg.dynamic-filtering.wait-timeout` catalog property, default 1s; JDBC `dynamic-filtering.wait-timeout` default 20s; Iceberg connector has NO session-property form; Hive/Delta/JDBC connectors DO have session-property forms | trino.io/docs/current/connector/iceberg.html (Iceberg side); trino.io/docs/current/connector/postgresql.html (JDBC side); supplemented by `SHOW SESSION` verification on Trino 467 | **Doc-quoted** for the catalog properties; **empirical** for the asymmetric session-property availability. |
| Aggregate-pushdown-requires-predicate-pushdown ORDERING DEPENDENCY (§13.2) | NOT explicitly stated in any official Trino doc. The trino.io optimizer/pushdown.html and connector/postgresql.html docs describe the two pushdowns as independent. The dependency is **inferred from the plan-tree rewrite-rule mechanics** (a `ScanFilterProject` between `Aggregate` and `TableScan` prevents the aggregate-pushdown rewrite rule from matching). | **Empirical / EXPLAIN-observable** — verifiable on a live Trino 467 cluster. NOT a doc-quoted rule. Use it as a mental model, not as a doc citation. |
| Top-N pushdown initial release (Trino 353, 5 Mar 2021) | trino.io/docs/current/release/release-353.html | **Doc-quoted** (release notes). |
| Top-N pushdown enabled-by-default after VARCHAR correctness fix (Trino 354, 19 Mar 2021) | trino.io/docs/current/release/release-354.html | **Doc-quoted** (release notes). |
| `postgresql.experimental.enable-string-pushdown-with-collate` flag added in Trino 365 (Dec 2021) via PR #9746 | trino.io/docs/current/release/release-365.html + github.com/trinodb/trino/pull/9746 | **Doc-quoted** (release notes + PR). |
...
| HAVING pushdown mechanism (§13.5A.2) — JDBC connectors model aggregation as a synthetic query, enabling subsequent pushdowns including HAVING | github.com/trinodb/trino/pull/6667 ("HAVING pushdown and more advanced aggregation pushdown in JDBC connectors") | **PR-quoted** — not separately enumerated on trino.io. The EXPLAIN signature (HAVING embedded inside the TableScan's synthetic query; no `Filter` above) is EXPLAIN-observable on Trino 467. |
| `count(1)` + `GROUP BY` does NOT push (use `count(*)` instead) — §13.5A.1 | github.com/trinodb/trino/issues/7251 ("count(1) aggregation is not pushed into JDBC connectors when used with GROUP BY") | **Issue-quoted.** |
| Cross-catalog joins NEVER push (§13.5A.3) — joins between two different catalogs always execute on Trino workers | trino.io/docs/current/optimizer/pushdown.html ("Join pushdown" section); trino.io/docs/current/connector/postgresql.html ("Join pushdown" section: applies only when both tables are in the same catalog) | **Doc-quoted.** |
| OR-of-mixed-pushability blocks the entire OR (§13.5A.4) — single un-pushable disjunct makes the whole OR stay on Trino | trino.io/docs/current/optimizer/pushdown.html (predicate pushdown semantics); **empirically EXPLAIN-observable** on Trino 467 — `Filter` node above bare `TableScan` for any OR with one un-pushable disjunct. | **Mixed** — pushdown semantics are doc-described; the specific OR-blocking behavior is EXPLAIN-observable on a live cluster. |
