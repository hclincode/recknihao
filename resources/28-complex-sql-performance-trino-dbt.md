...
> | Guard against an OLDER late-arriving row overwriting a NEWER target row ("only update if source is newer") | dbt's compiled merge has NO conditional matched branch (bare `WHEN MATCHED THEN UPDATE`, overwrites unconditionally). Add a target-side predicate so only strictly-newer source rows update. **Trino MERGE itself DOES support `WHEN MATCHED AND cond` — it is not Snowflake-only.** | `incremental_predicates=["DBT_INTERNAL_DEST.updated_at < DBT_INTERNAL_SOURCE.updated_at"]`. Within-batch `ROW_NUMBER()=1` dedupe is necessary but NOT sufficient (doesn't protect a prior run's row). Full canonical: r27 § late-arriving lookback + r13 § incremental_predicates. |
