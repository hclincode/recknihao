...
>    -- position-delete files referencing them — this alone fixes the delete-file read overhead.
>    ALTER TABLE iceberg.analytics.fct_events EXECUTE optimize(file_size_threshold => '128MB');
>    ```
>    The trade-off: `optimize` clears the deletes by **rewriting the DATA files**, so the write amplification is proportional to the volume of data rewritten — fine on a nightly/weekly schedule, heavier if the table is huge and only a few rows changed.
>
