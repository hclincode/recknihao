...
> **COUNT the elements in a JSON ARRAY STRING column — use `json_array_length`, do NOT explode.** **Keyword anchor (READ THIS FIRST if your question contains any of these phrases):** how many elements in a JSON array, count items in a JSON array string, size of a JSON array, length of a JSON array column, number of entries in a JSON array column, count elements in a varchar JSON array, how big is the JSON array, count of items in a JSON array field. If you only need the **count/size** of a JSON array stored as a `varchar` (text like `'[{"sku":"A"},{"sku":"B"},{"sku":"C"}]'`), you do **NOT** need to explode it with `UNNEST` — call `json_array_length(col)` directly. Per [trino.io/docs/467/functions/json.html](https://trino.io/docs/467/functions/json.html): `json_array_length(json) -> bigint` where *json* is "a string containing a JSON array" — it auto-coerces a VARCHAR JSON-array string, so the bare call works without `json_parse`. Verified: `json_array_length('[1,2,3]') = 3`.
>
> ```sql
> -- COUNT the elements in a JSON ARRAY STRING (no explode / no UNNEST needed):
> SELECT order_id, json_array_length(order_items_json) AS n_items
> FROM iceberg.analytics.orders;
> -- order_items_json = '[{"sku":"A"},{"sku":"B"},{"sku":"C"}]'  ->  3
> ```
> ❌ `cardinality(CAST(order_items_json AS ARRAY(JSON)))`  -- CAST(varchar AS ARRAY(JSON)) needs json_parse first; for a COUNT just use json_array_length(col) -- DO NOT COPY
>
> **Disambiguator — COUNT vs EXPLODE:** to just **COUNT** the elements, use `json_array_length(col)` (this card). To **EXPLODE** the array into one row per element, use `UNNEST(CAST(json_parse(col) AS ARRAY(...)))` — the JSON-ARRAY-STRING explode card immediately above (form B). If the array is nested under a key (`{"items":[...]}`), extract it first: `json_array_length(json_extract(col,'$.items'))` (see `resources/13-postgres-to-iceberg-ingestion.md` § JSON extraction family for the bare-array-vs-nested disambiguator).

> **Sub-note — `WITH ORDINALITY` for the ORIGINAL array position (1-based bigint, appended LAST in the alias list).**
> **DISAMBIGUATOR — READ THIS FIRST, there are TWO different questions:**
> - **(A) "WHERE is a SPECIFIC KNOWN VALUE in the array?"** (e.g. "what position is the tag `'onboarding'` at — if it's the 3rd element I get 3", "funnel-step number of `'checkout'`", "1-based index of value X", "where in the array is X") → that is **`array_position(arr, value)`** — returns ONE position per input row (3 if it's 3rd, **0 if absent**), does NOT explode rows. **This is NOT this card — go to [§1a.3](#1a3-trino-array-function-quick-reference--contains--cardinality--array_distinct--element_at--array_join--array_position-do-not-claim-trino-lacks-a-contains).**
> - **(B) "Label EVERY element with its 1-based ordinal WHILE I EXPLODE the whole array into rows"** (per-element ordinal across all elements) → THIS card, `UNNEST(arr) WITH ORDINALITY`.
>
> **Keyword anchor for case (B) — explode-EVERY-element (READ THIS FIRST if your question contains any of these phrases — route HERE, NOT to `array_position`, ONLY when you are exploding the WHOLE array into rows):** UNNEST with ordinality Trino, array element index/position while exploding, original array offset, explode array keep index, explode array keep position, position in array per element, ordinal per unnested row, index of each element as I explode, which position was each tag in while exploding, step number in the sequence per row, first tag vs second tag, row number within an exploded array, position of each item while expanding a list, per-element ordinal, 1-based position of each tag while unnesting, get the index alongside each element, give me the position of every tag as I explode. Append `WITH ORDINALITY AS t(elem, ord)` to either UNNEST form to recover the 1-based ORIGINAL array offset of each element. Per [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html), `WITH ORDINALITY` appends an **additional bigint ordinality column** at the **END** of the unnested columns — element first, ordinality last. The `AS t(elem, ord)` clause names BOTH columns (alias for the unnested element + alias for the ordinality column). **Canonical worked example (copy this — names the position column `position`, matches the common SaaS phrasing "position of each tag"):**
> ```sql
> -- Per-element ordinal alongside each exploded element — TRUE 1-based per-row position.
> -- WITH ORDINALITY appends a bigint ordinality column (1, 2, 3, ...) AT THE END;
> -- AS t(tag, position) names both: tag = the unnested element, position = the bigint ordinal.
> SELECT order_id, tag, position
> FROM iceberg.analytics.orders
> CROSS JOIN UNNEST(tags) WITH ORDINALITY AS t(tag, position);
> -- For tags = ARRAY['billing','export','billing'] in order_id=42, you get:
> --   (42, 'billing', 1)
> --   (42, 'export',  2)
> --   (42, 'billing', 3)   <-- duplicate 'billing' correctly gets position=3, NOT 1.
> ```
> Also works on the LEFT form when you must preserve parents with NULL/empty arrays: `LEFT JOIN UNNEST(arr) WITH ORDINALITY AS t(elem, ord) ON TRUE` (the ordinality is NULL for the padded NULL-array row). Older naming form `AS t(tag, idx)` is equivalent — `idx` and `position` are just alias choices for the same bigint ordinality column.
>
> **WHY `array_position` IS THE WRONG TOOL — but ONLY for "per-element ordinal WHILE exploding EVERY element"** (cross-ref to §1a.3): when you are UNNESTing the WHOLE array and want each exploded row's TRUE per-row ordinal, `array_position(tags, tag)` fails because it returns the **1-based index of the FIRST occurrence** of a **VALUE** (verified at [trino.io/docs/467/functions/array.html](https://trino.io/docs/467/functions/array.html)) — so for `tags = ARRAY['billing','export','billing']`, **both** `'billing'` rows would get `array_position = 1` (the duplicates collide on the first match). In THAT case `WITH ORDINALITY` is the only correct shape. **SCOPE THIS NARROWLY:** if instead you just want **the position of ONE specific known value** (e.g. "what position is `'onboarding'` at", one row per input row), `array_position(tags, 'onboarding')` IS the correct, canonical tool — see §1a.3. **DO NOT WRITE:** `array_position(tags, tag)` (the UNNESTed element variable) in the SELECT list of an UNNEST query to mean "position of this tag in the array" — it returns the first-match index, not the per-row ordinal; duplicates collapse. Use `WITH ORDINALITY` and project the ordinality column directly.
>
> **DO NOT WRITE — comma + `CROSS JOIN` combined is a parse error** (this is grammar, not semantics — verified at [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html)). The FROM clause accepts EITHER comma-separated relations (implicit cross join) OR explicit `CROSS JOIN`, **never both at once**:
>
> | Form | Verdict |
> |---|---|
> | `FROM orders CROSS JOIN UNNEST(tags) WITH ORDINALITY AS t(tag, position)` | ✅ CORRECT — keyword, no comma. **This is the canonical form — copy this.** |
> | `FROM orders, UNNEST(tags) WITH ORDINALITY AS t(tag, position)` | ✅ CORRECT — comma = implicit cross join, no `CROSS JOIN` keyword. |
> | `FROM orders, CROSS JOIN UNNEST(tags) WITH ORDINALITY AS t(tag, position)` | ❌ **WRONG — PARSE ERROR. DO NOT COPY.** Comma followed by `CROSS JOIN` keyword combines two mutually exclusive grammar paths; Trino raises a syntax error (`mismatched input 'CROSS'`) before the query runs. Pick ONE form. |
>
> **DO NOT WRITE:** "Trino has no `WITH ORDINALITY` — it is Postgres-only" (FALSE — Trino 467 supports it, verified at [trino.io/docs/current/sql/select.html](https://trino.io/docs/current/sql/select.html)); using `ROW_NUMBER() OVER (ORDER BY ...)` to recover the original array index — `ROW_NUMBER` assigns a NEW ordering per its own `ORDER BY`, NOT the array offset; using `array_position(arr, elem)` — passing the UNNESTed element variable — in the SELECT list of an UNNEST to label EVERY exploded row with its position — that returns the FIRST-MATCH index of a VALUE, NOT the per-row ordinal (duplicates collide). Use `WITH ORDINALITY` to get the original position. (This caveat applies ONLY to the explode-every-element case; for "position of ONE specific known value, one row per input row" `array_position(arr, 'literal')` is correct — see §1a.3.)
...
### 1a.3 Trino array-function quick reference — `contains` / `contains_sequence` / `cardinality` / `array_distinct` / `element_at` / `array_join` / `array_position` (DO NOT claim Trino lacks a `contains`)

**Keyword anchor:** Trino array contains, does array contain element, does array contain value, array membership test Trino, cardinality array length Trino, array_distinct dedup, array_intersect array_union array_except set operations on arrays, element_at array negative index, check if array has value, array has element, array membership without UNNEST, join array to string Trino, concat array elements Trino, array_join delimiter, array_position find element index, position of element in array Trino, where in array is value, what position is this tag/value at, where in the array is X, 1-based index of a value in an array, position of a specific value per row, funnel step number from a step-name array, what position does a specific tag appear at within each row's array, if it's the 3rd element I get back 3, two steps back-to-back, did the user go directly from step A to step B, consecutive steps in an array, step A immediately followed by step B, did event X happen right before event Y, contiguous subsequence in an array, in that exact consecutive order, contains_sequence Trino, ordered pair of steps with nothing in between, A then B with no step in between.
...
| `contains_sequence` | `contains_sequence(x, seq) -> boolean` | **Does array `x` contain array `seq` as a CONSECUTIVE (back-to-back, contiguous) subsequence — all values in the same consecutive order?** THE canonical tool for "did the user go **DIRECTLY** from step A to step B (no step in between)" / "do these events appear consecutively in this exact order". **COPY THIS:** `WHERE contains_sequence(funnel_steps, ARRAY['add_to_cart','checkout'])` → TRUE only if `'add_to_cart'` is **immediately** followed by `'checkout'` somewhere in the array, and it still works correctly when steps **REPEAT**. Verified at [trino.io/docs/467/functions/array.html](https://trino.io/docs/467/functions/array.html): "Return true if array `x` contains all of array `seq` as a subsequence (all values in the same **consecutive order**)." **Do NOT hand-roll adjacency with `array_position(arr,'A') + 1 = array_position(arr,'B')`** — `array_position` returns only the FIRST occurrence of each value, so for `ARRAY['add_to_cart','view','add_to_cart','checkout']` (a real 3→4 adjacency) that arithmetic returns a **FALSE NEGATIVE** (`1+1=2 ≠ 4`); it is correct ONLY when every step value is unique in the array. Use `contains_sequence`. (For "do these values appear in order but NOT necessarily adjacent", that is a different, non-built-in question — `contains_sequence` is specifically the CONTIGUOUS test.) |
...
| `array_intersect` / `array_union` / `array_except` | `array_intersect(a, b)`, `array_union(a, b)`, `array_except(a, b)` | Set operations on two ARRAY VALUES **within a single row**; result is deduped. **NOT for comparing two QUERIES / result sets** — for "customer IDs in BOTH queries" or "rows in query A not in query B" you want the ROW-level operators `INTERSECT` / `EXCEPT` / `UNION`, see [resource 23 § 3.1F row-level set operators](23-sql-best-practices-olap.md#31f-row-level-set-operators--intersect-rows-in-both--except-rows-in-a-not-b--union-vs-union-all). Rule: two array columns in one row → `array_*`; two queries → bare `INTERSECT`/`EXCEPT`/`UNION`. |
...
| `array_position` | `array_position(x, element) -> bigint` | **Returns the 1-based index of the FIRST occurrence of a VALUE in array `x`, or `0` if not found** (verified at [trino.io/docs/current/functions/array.html](https://trino.io/docs/current/functions/array.html)). **✅ THIS IS THE CANONICAL, CORRECT TOOL for "what position is a given VALUE at in the array — one row per input row"** (e.g. "what position does the tag `'onboarding'` appear at within each row's `tags` array — if it's the 3rd element I get back 3"): `array_position(tags, 'onboarding')` → `3` if onboarding is 3rd, `0` if absent. **COPY THIS:** `SELECT row_id, array_position(tags, 'onboarding') AS pos FROM iceberg.analytics.events` — ONE row per input row, no explosion, absent-tag rows kept (with `pos = 0`). Also: funnel-step number from a step-name array (`array_position(funnel_steps, 'checkout')`). Note: 0 means "not found", NOT NULL. **Disambiguator — position-of-ONE-value vs A-DIRECTLY-FOLLOWED-BY-B:** `array_position` answers "what position is value X at"; for "did step A go **DIRECTLY** to step B / two steps **back-to-back** / consecutive order with nothing in between", use **`contains_sequence(arr, ARRAY['A','B'])`** (the `contains_sequence` row in this same §1a.3 table), NOT position arithmetic — `array_position(arr,'A') + 1 = array_position(arr,'B')` is a FALSE-NEGATIVE trap when steps repeat (first-occurrence only). **CAUTION — duplicates collide:** for `arr = ARRAY['billing','export','billing']`, `array_position(arr, 'billing')` returns `1` for **both** `'billing'` occurrences — it always reports the FIRST match. **WRONG ❌ ONLY for per-element ordinal WHILE UNNESTing EVERY element:** `array_position(tags, tag)` (passing the UNNESTed element variable) in the SELECT list of `... CROSS JOIN UNNEST(tags) AS t(tag)` does **NOT** give each row's TRUE per-row position — duplicate tag values all collapse to the first-match index. For the position of EACH element WHILE exploding the WHOLE array, use **`UNNEST(arr) WITH ORDINALITY AS t(elem, position)`** — cross-ref [§1a above (WITH ORDINALITY sub-note)](#1a-exploding-an-array-column-to-one-row-per-element-unnest--array-to-rows--left-join-unnest) for that case. Inline marker for the explode-every-element case ONLY: `array_position(tags, tag)  -- ❌ WRONG for per-element ordinal while UNNESTing: returns FIRST-match index, duplicates collide — use UNNEST ... WITH ORDINALITY — DO NOT COPY`. |

**WHEN TO USE WHICH:**
- Membership ("does the array hold X?"): `contains(arr, X)`.
- Two steps BACK-TO-BACK / "did A go DIRECTLY to B" / consecutive subsequence in this exact order: `contains_sequence(arr, ARRAY['A','B'])` — NOT `array_position` arithmetic (first-occurrence only → false negative when steps repeat).
...
>
> ⚠️ **Literal `_`/`%` in the prefix?** Inside a filter lambda, `f LIKE 'beta_%'` is **WRONG** for a *literal* `beta_` prefix — in LIKE, `_` matches **ANY single char** (so it ALSO keeps `'betaX9'`, `'betamax'`, …), and `%` matches any run. To filter an array for elements that **start with the literal text `beta_`**, use **`filter(enabled_features, f -> starts_with(f, 'beta_'))`** (literal, no wildcard) or escape the underscore: **`filter(enabled_features, f -> f LIKE 'beta\_%' ESCAPE '\')`**. (Same rule as the bare-column prefix caveat — see r23 §3.1 "starts_with / prefix" Net-rule ⚠️.) *Keyword anchors: filter array for prefix, elements that start with beta_, feature flags starting with beta_, filter lambda LIKE underscore, array elements starting with literal underscore.*
...
> **For a WEIGHTED average / weighted mean (average with weights — e.g. score × number_of_reviews, price × quantity, grade × credit_hours), `AVG(x)` is the WRONG tool** — `AVG` is the un-weighted mean and ignores any weight column. Compute `SUM(value * weight) * 1.0 / NULLIF(SUM(weight), 0)` instead, and force the `1.0` (or a CAST to double/decimal) so the mean does not silently TRUNCATE when value & weight are integers (`17 / 4 -> 4`, not `4.25`). See [resource 23 §3.1B-WA — weighted average / force non-integer division](23-sql-best-practices-olap.md#31b-wa-weighted-average--sumvalue--weight--sumweight--force-non-integer-division-or-the-mean-truncates).
...
**`WITH RECURSIVE` IS supported** for bounded hierarchy walks (org tree, parts explosion) — see [resource 27 § 7A.1](27-oracle-plsql-to-dbt-trino.md) for the Oracle `CONNECT BY` → `WITH RECURSIVE` migration with the depth-cap caveat (`max_recursion_depth` session property; only single-element recursive cycles supported). **Quick fact (so the number is reachable from this CTE landing too):** `max_recursion_depth` **defaults to 10** (a small integer — NOT 100, NOT 1000) per [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html). A hierarchy deeper than 10 levels **raises an error** — `NOT_SUPPORTED: Recursion depth limit exceeded (10). Use 'max_recursion_depth'` — it does NOT silently truncate. To walk a 15-/20-/50-level-deep tree, `SET SESSION max_recursion_depth = <N>` BEFORE running the recursive CTE (a `WHERE depth < N` predicate inside the recursive term does NOT bypass the engine cap — the predicate must be `<=` the cap, or raise the cap first). See [resource 27 § 7A.1 caveat #2](27-oracle-plsql-to-dbt-trino.md) for the worked SET SESSION + WITH RECURSIVE shape. **DO NOT WRITE** "`max_recursion_depth` defaults to 1000" / "default 100" — wrong; the real default is **10**.
...
> **MIRROR INOCULATION — HLL and the ~2.3% figure are `approx_distinct` ONLY, NOT `approx_percentile` (iter697 PIN — FIX-A mirror).** The **HyperLogLog (HLL)** algorithm and the **~2.3% / 0.023 default standard error** quoted here both belong specifically to **`approx_distinct(x)`** — they solve the approximate **COUNT DISTINCT** (cardinality) problem. **DO NOT WRITE** that `approx_percentile` uses HLL, that it is "HyperLogLog-style", or that it has "~2.3% error" — those are all the iter696 Q4 conflation. `approx_percentile(x, p)` uses a **quantile-digest (qdigest) / T-digest** sketch (a sketch designed for the QUANTILE problem, not for cardinality) and Trino's docs do not publish a single fixed-percent error for that overload. See the **SKETCH-ALGORITHM INOCULATION** at [§ Pattern C2 below](#pattern-c2-percent_rank--what-percentile-is-this-tenant-in) for the full side-by-side table + DO-NOT-WRITE list, and [resource 23 §3 signature family at r23:267-277](23-sql-best-practices-olap.md#3-use-approximate-functions-when-exactness-isnt-required) for the four-overload `approx_percentile` signature card + the explicit "do NOT confuse with approx_distinct's 2.3% figure" lock.
...
**The one-fact summary — Trino does NOT support `DISTINCT` inside a window function.** `COUNT(DISTINCT user_id) OVER (ORDER BY occurred_date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW)   -- ❌ WRONG: analysis error in Trino 467 (#7885) — DO NOT COPY` is **NOT supported** in Trino (the same limitation exists in Redshift, SQL Server, and most engines — tracked at [trinodb/trino #7885](https://github.com/trinodb/trino/issues/7885)). For a **rolling N-day distinct user count per day**, reach for one of these two docs-verified patterns:
...
> -- nearest 10 min -> /600*600 ; nearest 15 min -> /900*900 ; nearest 30 min -> /1800*1800
> ```
>
> **Worked example — NEAREST 10 minutes (10 min = 600 s) — the dashboard-bucketing shape (`10:04 → 10:00`, `10:06 → 10:10`):**
> ```sql
> -- ✅ Snap dashboard event timestamps to the NEAREST 10-minute mark (rounds UP and DOWN):
> from_unixtime(round(to_unixtime(event_ts) / 600) * 600)
> --   10:04:00 -> 10:00  (4 min is closer to :00 — rounds DOWN)
> --   10:06:00 -> 10:10  (6 min is closer to :10 — rounds UP)
> --   10:05:00 -> 10:10  (exactly half — round() is half-up, so it goes to :10)
> ```
...
> ```
> FLOOR always rounds DOWN to the bucket start; NEAREST snaps to whichever mark is closer (`10:02:30 → 10:05`, `10:01:00 → 10:00`); CEILING always pushes UP to the next mark. Change one number for other sizes: `300` (5 min) → `600` (10 min) → `900` (15 min) → `1800` (30 min) → `3600` (hourly). All three return `timestamp(3) with time zone`; wrap in `CAST(... AS TIMESTAMP)` if your column is naive (see r13:5671).
...
> ⚠️ **Asked to ROUND / SNAP events to the NEAREST N-minute mark (closest mark — rounds UP *and* DOWN, e.g. `10:06 → 10:10`, `10:04 → 10:00`)? That is a DIFFERENT idiom: use the round-to-NEAREST-N-minutes canonical `from_unixtime(round(to_unixtime(ts) / N) * N)` (nearest 10 min = `/600*600`) — see the NEAREST card immediately ABOVE this one. THIS card FLOORS to the bucket START (tumbling window — ALWAYS rounds DOWN, e.g. `10:06 → 10:00`).** Pick by direction: *nearest/closest/snap/round* → the NEAREST card above; *bucket start / floor / chop down / tumbling window* → this card.

> **READ THIS FIRST if your question contains any of these keywords:** `bucket events into 15-minute windows`, `bucket clicks into 15-minute time windows`, `bucket events every N minutes (from a fixed origin)`, `tumbling window`, `tumbling time window Trino`, `N-minute time bucket`, `time-series chart bucket (bucket START)`, `group by 15 minutes`, `hourly bucket`, `hourly tumbling window`, `5-minute rollup`, `10-minute rollup`, `15-minute rollup`, `round timestamp DOWN to 15 minutes`, `floor timestamp to interval`, `floor timestamp to 15 minutes`, `floor timestamp to N minutes`, `truncate timestamp to N minutes`, `chop timestamp down to the bucket start`, `is there date_bin in Trino`, `does Trino have date_bin`, `does Trino have time_bucket`, `time_bucket Trino`, `time_bucket function Trino`, `Postgres date_bin in Trino`, `TimescaleDB time_bucket in Trino`, `Snowflake time_slice in Trino`, `BigQuery TIMESTAMP_TRUNC interval Trino`, `Spark window function Trino`, `epoch floor time bucket`, `unix timestamp modulo bucket`, `mod 900 timestamp bucket`, `% 900 bucket Trino`. **This card FLOORS (always rounds DOWN to the bucket start). For NEAREST / closest / snap / round (rounds up AND down), use the NEAREST card immediately above — not this one.** **Verified against [trino.io/docs/467/functions/datetime.html](https://trino.io/docs/467/functions/datetime.html) on 2026-06-09.**

> **DO NOT reach for `date_trunc('minute', ...)` to get a NEAREST 10/15/30-min mark (iter817 Q1 defect — each on its own line):**
> ```sql
> -- ❌ date_trunc('minute', ts) -- rounds DOWN to the minute only; does NOT give the NEAREST 10/15/30-min mark — DO NOT COPY
> -- ❌ date_trunc('minute', ts) + add-5-min-then-truncate-to-10-min hack -- brittle and wrong; use from_unixtime(round(to_unixtime(ts)/600)*600) — DO NOT COPY
> ```
> For NEAREST-N-min snapping use the round-to-NEAREST canonical `from_unixtime(round(to_unixtime(ts) / 600) * 600)` (nearest 10 min) in the NEAREST card above. THIS card only FLOORS.
...
> **Cross-references.** **The iter606 LEADING canonical for N-minute buckets (FLOOR-TO-HOUR + ADD-N-STEPS form) is the block immediately ABOVE this one** — that block also carries the original `::`-cast inoculation at line 1487 and the worked GROUP BY example. The TWO blocks are companions: this iter715 block adds the **EPOCH-FLOOR LEAD** and the **DATE_TRUNC + INTERVAL-MOD ALT**, plus the **date_bin / time_bucket-not-Trino pre-empt** that the iter606 block doesn't carry. **Use whichever form reads cleanest for your question — all three produce identical bucket boundaries.** Inverse-direction / epoch-to-timestamp + the seconds-vs-milliseconds pitfall: [resource 13 §`from_unixtime` SECONDS vs MILLISECONDS](13-postgres-to-iceberg-ingestion.md) line 5686 (the year-56378 trap if you pass raw epoch-ms straight into `from_unixtime`). Postgres `EXTRACT(EPOCH FROM ts)` → Trino `to_unixtime(ts)` (the canonical for converting timestamps to epoch seconds): [resource 23 § LEADING CANONICAL — Postgres `EXTRACT(EPOCH FROM ts)` → Trino `to_unixtime(ts)`](23-sql-best-practices-olap.md) line 1428. **Round to the NEAREST N MINUTES (snap to the CLOSEST sub-hour mark — rounds UP and DOWN, e.g. `10:06 → 10:10`, NOT floor): the LEADING CANONICAL `from_unixtime(round(to_unixtime(ts) / N) * N)` (nearest 10 min = `/600*600`, nearest 5 min = `/300*300`, nearest 15 min = `/900*900`, nearest 30 min = `/1800*1800`) — the NEAREST-N-min card immediately ABOVE this one (the `round()` snap card).** NEAREST-hour rounding (round-half-up, NOT floor): the iter631 LEADING canonical at line 1440 above. Postgres `::` cast inoculation full translation table: [resource 23 §3.1C](23-sql-best-practices-olap.md) line 678. Oracle-migration `::` ban: [resource 27 §4.4A](27-oracle-plsql-to-dbt-trino.md) line 1149.
...
>
> 1. **`format_datetime` accepts a bare `DATE` directly — no CAST is required (and it does NOT error).** The docs signature is typed `timestamp`, but Trino implicitly coerces `DATE -> TIMESTAMP(0)` at function-argument resolution, so `format_datetime(order_date, 'EEEE')` works WITHOUT a CAST and does NOT throw a type error. (Verified against the Trino git-tag 467 source `io/trino/type/TypeCoercion.java` `coerceTypeBase`: `case StandardTypes.DATE -> switch (resultTypeBase) { case StandardTypes.TIMESTAMP -> Optional.of(createTimestampType(0)); ... }` — this path flows through `canCoerce` / `compatibility`, which is exactly what implicit argument coercion uses.) The worked examples below keep an explicit `CAST(order_date AS timestamp)` — that is **harmless and equivalent**, write it if you prefer the clarity, but `format_datetime(order_date, 'EEEE')` is equally valid. See [resource 23 § format-vs-format_datetime block](23-sql-best-practices-olap.md).
> ```sql
...
>
> **Is `now()` valid in Trino? YES — `now()` IS a real Trino function (iter707 sub-clarifier).** *(Keyword anchors: "is now() valid in Trino" · "now() vs current_timestamp" · "does now() work in Trino" · "now() Trino alias" · "is now() real Trino" · "now() or current_timestamp" · "is now() Postgres-only" · "is now() foreign dialect".)* **THREE valid ways to get "now" in Trino 467:** (1) bare **`current_timestamp`** (no parens), (2) **`current_timestamp(p)`** with a precision argument, (3) **`now()`** — `now()` IS a valid Trino function (the parenthesized alias of `current_timestamp`, documented at [trino.io/docs/467/functions/datetime.html](https://trino.io/docs/467/functions/datetime.html) as *"`now() → timestamp(3) with time zone` — This is an alias for `current_timestamp`"*; returns `timestamp(3) with time zone`); it is **NOT** a foreign-dialect form. **`now()` and `current_timestamp` are equivalent in Trino** — pick whichever reads better; both yield the same value and type. Only the **EMPTY-PARENS niladic forms** `current_timestamp()` / `current_date()` / `current_time()` parse-error; **`now()` (the dedicated parenthesized alias) does NOT — it is fine.** The actual foreign "now" forms to avoid in Trino are **`GETDATE()` [SQL Server]** and **`SYSDATE` [Oracle]** (neither appears in the Trino 467 datetime-functions docs) — but `now()` itself is **fine**. Common misconception to defang: *"`now()` is Postgres syntax / won't parse in Trino"* → **FALSE.** `now()` is documented Trino; the Postgres-only form was always `current_timestamp()` with empty parens, never `now()`.
>
> **Quick recall: when you need "now" in a Trino UPDATE / INSERT / DELETE / WHERE / SET / DEFAULT clause, write either `current_timestamp` (no parens) or `now()`. Never `current_timestamp()`. `now()` is fine — it's the documented parenthesized alias.**

> **READ-ME-FIRST — Trino default NULL ordering in `ORDER BY` is `NULLS LAST` REGARDLESS of `ASC` or `DESC` (iter708 PIN — FIX-A — DIRECTION-INDEPENDENT, not direction-dependent).** *(Keyword anchors so the responder lands here on every shape: "NULLS LAST default Trino" · "NULLS FIRST Trino" · "push NULLs to bottom Trino" · "push NULLs to the bottom" · "NULLs sorting to the top" · "NULLs at the top on DESC" · "where do NULLs sort in Trino" · "null ordering in ORDER BY" · "ORDER BY NULL default" · "ORDER BY score DESC NULLS LAST" · "leaderboard NULL score sort" · "never logged in NULL sort" · "NULLs sort first or last in Trino" · "Trino NULL default direction" · "does DESC put NULLs first" · "is NULL treated as largest in Trino")*
>
> **The one fact you must keep straight (verified verbatim at [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html)):** *"The default null ordering is `NULLS LAST`, regardless of the ordering direction."* That means **BOTH** `ORDER BY score` AND `ORDER BY score DESC` put NULLs at the **END** of the result by default — `DESC` does **NOT** flip the default to `NULLS FIRST`. The default is **direction-INDEPENDENT**.
>
> **What to write — always be explicit, never rely on the engine default.** Spell out `NULLS FIRST` or `NULLS LAST` in every `ORDER BY` that touches a nullable column. Explicit ordering is self-documenting and grep-able for review.
>
> | What you want | Write this | Why |
> |---|---|---|
> | Highest scores first, NULL (never-logged-in) users at the **bottom** of a leaderboard | `ORDER BY score DESC NULLS LAST` | Matches Trino's default but spells it out so a reviewer doesn't have to remember the rule. The copy-attractive form. |
> | Highest scores first, NULL (unscored) users at the **top** of the page | `ORDER BY score DESC NULLS FIRST` | Must be explicit — Trino's default would put NULLs at the bottom on `DESC`. |
> | Lowest scores first, NULL users at the **bottom** | `ORDER BY score ASC NULLS LAST` | Matches Trino's default; explicit for grep-ability. |
> | Lowest scores first, NULL users at the **top** | `ORDER BY score ASC NULLS FIRST` | Must be explicit — Trino's default puts NULLs at the bottom even on `ASC`. |
>
> **GOTCHA — Trino's NULL-ordering default DIFFERS from Oracle / PostgreSQL / SQL Server.** *Do not assume cross-engine NULL-ordering behavior:* Oracle and PostgreSQL both default `NULLS LAST` for `ASC` but `NULLS FIRST` for `DESC` (they treat NULL as **larger than any non-null value**, so NULL bubbles to wherever "largest" lives in the chosen direction). **Trino does NOT** — Trino picks `NULLS LAST` in BOTH directions as a fixed engine design choice. The full cross-engine matrix (Trino vs Oracle vs Postgres vs SQL Server vs Snowflake) and the docs links are in [resource 27 § LEADING CANONICAL — Oracle vs Trino NULLS-default semantics in ORDER BY](27-oracle-plsql-to-dbt-trino.md). ANSI SQL leaves the NULLS default **implementation-defined** — there is no "standard SQL default" to fall back on; cite the Trino docs directly.
>
> **OBSERVED-vs-DEFAULT diagnostic — if you SEE NULLs sorting "wrong":** if a user reports observing NULLs at the **top** of a `DESC` sort on Trino (the Oracle/Postgres muscle-memory expectation), that is **NOT** the Trino engine default firing — the engine default is `NULLS LAST` on `DESC`, which puts NULLs at the **bottom**. Suspect, in this order: (1) an **explicit `NULLS FIRST`** somewhere in the SQL the user is looking at (grep the actual query, including any view definitions and dbt model SQL); (2) a **BI-tool / Tableau / Looker / Superset re-sort** applied client-side after Trino returned the rows (BI tools often have their own null-ordering convention and re-sort the result); (3) a **downstream `ORDER BY` in a wrapping query** (e.g., `SELECT * FROM (... ORDER BY score DESC) ORDER BY score DESC NULLS FIRST`); (4) a **session or connector property** the user's session set (uncommon — Trino has no `nulls_default` session property, but custom JDBC clients sometimes inject one). If none of those, the user is probably looking at the **wrong query** or a **stale cached result** — re-run the exact SQL with `EXPLAIN ANALYZE` and a small `LIMIT` to confirm the engine's actual output. The engine itself will never spontaneously emit NULLs at the top of a bare `DESC` sort on Trino 467.
>
> **DO NOT WRITE (these are the exact wrong claims to defang):**
> - *"Trino defaults `NULLS FIRST` for `DESC` (like Oracle / Postgres)."* — **FALSE.** That is Oracle's / Postgres's rule, not Trino's. Trino defaults `NULLS LAST` for `DESC`.
> - *"For descending sorts NULLs still sort to the end unless you explicitly handle them with `NULLS LAST`."* — **MISLEADING / circular.** Trino's default ALREADY puts NULLs at the end on `DESC` — you don't have to "handle them" to get that behavior; you'd only write `NULLS LAST` for self-documentation, or `NULLS FIRST` to override the default. (The original wording suggests `NULLS LAST` is required to get NULLs at the end on `DESC` — it is not.)
> - *"Trino defaults to `NULLS LAST` for `ASC` and `NULLS FIRST` for `DESC`."* — **FALSE.** That is Oracle's direction-dependent rule projected onto Trino. Trino is direction-INDEPENDENT.
> - *"Trino follows the SQL standard's NULLS default."* — **FALSE.** ANSI SQL leaves the default implementation-defined; Trino's `NULLS LAST regardless of direction` is Trino's own design choice, not "the standard."
> - *"NULL is treated as the largest value in Trino, so it sorts last on `ASC` and first on `DESC`."* — **FALSE.** That is the Oracle/Postgres mental model. Trino's model is simpler: NULLs go to the END regardless of direction.
>
> **CORRECT copy-attractive one-liners (use these verbatim):**
> - *"Trino's default NULL ordering is `NULLS LAST` **regardless of `ASC` or `DESC`** (per [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html) verbatim). So `ORDER BY score DESC` puts NULLs at the BOTTOM by default."*
> - *"To push NULLs to the bottom of a `DESC` leaderboard, the engine already does that by default — but always write `ORDER BY score DESC NULLS LAST` explicitly for self-documentation and grep-ability."*
> - *"To put NULLs at the TOP of a `DESC` sort, you MUST write `ORDER BY score DESC NULLS FIRST` explicitly — this is the override, not the default."*
>
> **Window-function ORDER BY follows the same rule.** Every `ROW_NUMBER() / RANK() / LAG() / LEAD() / FIRST_VALUE() / LAST_VALUE() / NTILE() OVER (... ORDER BY ts DESC ...)` clause defaults to `NULLS LAST` on the partition. If you need NULL-first window-ordering semantics (e.g., the Oracle "latest per group" pattern that put NULLs first), spell it out: `OVER (PARTITION BY user_id ORDER BY ts DESC NULLS FIRST)`. The C3 NTILE NULL-handling note in [§ Pattern C3 below](#pattern-c3-ntile--bucket-rows-into-equal-size-groups-quartiles-deciles-percentile-buckets) and the migration treatment in [resource 27 § LEADING CANONICAL — Oracle vs Trino NULLS-default semantics in ORDER BY](27-oracle-plsql-to-dbt-trino.md) both follow this same direction-independent rule.
>
> **Quick recall: Trino default null ordering = `NULLS LAST`, regardless of direction. `ORDER BY x DESC` puts NULLs at the END. To put NULLs first, write `NULLS FIRST` explicitly. Always spell out `NULLS FIRST` / `NULLS LAST` for deterministic, self-documenting ordering. Cross-engine assumption is the trap — Oracle / Postgres differ from Trino on `DESC`.**
...
**Fact 3 — `GROUP BY` daily activity in a USER'S LOCAL timezone when events are stored as UTC.** *(iter581 PIN — landing point for "events store UTC, group daily by user's local timezone US/Eastern / convert UTC to local date before bucketing / chart in customer's timezone".)*
>
> **⚠️ STEP 0 — check the COLUMN TYPE first (`DESCRIBE <table>`); the correct form depends on it:**
> - **`timestamp(p) WITH TIME ZONE`** (the canonical Iceberg instant type — value is already a UTC instant) → bare **`event_ts AT TIME ZONE 'America/New_York'`** CONVERTS it. ✅ Use Fact 3 below.
> - **bare `timestamp(p)` (NO time zone) holding UTC-intent values** → bare `event_ts AT TIME ZONE 'Asia/Tokyo'` is **WRONG / session-trap**: on a tz-naive column `AT TIME ZONE 'zone'` *attaches* the zone to the existing wall-clock (does NOT shift hours), so you get the UTC wall-clock mislabeled as Tokyo — off by the full offset. You **MUST attach UTC first**: **`with_timezone(event_ts, 'UTC') AT TIME ZONE 'Asia/Tokyo'`** (or the two-step `event_ts AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo'`). See the "CAST-trap companion" + Gotcha blocks below.
> - **per-row timezone held in a COLUMN** (each user a different zone) → `at_timezone(with_timezone(event_ts,'UTC'), user_tz)` — see Fact 3b below.
>
> The canonical Trino 467 idiom **(for a `timestamp WITH TIME ZONE` column)** is **`date_trunc('day', event_ts AT TIME ZONE 'America/New_York')`** — applied uniformly in both the SELECT list and the GROUP BY. The `AT TIME ZONE 'America/New_York'` re-renders each UTC instant in NYC wall-clock; `date_trunc('day', ...)` then floors to the start of that NYC wall-clock day. Both pieces are verified at [trino.io/docs/current/functions/datetime.html](https://trino.io/docs/current/functions/datetime.html): the operator example *"`SELECT timestamp '2012-10-31 01:00 UTC' AT TIME ZONE 'America/Los_Angeles'; -- 2012-10-30 18:00:00.000 America/Los_Angeles`"* (same instant, different wall-clock label) and `date_trunc(unit, x) → [same as input]` (so the result of date_trunc on a `timestamp(p) with time zone` is itself `timestamp(p) with time zone` floored to the unit boundary in the local wall-clock). Worked example — daily signup count in US/Eastern over a UTC-stored events table:
...
**CAST-trap companion — DO NOT `CAST(naive_timestamp AS TIMESTAMP WITH TIME ZONE)` to "make a bare timestamp UTC".** *(iter686 PIN — dual-destination cross-ref companion to the federation file. Keyword anchors: CAST timestamp to timestamp with time zone, make a plain timestamp UTC, bare timestamp to local timezone, plain UTC timestamp bucketing by local day, session time zone CAST timestamptz, CAST naive timestamp UTC, CAST to make it UTC.)* When you have a bare `TIMESTAMP(6)` column whose values are KNOWN to hold UTC and you want to bucket by local day, the seemingly natural step `CAST(event_ts AS TIMESTAMP WITH TIME ZONE) AT TIME ZONE 'America/New_York'` is **SESSION-DEPENDENT and silently wrong on most clusters**. Per the Trino 467 type-cast semantics ([SET TIME ZONE](https://trino.io/docs/current/sql/set-time-zone.html) docs + [datetime functions](https://trino.io/docs/current/functions/datetime.html) — the `with_timezone` vs CAST distinction): `CAST(timestamp → timestamp with time zone)` attaches **whatever the current session time zone is** (whatever `SET TIME ZONE` set, or the JDBC client connection default, or the JVM `-Duser.timezone` on the coordinator if no other override) — it is **NOT unconditionally UTC**. If the session zone is, say, `America/Los_Angeles` (PT), the bare `2024-03-15 14:00:00` becomes `2024-03-15 14:00:00 America/Los_Angeles` (i.e. 22:00 UTC) — then converting to NYC gives 18:00 ET on Mar 15 instead of the correct 10:00 ET (off by 3 hours, possibly into the wrong local day at boundaries). Use one of the two **session-INDEPENDENT** forms instead:

1. **Two-step `AT TIME ZONE 'UTC' AT TIME ZONE '<local>'`** (operator form — same chain the canonical above uses): `event_ts AT TIME ZONE 'UTC' AT TIME ZONE 'America/New_York'`. First operator attaches UTC (bare → timestamptz with UTC label, wall-clock unchanged); second operator converts (timestamptz → timestamptz with NYC label, wall-clock shifted by UTC→NYC offset). No session-zone involvement at any step.
2. **`with_timezone(event_ts, 'UTC') AT TIME ZONE 'America/New_York'`** (function form): [`with_timezone(timestamp(p), zone)`](https://trino.io/docs/current/functions/datetime.html) takes a naive `TIMESTAMP` and **directly assigns** the given zone as the attached label — **no session-zone involvement**. The function form is operator-precedence-safe inside complex expressions (e.g., `COALESCE`, `CASE`, arithmetic) where the bare `AT TIME ZONE 'UTC'` form needs extra parentheses.

Worked-example bucketing on a bare-timestamp UTC-intent column:

```sql
-- Schema: events.occurred_at is TIMESTAMP(6) (no time zone), values KNOWN to be UTC.
-- Want: count per LOCAL America/New_York day.

-- CORRECT (two-step, session-independent):
SELECT date_trunc('day', occurred_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/New_York') AS local_day_et,
       COUNT(*) AS events
FROM iceberg.analytics.events
GROUP BY date_trunc('day', occurred_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/New_York')
ORDER BY local_day_et;

-- ALSO CORRECT (function form, session-independent — preferred inside complex expressions):
SELECT date_trunc('day', with_timezone(occurred_at, 'UTC') AT TIME ZONE 'America/New_York') AS local_day_et,
       COUNT(*) AS events
FROM iceberg.analytics.events
GROUP BY date_trunc('day', with_timezone(occurred_at, 'UTC') AT TIME ZONE 'America/New_York')
ORDER BY local_day_et;

-- WRONG (session-dependent — only correct when current_timezone() = 'UTC', off by hours otherwise):
-- SELECT date_trunc('day', CAST(occurred_at AS TIMESTAMP WITH TIME ZONE) AT TIME ZONE 'America/New_York') ...

-- WRONG (bare AT TIME ZONE on a tz-NAIVE column ATTACHES, does not CONVERT — off by the full offset):
-- SELECT date_trunc('day', occurred_at AT TIME ZONE 'America/New_York') ...   -- occurred_at is bare TIMESTAMP -> mislabels UTC wall-clock as ET; MUST with_timezone(occurred_at,'UTC') first
```
...
- *"`CAST(bare_ts AS TIMESTAMP WITH TIME ZONE)` is a shortcut to make a bare `timestamp` UTC, so `CAST(occurred_at AS TIMESTAMP WITH TIME ZONE) AT TIME ZONE 'America/New_York'` correctly buckets a UTC-stored bare timestamp by NYC local day."* **FALSE — session-dependent and silently wrong on most clusters.** `CAST(timestamp → timestamp with time zone)` attaches the **current session time zone** (whatever `SET TIME ZONE` set, or the client/JVM default), NOT unconditionally UTC. On a cluster where the session zone ≠ UTC (e.g. `America/Los_Angeles`), the CAST stamps PT on a UTC-intent value, so the subsequent `AT TIME ZONE 'America/New_York'` is off by hours and rows may fall in the wrong local day. Use the **session-independent** forms instead: (1) two-step operator chain `occurred_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/New_York'`, OR (2) function form `with_timezone(occurred_at, 'UTC') AT TIME ZONE 'America/New_York'` — both attach UTC explicitly with no session-zone involvement. (See the CAST-trap companion paragraph immediately above for the worked example, and resource 22 §2A.3 ~line 2179 for the federation companion.)
...
**Cross-references:** Resource 27 §4.2-NOW (the Oracle-migration angle — same two facts, SYSDATE-replacement framing — keep both consistent). Resource 27 §4.2A (the dedicated `SET TIME ZONE` command + `sql.forced-session-time-zone` server property — how to change what zone `current_timestamp` / `now()` use). Resource 27 §4.2B (filtering a `timestamp with time zone` column by a local-date range — boundary-literal form; DO-NOT-WRITE for `AT TIME ZONE >= DATE` and BETWEEN on bare VARCHAR). Resource 13 §timestamp-type-mapping (Postgres `timestamptz` → Iceberg `TIMESTAMP(6) WITH TIME ZONE`). Resource 13 §`from_unixtime` (epoch-seconds-to-timestamp; returns `timestamp(3) with time zone`). **Resource 22 §2A.3 (≈line 2179) — federation companion for the CAST-trap:** the same `CAST(naive_ts AS TIMESTAMP WITH TIME ZONE) = session-zone` rule applies to MySQL `DATETIME`-naive → Trino `TIMESTAMP WITH TIME ZONE` in federated joins; `with_timezone(naive_ts, 'UTC')` and `naive_ts AT TIME ZONE 'UTC'` are the session-independent fixes there too. (Dual-destination cross-ref — whichever file you land at, the CAST-trap fact is consistent.)
...
> ⚠️ **Source table has MANY rows per grouping key (e.g. many orders per DAY)? Do NOT write `SUM(x) OVER (...)` beside `GROUP BY day` in the same SELECT.** The base example above works because `daily_revenue` is ALREADY one row per day. But if you start from a RAW row-grain table (e.g. `orders` with `created_at` + `revenue`, many rows per day) and add `GROUP BY DATE(created_at)`, a bare `SUM(revenue) OVER (...)` in that same SELECT is **INVALID** — Trino raises `'revenue' must be an aggregate expression or appear in GROUP BY clause`. Two correct forms:
> - **(1) Pre-aggregate to one-row-per-day in a CTE first**, then run the window on the daily totals: `SUM(daily_total) OVER (ORDER BY day ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)`. See the CTE recipe further down (**"do NOT write `SUM(amount) OVER` directly on row-grain orders"**).
> - **(2) Nest the aggregate** in a single SELECT: `SUM(SUM(revenue)) OVER (ORDER BY DATE(created_at) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)` — the inner `SUM(revenue)` is the per-day total, the outer `SUM(...) OVER` accumulates it. See **Pattern A2** below.
>
> *Keyword anchors: running total of revenue by day from orders, cumulative revenue by day, daily running total many rows per day, running total grouped by day.*
...
> **Round the VALUE vs. pin the SCALE for display (money / "force exactly two decimal places").** `round(x, 2)` rounds the *value* but **returns the SAME TYPE as its input** — `round(DOUBLE, 2) → DOUBLE` (verified at [trino.io/docs/current/functions/math.html](https://trino.io/docs/current/functions/math.html): `round(x, d) → [same as input]`), so a DOUBLE stays a DOUBLE and may still render as `20.0` (one displayed decimal), not `20.00`. To **lock a column to exactly two displayed decimals** (fixed scale → `20.00`) for storage or a billing/money column, CAST it: **`CAST(x AS DECIMAL(18,2))`**. *(Keyword anchors: force two decimal places, fixed scale for display, clean dollars and cents, exactly two decimals, lock a column to two decimal places, money rounding Trino, billing column two decimals.)* See [resource 27 §4.4 / §4.4A](27-oracle-plsql-to-dbt-trino.md) and [resource 23 §3.1B/§3.1C](23-sql-best-practices-olap.md) for the money-decimal canonical (HALF_UP cast semantics, `SUM(DECIMAL)` exactness, never-SUM-DOUBLE).
...
```sql
-- ✅ COPY THIS — the only correct cumulative-distinct form (first-appearance + running-SUM).
-- Trino 467 has NO COUNT(DISTINCT) OVER (#7885) and SUM(COUNT(DISTINCT ...)) OVER double-counts
-- multi-period entities — both forms are BANNED. The recipe below is the ONLY correct shape.
...
> **WARNING — the SQL fragments shown in this DO-NOT-WRITE section are BANNED forms.** Each banned snippet below is presented as a fenced block where EVERY line carries an inline `-- ❌ WRONG ... DO NOT COPY` comment ON THE SAME LINE — so if you copy-paste any line, the comment self-documents the line as broken. The ONLY correct form is the ✅ COPY THIS canonical block above (first-appearance cohort + running-SUM).

**WRONG #1 — `SUM(COUNT(DISTINCT ...)) OVER` running-sum of per-period distinct-ACTIVE counts (the EXACT iter692 Q4 bug):**

```sql
SUM(COUNT(DISTINCT customer_id)) OVER (ORDER BY order_month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)   -- ❌ WRONG: double-counts multi-period customers (Jan+Feb active counted +2 not +1) — output is cumulative active-instances NOT cumulative-distinct — DO NOT COPY
```

What it produces: **DOUBLE-counts multi-period customers.** A customer active in Jan AND Feb is counted in BOTH per-month COUNT(DISTINCT)s (one per month), and the running-sum then accumulates BOTH — so they contribute `+2` to the cumulative total, not `+1`. Output is **cumulative active-instances** (customer-period pairs), NOT cumulative-distinct customers. The running total can overshoot the true distinct-ever count by 2x-10x depending on retention rate. The per-period column is also **mislabeled if called "new_customers"** — it's distinct-ACTIVE-this-period (including returning customers), not new/first-appearance. RIGHT: use the first-appearance recipe in the ✅ COPY THIS canonical above — `MIN(order_date) GROUP BY customer_id` → `COUNT(*) GROUP BY first_month` → `SUM(new_customers) OVER (ORDER BY order_month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)`. Each customer counted ONCE, at their first-appearance period.

**WRONG #2 — aliasing per-period distinct-ACTIVE as "new_customers" (misleading alias / wrong semantics):**

```sql
SELECT order_month, COUNT(DISTINCT customer_id) AS new_customers FROM orders GROUP BY order_month   -- ❌ WRONG: this is distinct-ACTIVE-this-period (returning customers too) NOT new/first-appearance — DO NOT COPY (and DO NOT alias as "new_customers")
```

What it produces: **Misleading alias / wrong semantics.** `COUNT(DISTINCT customer_id) GROUP BY order_month` gives **distinct-ACTIVE customers in that period** (including returning customers who shopped in earlier months too). It is **NOT** the count of new/first-appearance customers. A customer who first ordered in January and ordered again in February appears in BOTH January's and February's per-month distinct-active count — so February's `new_customers` (in this WRONG aliasing) includes the returning January customer. The correct per-period NEW count must come from the `MIN(order_date) GROUP BY customer_id` first-appearance CTE — see the ✅ COPY THIS canonical above. RIGHT: if you need per-period NEW customer count, use `WITH first_order AS (SELECT customer_id, DATE_TRUNC('month', MIN(order_date)) AS first_month FROM orders GROUP BY customer_id) SELECT first_month AS order_month, COUNT(*) AS new_customers FROM first_order GROUP BY first_month`. If you need per-period DISTINCT-ACTIVE count, name it `active_customers` (not `new_customers`) to avoid the misleading alias.

**WRONG #3 — `COUNT(DISTINCT ...) OVER` window form (parse / analysis error in Trino 467):**

```sql
COUNT(DISTINCT customer_id) OVER (ORDER BY order_month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)   -- ❌ WRONG: Trino 467 has NO COUNT(DISTINCT) OVER (analysis error, trinodb/trino #7885) — DO NOT COPY
```

What it produces: **Parse / analysis error in Trino 467** — `DISTINCT` is not supported inside a window function (tracked at [trinodb/trino #7885](https://github.com/trinodb/trino/issues/7885)). The query fails before execution; no result. The Trino analyzer rejects the query with `DISTINCT in window function parameters not yet supported`. Same banned form covered in detail at the [r07:638-740 rolling-distinct-HLL card](#) — that card is for **trailing N-day rolling-distinct**, this current card is for **cumulative-ever-growing distinct**. Both share the same Trino-grammar gap. RIGHT: use the ✅ COPY THIS canonical above — it sidesteps the window-DISTINCT limitation by precomputing each customer's first-appearance period (a plain GROUP BY aggregate), then running-SUM-ing first-appearance counts (a plain window-SUM over an aggregated column, **no DISTINCT inside the window**).
...
> **The FINAL COUNT step — three equivalent forms, and the SHORTEST one (read this before writing the last SELECT).** Counting the sessions/stretches/segments per entity is the step most often over-complicated or written with **invalid grammar**. Once each row carries the `is_new_session` 0/1 flag (the `events_with_gap` CTE), there are three correct ways to get the per-entity count — listed shortest first:
>
> | Form | When to use | SQL |
> |---|---|---|
> | **SHORTEST — sum the flags directly** (no second window CTE needed at all) | You only need the COUNT of sessions per entity (this question). Each session-start is flagged `1`, so the sum of the flags **IS** the session count. | `SELECT user_id, SUM(is_new_session) AS session_count FROM events_with_gap GROUP BY user_id` |
> | `MAX(session_id)` | You already built the `session_id` running-sum CTE for other per-session work. session_ids are dense from 1, so the max = the count. | `SELECT user_id, MAX(session_id) AS session_count FROM sessions GROUP BY user_id` |
> | `COUNT(DISTINCT session_id)` | Same as above; clearer intent, marginally costlier. | `SELECT user_id, COUNT(DISTINCT session_id) FROM sessions GROUP BY user_id` |
>
> **Threshold / "at least N stretches" rule (the iter1153 off-by-one).** N activity stretches are separated by **N−1** qualifying gaps, so a customer who went dark once and came back = **2** stretches (NOT 3). To find entities with **at least one** come-back / resurrection (active → gap → active), filter for **`SUM(is_new_session) >= 2`** (or `HAVING COUNT(DISTINCT session_id) >= 2`) — **not `>= 3`**. To find entities resurrected at least K times, use `>= K + 1`. Keyword anchors: count sessions per device, number of online sessions, sessions per user single query, how many comeback periods, resurrected customers threshold, active-dark-active count.
...
| `COUNT(DISTINCT SUM(is_new_session) OVER (PARTITION BY user_id ORDER BY event_time)) AS session_count ... GROUP BY user_id` (the **iter1154 fab** — trying to count sessions by wrapping the running-sum window directly inside `COUNT(DISTINCT ...)`) | **INVALID Trino 467 — a window function cannot be an argument to an aggregate at the same query level.** Trino evaluates `GROUP BY` → aggregates → window functions, so `SUM(...) OVER (...)` (a window) nested inside `COUNT(DISTINCT ...)` (an aggregate) fails analysis with *"WINDOW function not allowed in aggregate function"* / *"Cannot nest window functions inside aggregate functions."* It is also needlessly complex. | Use the **SHORTEST** final-count form — sum the flags directly: `SELECT user_id, SUM(is_new_session) AS session_count FROM events_with_gap GROUP BY user_id`. OR compute `session_id = SUM(is_new_session) OVER (...)` in its **own** `sessions` CTE first, **then** `SELECT user_id, COUNT(DISTINCT session_id) FROM sessions GROUP BY user_id` — never window-inside-aggregate in one layer. See the "FINAL COUNT step — three equivalent forms" block above. |
...
> ```
>
> **THREE-SURFACES trap — don't over-correct.** `quarter` and `week` ARE perfectly valid **UNIT strings** for the date-part functions: `date_trunc('quarter', x)`, `date_trunc('week', x)`, `date_add('quarter', n, x)`, `date_add('week', n, x)`, `date_diff('quarter', a, b)`, `date_diff('week', a, b)` all work fine. It is **only the INTERVAL-LITERAL qualifier grammar** that excludes `WEEK`/`QUARTER` — and that same INTERVAL-literal grammar is used on THREE surfaces, all of which reject `WEEK`/`QUARTER`:
> 1. **Date arithmetic** — `d + INTERVAL '1' WEEK` ❌ → `d + INTERVAL '7' DAY` (or `date_add('week', 1, d)`).
> 2. **WHERE range predicates** — `WHERE x >= current_date - INTERVAL '1' QUARTER` ❌ → `... - INTERVAL '3' MONTH` (or `date_add('quarter', -1, current_date)`).
> 3. **WINDOW-FRAME RANGE bounds** — `... OVER (ORDER BY week_start RANGE BETWEEN INTERVAL '3' WEEK PRECEDING AND CURRENT ROW)` ❌ **parse error** → use **`INTERVAL '21' DAY PRECEDING`** (3 weeks = 21 days; gap-safe, value-based) or **`ROWS BETWEEN 3 PRECEDING AND CURRENT ROW`** (positional — correct ONLY if the pre-aggregation has exactly one row per week with NO gaps; a week with zero rows would make ROWS-3-PRECEDING reach back more than 3 calendar weeks — the `INTERVAL '21' DAY` form is the gap-safe choice). The valid `INTERVAL '6' DAY PRECEDING` / `INTERVAL '30' MINUTE PRECEDING` window-frame exemplars elsewhere in this file are exactly that — DAY/MINUTE qualifiers, never WEEK/QUARTER.
>
> So `date_trunc('quarter', current_date)` / `RANGE BETWEEN INTERVAL '21' DAY PRECEDING` are correct; the *only* broken piece is an `INTERVAL '<n>' QUARTER` / `INTERVAL '<n>' WEEK` literal on any of the three surfaces — convert WEEK→`'<7n>' DAY` and QUARTER→`'<3n>' MONTH` (or use `date_add`).
...
> **SKETCH-ALGORITHM INOCULATION — `approx_percentile` uses a QUANTILE-DIGEST / T-DIGEST sketch, NOT HyperLogLog (iter697 PIN — FIX-A).** *Keyword anchors so the responder lands here on every shape:* approx_percentile algorithm, approx_percentile HyperLogLog, approx_percentile vs approx_distinct, approx_percentile accuracy, approx_percentile error, approx_percentile standard error, approx_percentile 2.3%, is approx_percentile HyperLogLog, what algorithm does approx_percentile use, fast approximate p95 per endpoint, fast approximate p99 algorithm, t-digest, qdigest, quantile sketch, quantile-digest, p95 approximate algorithm Trino.
>
> **The one fact you must keep straight.** `approx_percentile(x, p)` computes approximate **QUANTILES** (p50/p95/p99/etc.) using a **quantile-digest (qdigest) / T-digest** sketch — a sketch designed for the QUANTILE problem. It is **NOT** HyperLogLog. **HyperLogLog (HLL)** and the **~2.3% / 0.023 default standard error** belong to the SISTER function `approx_distinct(x)` — which solves a DIFFERENT problem (approximate **COUNT DISTINCT** / cardinality). The two functions live next to each other on the [Trino 467 aggregate-functions page](https://trino.io/docs/467/functions/aggregate.html) and it is easy to misattribute one's algorithm to the other — DO NOT.
>
> | Function | Problem | Sketch algorithm | Documented default error |
> |---|---|---|---|
> | `approx_percentile(x, p)` | Approximate **quantile** (value at percentile `p`) | **Quantile-digest / T-digest** (NOT HLL) | The Trino docs do not publish a single fixed-percentage error for this overload; accuracy comes from the internal sketch. There is no `approx_percentile(x, p, accuracy)` overload on current Trino — `accuracy` lives on the separate `qdigest_agg(x, w, accuracy)` function. |
> | `approx_distinct(x)` | Approximate **count distinct** (cardinality) | **HyperLogLog (HLL)** | **2.3% standard error by default** (per [trino.io/docs/467/functions/aggregate.html](https://trino.io/docs/467/functions/aggregate.html)); the 2-arg form `approx_distinct(x, e)` tunes `e` in `[0.0040625, 0.26]`. |
>
> **DO NOT WRITE** (these are the exact iter696 Q4 conflations — make sure the answer never says any of these):
> - "`approx_percentile` builds a tiny HyperLogLog sketch" — **WRONG**. HLL is for `approx_distinct`. `approx_percentile` uses a quantile-digest / T-digest.
> - "`approx_percentile` is a HyperLogLog-style algorithm" / "HLL-style sketch" — **WRONG**. Different sketch family (quantile sketches solve a different problem than cardinality sketches).
> - "`approx_percentile` has ~2.3% standard error" / "2.3% error" — **WRONG**. The 2.3% figure is `approx_distinct`'s documented HLL default. The Trino docs do **not** quote a single fixed-percent error for `approx_percentile`.
> - "tune `approx_percentile` accuracy with `approx_percentile(x, p, accuracy)`" — **WRONG**. No such overload exists on current Trino 467/481. For tunable accuracy build a `qdigest_agg(x, w, accuracy)` explicitly and call `value_at_quantile(qdigest, p)`.
>
> **CORRECT one-liner (the copy-attractive sentence).** `approx_percentile(latency_ms, 0.95)` returns the approximate p95 of `latency_ms` using a **quantile-digest / T-digest sketch** — a per-worker fixed-size sketch merged in one cheap shuffle. For multiple percentiles in one pass, use the array form: `approx_percentile(latency_ms, ARRAY[0.5, 0.95, 0.99])`. For per-endpoint p95, add `GROUP BY endpoint`. Cross-link: [resource 23 §3 `approx_percentile` signature family + accuracy-note inoculation (lines 267-277)](23-sql-best-practices-olap.md#3-use-approximate-functions-when-exactness-isnt-required) for the full four-overload signature card + the docs-anchored DO-NOT-WRITE for the nonexistent `accuracy` overload.
>
> **Mnemonic:** **`approx_distinct` = HLL = 2.3% (cardinality problem). `approx_percentile` = qdigest / T-digest = no published fixed-percent error (quantile problem).** When the question says "fast approximate p50/p95/p99", the function is `approx_percentile` and the sketch is **NOT** HyperLogLog.
...
> **`floor(x/N)*N` vs integer-division bucketing `(x/N)*N` — round down to nearest multiple / bucket into tens / decades (floor vs integer division).** These are **NOT the same operation in general** — keep `floor(x/N)*N` as the lead for a true round-DOWN-to-the-nearest-multiple, because **`floor()` rounds toward −∞** (verified at [trino.io/docs/467/functions/math.html](https://trino.io/docs/467/functions/math.html): *"Returns `x` rounded down to the nearest integer"*), whereas **integer division `/` on two integers truncates toward ZERO** (same page: *"integer division performs truncation"*).
> - **For NON-NEGATIVE values they coincide.** On an all-non-negative integer column like `customer_age`, `floor(age/10)*10` and the integer-division form `(age/10)*10` give the *same* decade bucket (`0, 10, 20, …`). And on an **integer** column `age`, `age/10` is already integer division (truncates toward zero) — so the wrapping `floor(...)` is a **redundant no-op there** (it's flooring an already-whole number). Both `floor(customer_age/10)*10` and `customer_age/10*10` are correct for ages. The explicit `floor()` form is the safer habit because it stays correct if the column is ever a DECIMAL/DOUBLE or could go negative.
> - **They DIFFER for NEGATIVE values.** `-27 / 10` = `-2` (integer division, truncates toward **0**) so `(-27/10)*10` = `-20`; but `floor(-27 / 10.0)` = `-3` (toward **−∞**) so `floor(-27/10.0)*10` = `-30`. For a true round-DOWN on data that can be negative (temperatures, balances, deltas), use `floor(x/N)*N` (and divide by a DECIMAL/DOUBLE `N` like `10.0` so the division isn't truncated first). The full floor-vs-truncate-vs-CAST direction table is the [4-way "reduce to a whole number" canonical immediately below](#reduce-to-a-whole-number) — `floor` = toward −∞ (round down), `truncate` = toward zero (chop), `CAST(x AS integer)` = half-up (rounds, not a floor).
