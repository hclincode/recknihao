...
>
> **WHY `array_position` IS THE WRONG TOOL for "per-element position while exploding"** (cross-ref to §1a.3): `array_position(arr, value)` returns the **1-based index of the FIRST occurrence** of a **VALUE** (verified at [trino.io/docs/467/functions/array.html](https://trino.io/docs/467/functions/array.html)) — so for `tags = ARRAY['billing','export','billing']`, **both** `'billing'` rows would get `array_position = 1` (the duplicates collide on the first match). `WITH ORDINALITY` is the **only** correct shape when you need each unnested row's TRUE per-row ordinal. **DO NOT WRITE:** `array_position(tags, tag)` in the SELECT list of an UNNEST query to mean "position of this tag in the array" — it returns the first-match index, not the per-row ordinal; duplicates collapse. Use `WITH ORDINALITY` and project the ordinality column directly.
>
...
| `array_position` | `array_position(x, element) -> bigint` | **Returns the 1-based index of the FIRST occurrence of a VALUE in array `x`, or `0` if not found** (verified at [trino.io/docs/current/functions/array.html](https://trino.io/docs/current/functions/array.html)). Use this when you want the index of a KNOWN literal value in an array — e.g. funnel-step number from a step-name array (`array_position(funnel_steps, 'checkout')`). Note: 0 means "not found", NOT NULL. **CAUTION — duplicates collide:** for `arr = ARRAY['billing','export','billing']`, `array_position(arr, 'billing')` returns `1` for **both** `'billing'` occurrences — it always reports the FIRST match. **WRONG ❌ for per-element ordinal while UNNESTing:** `array_position(tags, tag)` in the SELECT list of `... CROSS JOIN UNNEST(tags) AS t(tag)` does **NOT** give each row's TRUE per-row position — duplicate tag values all collapse to the first-match index. For the position of EACH element WHILE exploding an array, use **`UNNEST(arr) WITH ORDINALITY AS t(elem, position)`** — cross-ref [§1a above (WITH ORDINALITY sub-note)](#1a-exploding-an-array-column-to-one-row-per-element-unnest--array-to-rows--left-join-unnest) for the canonical. Inline marker: `array_position(tags, tag)  -- ❌ WRONG for per-element ordinal while UNNESTing: returns FIRST-match index, duplicates collide — use UNNEST ... WITH ORDINALITY — DO NOT COPY`. |
