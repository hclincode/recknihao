...
> **POINTER — `ANALYZE` (CBO stats command) is a DIFFERENT thing from `EXPLAIN ANALYZE` (plan-with-runtime-stats command). Do not confuse them.** This resource covers `ANALYZE` — the Trino DDL-like command that populates Puffin NDV statistics for the cost-based optimizer. `EXPLAIN ANALYZE <query>` is a separate command that **executes** the query and reports per-operator runtime stats. They share the keyword `ANALYZE` and nothing else. For the canonical EXPLAIN-modes reference card (VALIDATE / IO / DISTRIBUTED / EXPLAIN ANALYZE — when each runs the query, when each just plans), see **[`resources/22` §3.4 EXPLAIN-MODES REFERENCE CARD](22-trino-federation-postgresql.md#34-how-to-verify-pushdown-actually-happens--explain)** and **[`resources/18` §EXPLAIN TYPE IO and EXPLAIN TYPE VALIDATE](18-query-performance-regression.md#explain-type-io-and-explain-type-validate--the-two-other-explain-variants-you-should-know)**. **DO-NOT-WRITE forms (these fail to parse — verified against trino.io/docs/current/sql/explain-analyze.html):** `EXPLAIN ANALYZE (ANALYZE false) <query>` is FABRICATED (the ONLY documented option is `VERBOSE`; `EXPLAIN ANALYZE` ALWAYS executes); `EXPLAIN (EXECUTE false)` is FABRICATED. Do not say "Trino has no syntax-checker / validator" — `EXPLAIN (TYPE VALIDATE) <query>` IS that command and returns a single boolean column `Valid`. Do not recommend `LIMIT 1` or `LIMIT 0` as cheap validation — both execute through the planner.
...
>
> ### DO-NOT-WRITE matrix — FABRICATED session-property names (responder muscle-memory traps)
>
> > **Read this even if the `/*+ ... */` matrix above looks like enough.** The session-property *name* itself is a separate fabrication vector. The correct Trino 467 session property is literally `join_distribution_type` — there is **NO `distributed_` prefix on it**, NO `broadcast_`/`hash_` qualifier prefix, and NO `_strategy` suffix. The forms below are common splices that combine a long-removed dead prefix (`distributed_joins` was deprecated and removed *years* ago in favor of `join_distribution_type`) with the current property name, or borrow a Spark/Snowflake naming convention. Every one of them produces `Session property <name> does not exist` at the Trino CLI. Verified against [trino.io/docs/current/optimizer/cost-based-optimizations.html](https://trino.io/docs/current/optimizer/cost-based-optimizations.html) and [trino.io/docs/current/admin/properties-general.html](https://trino.io/docs/current/admin/properties-general.html).
>
> | Forbidden session-property name | Why it's wrong | Trino-correct form |
> |---|---|---|
> | `SET SESSION distributed_join_distribution_type = 'PARTITIONED'` | **FABRICATION** — splices the long-dead `distributed_` prefix with the current `join_distribution_type` name. There is NO `distributed_` prefix on the property. Trino rejects: `Session property distributed_join_distribution_type does not exist`. The `distributed_join` splice is the kind of muscle-memory mash-up that produces this fab (do not write `distributed_join` anywhere — that token does not appear in any Trino 467 session-property or config-property name). | `SET SESSION join_distribution_type = 'PARTITIONED';` |
> | `SET SESSION distributed_joins = true` / `false` | **DEAD / REMOVED.** `distributed_joins` was the legacy boolean session property replaced by `join_distribution_type` (which accepts the three named values `AUTOMATIC`/`BROADCAST`/`PARTITIONED` instead of a true/false). Long removed from Trino — not present in Trino 467. | `SET SESSION join_distribution_type = 'PARTITIONED';` (PARTITIONED is the modern equivalent of the dead `distributed_joins=true`; BROADCAST is the equivalent of `distributed_joins=false`). |
> | `distributed-joins-enabled=true` in catalog/config properties | **DEAD / REMOVED.** The legacy config-property form of the boolean above. Replaced by `join-distribution-type` (config-property form of `join_distribution_type`). Not present in Trino 467 `etc/config.properties` or any catalog file. | `join-distribution-type=PARTITIONED` (or `AUTOMATIC` / `BROADCAST`) in `etc/config.properties`. |
> | `SET SESSION distributed_join = 'BROADCAST'` | **FABRICATION.** The singular `distributed_join` token is not a Trino property name in any form. Same muscle-memory splice as the first row (dead prefix + truncation). Trino rejects: `Session property distributed_join does not exist`. | `SET SESSION join_distribution_type = 'BROADCAST';` |
> | `SET SESSION broadcast_join_distribution_type = 'BROADCAST'` | **FABRICATION.** No `broadcast_` qualifier prefix. The single property `join_distribution_type` already controls broadcast-vs-partitioned via its value. | `SET SESSION join_distribution_type = 'BROADCAST';` |
> | `SET SESSION hash_join_distribution_type = 'PARTITIONED'` | **FABRICATION.** No `hash_` qualifier prefix. Same single property covers hash-partitioned via the `PARTITIONED` value. | `SET SESSION join_distribution_type = 'PARTITIONED';` |
> | `SET SESSION join_distribution = 'PARTITIONED'` (missing the `_type` suffix) | **FABRICATION** — typo / truncation. The property name carries the `_type` suffix. Trino rejects: `Session property join_distribution does not exist`. | `SET SESSION join_distribution_type = 'PARTITIONED';` |
> | `SET SESSION join_strategy = 'BROADCAST'` | **CROSS-DIALECT SPILLOVER (Spark-ism).** `joinStrategy` / `join_strategy` is Spark-/Databricks-influenced naming. Trino has no `join_strategy` session property. | `SET SESSION join_distribution_type = 'BROADCAST';` |
>
> **The meta-rule for the property name**: it is exactly `join_distribution_type` (session) / `join-distribution-type` (config). One token, no prefix qualifier, with the `_type` suffix, with three named values `'AUTOMATIC' | 'BROADCAST' | 'PARTITIONED'`. If you find yourself writing `distributed_…`, `broadcast_join_…`, `hash_join_…`, or `_strategy` in a Trino session-property name, stop and re-read this block — every one of those splices is a fab.
