...
>
> **The truth, stated plainly.** Trino 467 with the Iceberg connector has **NO user-creatable secondary indexes** on any object — no `CREATE INDEX` statement, no `ADD INDEX` clause on `ALTER TABLE`, no implicit indexing on primary-key declarations (the Iceberg connector accepts `PRIMARY KEY` only for schema-level documentation, not as an enforced uniqueness or index-creating constraint). Verified against [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html) — there is no index DDL documented for the Iceberg connector. This is **true for both tables and views** — neither object type supports user indexes in Trino.
>
...
> | "Trino's PRIMARY KEY constraint creates an implicit index." | The Iceberg connector accepts `PRIMARY KEY` syntax only as **documentation metadata** — it is NOT enforced and creates NO index. There is no implicit-index behavior on Trino + Iceberg. | Same as above — use levers 1–4. |
