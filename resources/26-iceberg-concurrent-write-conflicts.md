...
> 9. `data_location`
>
> Plus `extra_properties` (the pass-through map; ALTER-settable on Trino 465+ per [trinodb/trino PR #24031](https://github.com/trinodb/trino/pull/24031), merged Nov 2024, shipped in 465 — we're on 467, so it's available). The complete `CREATE TABLE` property allow-list is broader (adds `partitioning`, `sorted_by`, `location`, `format_version`, `max_commit_retry`, `delete_after_commit_enabled`, `max_previous_versions`, `orc_bloom_filter_columns`, `orc_bloom_filter_fpp`, `parquet_bloom_filter_columns`, `object_store_layout_enabled`, `data_location`, `extra_properties` to `format`) — **note: `compression_codec` is NOT a Trino 467 table property** (added in Trino **477**, [#25755](https://github.com/trinodb/trino/pull/25755); on 467 compression is the catalog config `iceberg.compression-codec`, default ZSTD). **`ALTER TABLE SET PROPERTIES` only accepts the 9 + `extra_properties` listed above**.
>
...
> **POINTER — if asked about exact Spark SQL for writing to / promoting a branch, use the CANONICAL END-TO-END WAP WORKED EXAMPLE in [`resources/17-iceberg-table-maintenance.md` § Write-Audit-Publish (WAP) with Iceberg branches](17-iceberg-table-maintenance.md#write-audit-publish-wap-with-iceberg-branches) — the section opens with a single authoritative worked example using branch name `audit_2026_06_04` (underscore-only, no quoting trap). That worked example is the single source of truth; this resource only summarizes its shape. Verified against [iceberg.apache.org/docs/latest/spark-writes/](https://iceberg.apache.org/docs/latest/spark-writes/) + [spark-procedures/](https://iceberg.apache.org/docs/latest/spark-procedures/):**
> - **CREATE / DROP branch (Spark DDL):** `ALTER TABLE <cat>.<db>.<table> CREATE BRANCH <name> [AS OF VERSION <snapshot_id>] [RETAIN <n> DAYS]` / `ALTER TABLE ... DROP BRANCH <name>`. Prefer underscore-only names (e.g., `audit_2026_06_04`) so no backtick-quoting is needed.
> - **WRITE to branch (Spark — TWO forms):** (i) suffix on identifier: `INSERT INTO <cat>.<db>.<table>.branch_<name> SELECT ...` — the suffix is LITERALLY `branch_<exact-branch-name>` (also works for UPDATE/DELETE/MERGE); (ii) WAP session conf: `SET spark.wap.branch=<name>;` then plain `INSERT INTO <cat>.<db>.<table> SELECT ...`.
> - **PUBLISH branch to main (Spark — fast_forward):** `CALL <cat>.system.fast_forward('<db>.<table>', 'main', '<audit_branch>')` — a PROCEDURE call (not DDL). Signature is `fast_forward(table, branch, to)` where `branch` (2nd arg) = the branch being MOVED, `to` (3rd arg) = the SOURCE whose tip is taken. **Publish = MOVE MAIN UP TO the audit branch**, so `branch='main', to='<audit_branch>'`. Mnemonic: "fast-forward MAIN to the audit branch."
> - **Trino 467 (READ-ONLY):** `SELECT ... FROM <table> FOR VERSION AS OF '<branch_name>'`. Cannot create / write / publish / drop branches.
>
> **DO-NOT-WRITE (these are FABRICATED or LOAD-BEARING WRONG — see the matching DO-NOT-WRITE block in `resources/17` § Write-Audit-Publish for the full canonical version):**
> - **REVERSED `fast_forward` args** — `fast_forward('analytics.orders', '<audit_branch>', 'main')` or `(branch => '<audit_branch>', to => 'main')`. **LOAD-BEARING WRONG.** This moves the AUDIT branch up to main's tip, which abandons the staged work and typically errors "not a fast-forward." The correct publish form is `branch='main', to='<audit_branch>'`.
> - **Mismatched suffix (silent hyphen→underscore conversion)** — branch declared as `staging-branch` but suffix written `branch_staging_branch`. The suffix is LITERAL — `staging-branch` requires `` `branch_staging-branch` `` (backtick-quoted). Easiest fix: rename the branch to `staging_branch` from the start.
