...
> **NAV-HINT — dbt OPS / CONNECTION / SECRETS questions belong to a DIFFERENT resource.** This file is about Spark-based **ingestion** from Postgres into Iceberg — NOT about dbt setup. If your question is *"how do I store my dbt database password safely?"* / *"how do I read a shell environment variable into dbt?"* / *"where do I put credentials in `profiles.yml`?"* / *"what is `env_var()` in dbt?"* / *"what is `DBT_ENV_SECRET_`?"* — jump to **[resources/27-oracle-plsql-to-dbt-trino.md — "dbt connection & secrets — profiles.yml, env_var(), DBT_ENV_SECRET_" section (a dedicated H2 placed just ABOVE §6 "Worked end-to-end example")](27-oracle-plsql-to-dbt-trino.md)**. That section is the canonical home for ALL dbt-trino connection/secrets/`profiles.yml` configuration on this stack (not migration-only — applies to every dbt-trino setup). Keywords: dbt profiles.yml password, env_var, DBT_ENV_SECRET, dbt connection credentials, store dbt password safely, keep dbt password out of logs.
...
|---|---|---|
| "Spark's JDBC reader auto-parallelizes by Postgres partition, so I just point it at a 100M-row table and it's fast." | **NO — Spark JDBC reads single-threaded BY DEFAULT.** Without `partitionColumn` + `numPartitions` + `lowerBound` + `upperBound`, the entire table streams through ONE JDBC connection on ONE executor — a single-machine bottleneck that frequently OOMs Spark and hammers Postgres replication lag. You must explicitly supply the four partitioning options OR a `predicates` array. The reader will NOT auto-detect Postgres table partitions/indexes. | [§ Pattern A — Full refresh](#pattern-a--full-refresh-start-here) JDBC parallelism callout |
| "Postgres JSONB lands in Iceberg as a structured/queryable JSON type — I can do `WHERE jsonb_col->>'plan' = 'pro'` on it from Trino." | **NO — there is no first-class JSON column type in Iceberg 1.5.2 or Trino 467.** JSONB is read by Spark JDBC as VARCHAR, written into Iceberg as VARCHAR (Parquet's `BYTE_ARRAY` with `STRING` logical annotation), and queried in Trino with `json_extract_scalar(col, '$.plan') = 'pro'`. Parquet has a `JSON` logical type annotation, but on this stack the JSON content is **opaque binary with no per-key column statistics** — predicates on JSON keys do NOT prune files. The fix is to flatten the top 5–10 hot keys into real columns at ingest. | [§ JSONB handling](#jsonb--how-to-handle-it) leading callout |
| "I can write directly to Iceberg from Trino — no Spark needed for ingestion." | **TECHNICALLY TRUE but RISKY for batch ingest.** Trino 467 supports `INSERT INTO iceberg.x.y SELECT ... FROM pg.public.t` via the Iceberg connector, but: (a) the Postgres-side SELECT pulls the whole table through a single Trino worker connection per split-bound source (the Postgres connector defaults to **no source parallelism on a single table** unless `parallelism-type=PARTITIONS` is set, which requires the source table to be Postgres-partitioned), (b) you give up Spark's `numPartitions`/JDBC fetch-size tuning, (c) you lose retries on transient Postgres failures, (d) for large/regular loads, this saturates the Trino coordinator with long-running queries. **Use Spark for the production ingestion path; use Trino only for ad-hoc one-shot copies of small tables.** | [§ Pattern A](#pattern-a--full-refresh-start-here) engine choice callout |
| "Trino's `start-snapshot-id` / `end-snapshot-id` works the same as Spark's for incremental reads downstream." | **NO — `start-snapshot-id` and `end-snapshot-id` are SPARK-ONLY DataFrameReader options on Iceberg 1.5.2.** Trino 467 has NO equivalent reader API; the only way to do incremental reads from Trino is a `WHERE updated_at >` watermark column or `FOR VERSION AS OF` (which reads AT a single snapshot, not BETWEEN two). If the downstream consumer is Trino, design the source table with an `updated_at` watermark column from day 1. | [§ QUICK REFERENCE: Iceberg Incremental Reads](#quick-reference-iceberg-incremental-reads--new-rows-since-last-run-start-snapshot-id) Trino note |
| "`MERGE INTO ... USING ... WHEN MATCHED THEN UPDATE` works the same in Trino 467 and Spark 3.x — I can do CDC upserts from either." | **PARTIALLY.** Trino 467's Iceberg connector DOES support MERGE INTO (added in earlier releases; works on format-version 2 tables). Spark also supports MERGE on Iceberg v2 tables. The two engines have **different MERGE-clause arities**: Spark allows multiple WHEN MATCHED / WHEN NOT MATCHED clauses with conditions; Trino historically had narrower MERGE shape support. For complex multi-clause CDC upserts with conditional UPDATE+DELETE branches, USE SPARK. For simple "if matched update, else insert" two-branch MERGE, both engines work. Verify the shape of your merge before assuming Trino matches Spark. | [§ CDC pattern](#pattern-c--cdc-advanced) engine choice callout |
| "If I write to Iceberg from Spark and someone else reads from Trino concurrently, they see partial rows mid-write." | **NO — Iceberg writes are ATOMIC at snapshot granularity.** Until the committing engine flips the table pointer to the new snapshot (a single atomic metadata pointer move backed by HMS), readers see the OLD snapshot. The moment the pointer flips, ALL new rows become visible at once. There is no partial-write window. This is true regardless of file count (a 10,000-file write commits atomically). See [iceberg.apache.org/docs/1.5.1/reliability/](https://iceberg.apache.org/docs/1.5.1/reliability/). | [§ Why Iceberg is atomic](#why-iceberg-is-atomic) callout |
| "I should call `df.write.format('iceberg').mode('append').save('iceberg.x.y')` — that's how Spark writes to Iceberg." | **NO — that path is the legacy `save(path)` API and doesn't work cleanly with the SparkCatalog plugin.** The correct Iceberg-1.5.2 write API is the **catalog-aware DataFrameWriterV2**: `df.writeTo("iceberg.x.y").append()` (or `.overwritePartitions()`, `.createOrReplace()`). The `save()` form will sometimes write to the wrong location or skip the catalog entirely. See [iceberg.apache.org/docs/1.5.1/spark-writes/](https://iceberg.apache.org/docs/1.5.1/spark-writes/). | [§ The Spark job skeleton](#the-spark-job-skeleton) write API callout |
| "I can use Spark's `partitionBy('event_date')` in `df.write` to write an Iceberg-partitioned table." | **NO — `partitionBy()` in `df.write` is Hive-style and IS IGNORED by the Iceberg writer.** Iceberg partitioning is defined on the TABLE via `CREATE TABLE ... PARTITIONED BY (days(event_date))` (or `ALTER TABLE ... SET PROPERTIES partitioning = ARRAY[...]`). The writer reads the partition spec from table metadata, not from `partitionBy()`. If you call `partitionBy('event_date')` on `writeTo()`, it's silently ignored and your "partitioned" table is actually unpartitioned. Always verify with `SELECT * FROM iceberg.x."y$partitions"` after the first write. | [§ Defining partitioning](#defining-partitioning) callout |
| "Hive Metastore is just a metadata cache — if it goes down, my Iceberg tables are still queryable directly from S3/MinIO." | **NO on this stack.** Trino 467's Iceberg connector is configured with `iceberg.catalog.type=hive_metastore`, meaning HMS holds the **current snapshot pointer** for each table. If HMS is down, Trino cannot resolve `iceberg.analytics.events` to its current metadata.json — every query fails. The Parquet files are still safe in MinIO and the metadata.json files are still in MinIO, but discovery requires HMS. The fix for HMS outage tolerance is to migrate to a different Iceberg catalog (Nessie, REST catalog) — NOT to expect HMS-less queries to work. | [§ Hive Metastore wiring](#hive-metastore-wiring) availability callout |

> **Why these specific myths matter.** Each is a load-bearing topic-specific claim about what Spark/Trino/Iceberg "can / can't / does" do during ingestion. Stated as an absolute, it causes engineers to either build expensive workarounds for non-problems (writing custom JDBC paginators because they think Spark won't parallelize), OR to confidently break things (assuming `partitionBy()` partitioned the Iceberg table when it didn't; assuming MERGE behavior is engine-portable when clause arity differs). **The correct discipline:** when about to say "Spark / Trino / Iceberg can / can't / does X" about ingestion, check (a) which engine, (b) which version, (c) the official docs at iceberg.apache.org/docs/1.5.1/ or trino.io/docs/current/connector/iceberg.html, (d) the team's own resource 13.
...
##### 2b. Adding `NOT NULL` to an EXISTING column (no new column) — the literal `ALTER COLUMN ... SET NOT NULL` case

Section 3 below covers the new-column scenario. **This sub-section covers the strictly different case of taking an existing nullable column and constraining it to NOT NULL** — `ALTER TABLE events ALTER COLUMN existing_col SET NOT NULL`. The CDC-pipeline implications are different from the new-column path and frequently mis-debugged:

**Case A — the existing column has no NULL values.** The statement is **metadata-only on Postgres 12+** (catalog flip of `pg_attribute.attnotnull`; a scan happens but no row-level lock is held for it). **Debezium emits nothing** — constraint additions are not captured by `pgoutput` logical decoding. The replication slot does not stall, the WAL sender does not pause, no RELATION message is generated by the constraint flip itself, and the next DML on the table works exactly as before. **The Spark consumer does not need to be restarted.** The pipeline continues uninterrupted.

**Case B — the existing column contains NULL values.** The `ALTER COLUMN ... SET NOT NULL` fails at the DDL level inside Postgres before commit:

```
ERROR:  column "existing_col" of relation "events" contains null values
```

Postgres scans the column as part of executing the ALTER, finds the NULL(s), and aborts the transaction. **The ALTER never commits, never reaches the WAL, and Debezium sees nothing.** The Postgres table is unchanged and the pipeline keeps running. If a colleague says "I ran `SET NOT NULL` and the pipeline broke right after," the constraint addition itself is almost certainly NOT the cause — verify it actually committed before chasing the CDC layer (see the diagnostic ladder below).

**Diagnostic ladder — "pipeline broke after the DDL."** Run these checks in order before assuming the constraint change was the cause:

1. **Did the ALTER actually commit?** `SELECT attnotnull FROM pg_attribute WHERE attrelid = 'events'::regclass AND attname = 'existing_col';` — if `false`, the ALTER errored at the DDL level (Case B) and Debezium definitionally saw nothing.
2. **Is the Debezium task healthy?** `curl http://kafka-connect:8083/connectors/<name>/status | jq '.tasks[0].state'` — expect `RUNNING`. If `FAILED`, the `trace` field has the connector exception; the failure is rarely the constraint and usually something coincident (e.g., a Kafka partition leader change, a Kafka Connect rolling restart, an unrelated schema-history issue on a different table).
3. **Is the replication slot keeping up?** `SELECT slot_name, confirmed_flush_lsn, pg_current_wal_lsn(), pg_size_pretty(pg_current_wal_lsn() - confirmed_flush_lsn) AS lag FROM pg_replication_slots WHERE slot_name = 'debezium';` — if `lag` is many GB, the slot is behind for an unrelated reason (slow consumer, bloated WAL since restart). The constraint-only DDL does not produce WAL on its own.
4. **Did the Spark consumer error?** Check the Spark consumer's structured-streaming log, not the Debezium connector log. Failures attributed to "the NOT NULL change" overwhelmingly turn out to be: (a) a downstream Iceberg-side type mismatch that was already latent, (b) a connectivity blip with Kafka or MinIO at the same time as the DDL, or (c) the application code that was *referenced* by the DDL change failing on its own — none of which involve Debezium.

**Default mental model: a constraint addition cannot break the CDC pipeline** because constraint additions are not captured by `pgoutput`. If the pipeline broke and the DDL was a constraint change, the DDL is almost certainly NOT the cause — start the investigation downstream of Postgres.
...
> **What Trino `EXECUTE optimize` does with position-delete files (and the file_size_threshold catch).** Trino `ALTER TABLE ... EXECUTE optimize` rewrites the data files **below `file_size_threshold`** (default 100MB) into fewer larger files under the **current** partition spec. When it rewrites a data file that has position-delete files against it, it reads the live rows — which **applies those position deletes** — and writes a new clean data file; a full-table optimize (no narrowing `WHERE`/path predicate) then **removes the now-orphaned delete files** ([trinodb/trino#12617](https://github.com/trinodb/trino/issues/12617), implemented via PR #12704; confirmed by the maintainer note on [#24086](https://github.com/trinodb/trino/issues/24086): position deletes are cleaned up in OPTIMIZE when there are no path / file_modified_time predicates). **So Trino `optimize` DOES clear position-delete read overhead — for the data files it actually rewrites — and a Trino-only shop can fix the slowdown without Spark.**
>
> **The catch (this is what trips people up on a CDC bulk-delete table):** Trino's `optimize` picks rewrite candidates **by SIZE only** — it has **NO delete-file-threshold** option ([trinodb/trino#16574](https://github.com/trinodb/trino/issues/16574) is an open feature request to add one). So if the data files carrying the deletes are already well-sized (≥ the 100MB default), a **default** optimize SKIPS them and their position deletes persist — which is the situation people mistake for "optimize doesn't touch delete files." **Trino-only fix: raise `file_size_threshold` ABOVE the size of those data files** (e.g. `EXECUTE optimize(file_size_threshold => '512MB')`, or larger than your biggest file) so they become rewrite candidates; Trino then rewrites them, applies the deletes, and drops the delete files — all from Trino, no Spark.
>
> **Spark alternatives (optional, more targeted — NOT required on a Trino-only stack):** Spark `rewrite_data_files` with `options => map('delete-file-threshold', '1')` rewrites any data file that has ≥1 delete file *regardless of its size* (handy when you don't want to bulk-raise the Trino size threshold); Spark `rewrite_position_delete_files` compacts the delete files *only*, without rewriting data (cheapest, but the rows stay marked-deleted so reads still merge — a smaller/dangling-delete cleanup). Trino has neither procedure today (the position-delete one is on the roadmap, [#27371](https://github.com/trinodb/trino/issues/27371)). Choose Spark `rewrite_data_files(delete-file-threshold)` only if you specifically want delete-driven (not size-driven) candidate selection; otherwise Trino `optimize` with a raised threshold covers it.
...
> ```
>
> The library defaults are `write.delete.mode = copy-on-write`, `write.update.mode = copy-on-write`, and `write.merge.mode = copy-on-write`. **Merge-on-Read must be EXPLICITLY configured** at table creation or via `ALTER TABLE ... SET TBLPROPERTIES` for **Spark-executed** DML to write delete files. Older blog posts and AI-generated content frequently get this backwards — do not trust any claim that "MoR is the Iceberg default" without checking the source. Check what your table is actually using with `SHOW CREATE TABLE iceberg.analytics.events` (Spark) or query `iceberg.analytics."events$properties"` (Trino) and look for all three properties; if a property is absent, the table is using the CoW library default for that operation **(Spark writer behavior only — see the next callout for Trino).**
>
> **CRITICAL — Trino 467's writer IGNORES the property: it is merge-on-read only.** The CoW library defaults above govern **Spark-executed** DML behavior. **Trino 467's Iceberg connector performs row-level UPDATE/DELETE/MERGE in merge-on-read (MoR) mode by DEFAULT** — it writes position-delete files (plus new data files for changed rows), NOT a full copy-on-write file rewrite, **regardless of whether the table's `write.delete.mode` / `write.update.mode` / `write.merge.mode` property is `copy-on-write` or `merge-on-read`**. Trino's writer is MoR-only per [trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272) ("Today Iceberg writes only support merge-on-read mode"). Full copy-on-write support in Trino is an open roadmap item, NOT the Trino 467 default. So if your CDC pipeline ever has Trino apply UPDATEs/DELETEs/MERGEs (e.g., dbt incremental models that compile to MERGE INTO and run on Trino), you will see position-delete files accumulate **even on tables whose properties report `copy-on-write`**. **DO-NOT-WRITE:** "Trino Iceberg UPDATE/DELETE uses CoW by default" — FALSE on Trino 467; "CoW is the Trino default" — FALSE.
>
> **If a CDC table has accumulated position delete files** (`content = 1` rows in `$files` — see the diagnostic section below), that is **either** (a) MoR was explicitly enabled at some point and Spark wrote DML against it, **or** (b) ANY UPDATE/DELETE/MERGE was run from Trino 467 against the table — Trino's writer produces position deletes regardless of property. Delete files can also arrive from a Spark `MERGE INTO` that ran while `write.merge.mode = 'merge-on-read'` was set. **Do not assume "the property says CoW so position deletes must have come from a property-flip"** — on this stack (Trino is the primary query+DML engine alongside Spark CDC ingestion), Trino-executed DML is the more common source of position-delete files.
...
>
> **ADD COLUMN is always nullable.** You cannot add a NOT NULL column to an Iceberg table in one step — the constraint would be violated by historical rows that will read NULL.
>
...
| `timestamptz`, `timestamp with time zone` | `TIMESTAMP(6) WITH TIME ZONE` | Microsecond precision; **UTC-NORMALIZED on Iceberg storage** per the [Iceberg spec](https://iceberg.apache.org/spec/) verbatim *"values are stored as UTC and do not retain a source time zone"* — see **resource 07 §LEADING CANONICAL — `now()` / `current_timestamp` / `current_date` in Trino (+ Iceberg `timestamptz` is UTC-normalized on storage)** for the generic Trino-time-function angle (is `now()` a real Trino function, `now()` vs `current_timestamp`, what timestamp does Trino store, Iceberg UTC) and resource 27 §4.2-NOW for the Oracle-migration angle. **For Trino filtering on this column by local-date range, see resource 27 §4.2B (canonical zone-aware boundary-literal form + DO-NOT-WRITE for BETWEEN with bare VARCHAR strings and for `AT TIME ZONE >= DATE`).** Once landed in Iceberg, the right WHERE clause is `created_at >= TIMESTAMP '2026-06-01 00:00:00 America/New_York' AND created_at < TIMESTAMP '2026-07-01 00:00:00 America/New_York'` — sargable, partition-prunes, half-open. |
...
| `uuid` | `VARCHAR` | **Pragmatic on this stack.** Iceberg DOES have a native `UUID` primitive type (16-byte fixed per the [Iceberg spec](https://iceberg.apache.org/spec/) — "Universally unique identifiers; should use 16-byte fixed"), AND Trino has a top-level `UUID` SQL type. The reason this mapping recommends `VARCHAR` anyway is the **Trino Iceberg connector's incomplete UUID handling** (see [trinodb/trino #6663](https://github.com/trinodb/trino/issues/6663) for the initial-support history and [#9918](https://github.com/trinodb/trino/issues/9918) for the nested-field gap — both relevant as of Trino 467). On this stack, store as 36-char `VARCHAR` for query/dbt portability; if a downstream needs the binary form for storage efficiency, `VARBINARY` (the Iceberg `binary` type, 16 bytes) is the fallback. **DO NOT WRITE: "Iceberg has no UUID type" — it does; the recommendation here is connector-pragmatic, not spec-mandated.** |
...
> **SCHEMA-EVOLUTION COLUMN-TYPE-CHANGE GUARDRAIL — Trino 467 CAN widen Iceberg column types natively; Iceberg-Spark uses `ALTER COLUMN ... TYPE`, NOT `MODIFY COLUMN`.** Read this BEFORE answering ANY question about "widen INT to BIGINT", "promote float to double", "widen decimal precision", or "do I need Spark to change a column type". This guardrail exists because the iter433 Q4 confident-inaccuracy claimed **(A)** "Trino 467 does NOT expose column-type modification, must use Spark" — **WRONG** — and **(B)** used Spark syntax `ALTER TABLE t MODIFY COLUMN col BIGINT` — **WRONG** (`MODIFY COLUMN` is MySQL/Hive, not Iceberg).
>
> **Fact 1 — Trino 467 supports Iceberg column-type widening natively.** `ALTER TABLE iceberg.<schema>.<table> ALTER COLUMN <col> SET DATA TYPE <new_type>` is supported in Trino since **release 406 (25 January 2023, [trinodb/trino #15515](https://github.com/trinodb/trino/pull/15515))**. Trino 467 inherited it. **You do NOT need to start a Spark session for safe widening on this stack.**
>
> **Fact 2 — Iceberg-Spark uses `ALTER COLUMN ... TYPE`, NOT `MODIFY COLUMN`.** Per [iceberg.apache.org/docs/latest/spark-ddl/](https://iceberg.apache.org/docs/latest/spark-ddl/), the canonical Spark/Iceberg form is `ALTER TABLE t ALTER COLUMN c TYPE new_type`. `MODIFY COLUMN` is MySQL/Hive DDL and produces a parser error in spark-sql against an Iceberg table.
>
> **Two-engine syntax table — memorize this:**
>
> | Engine | Exact column-type-change syntax | Source |
> |---|---|---|
> | **Trino 467** | `ALTER TABLE iceberg.analytics.events ALTER COLUMN row_count SET DATA TYPE bigint;` | [trino.io/docs/current/sql/alter-table.html](https://trino.io/docs/current/sql/alter-table.html), [release 406 notes](https://trino.io/docs/current/release/release-406.html) |
> | **Spark 3.5 / Iceberg 1.5.2** | `ALTER TABLE iceberg.analytics.events ALTER COLUMN row_count TYPE bigint;` | [iceberg.apache.org/docs/latest/spark-ddl/](https://iceberg.apache.org/docs/latest/spark-ddl/) |
>
> **DO-NOT-WRITE (must never reproduce — these are the iter433 Q4 confident-inaccuracies):**
>
> 1. **"Trino 467 does NOT expose column-type modification, must use Spark."** — FALSE. Trino 406+ (Jan 2023, three years before production Trino 467) supports `ALTER COLUMN ... SET DATA TYPE` on the Iceberg connector. An engineer following this advice would needlessly spin up Spark for a Trino-native operation.
> 2. **"Trino can't change column types in Iceberg / must use Spark to widen a column."** — FALSE (same as #1, paraphrase).
> 3. **`ALTER TABLE t MODIFY COLUMN col BIGINT`** — FALSE syntax for Iceberg on BOTH engines. `MODIFY COLUMN` is MySQL/Hive DDL. Trino-Iceberg uses `ALTER COLUMN ... SET DATA TYPE`; Iceberg-Spark uses `ALTER COLUMN ... TYPE`. Pasting `MODIFY COLUMN` against an Iceberg table produces a parser error.
> 4. **"Spark syntax is `ALTER TABLE t MODIFY COLUMN col BIGINT`."** — FALSE. Correct Spark form is `ALTER TABLE t ALTER COLUMN col TYPE bigint`.
>
> **Safe widenings (metadata-only):** `int`→`long` (Trino/Spark `int`→`bigint`); `float`→`double` (Trino `real`→`double`); `decimal(P, S)`→`decimal(P', S)` where `P' > P` **and scale unchanged**. **Unsafe / rejected:** narrowing (bigint→int, double→float), decimal scale change, cross-family (numeric↔varchar↔date/timestamp). See "Iceberg spec — the complete list of safe type promotions" table above for the full unsafe list and the multi-step migration pattern for unsupported changes.
>
> ---
>
> **Engine-specific `ALTER COLUMN` syntax — Trino 467 vs. Spark SQL (expanded reference).** The Iceberg spec defines what promotions are allowed, but the SQL **syntax** for issuing the change differs by engine. Pasting the wrong form into the wrong client is one of the most common newcomer footguns on this stack — the spec is permissive, but the parser is not.
>
> | Engine | Syntax | Notes |
> |---|---|---|
...
>    - From Spark SQL (Iceberg-canonical form, per [iceberg.apache.org/docs/latest/spark-ddl/](https://iceberg.apache.org/docs/latest/spark-ddl/)): `ALTER TABLE iceberg.analytics.events ALTER COLUMN score TYPE BIGINT;`
>      (Spark also accepts the Hive-compat alias `ALTER TABLE iceberg.analytics.events CHANGE COLUMN score score BIGINT;` with the column name repeated twice — both work in Spark, but the `ALTER COLUMN ... TYPE` form is the Iceberg-documented canonical syntax.)
>      **NEVER use `MODIFY COLUMN ... BIGINT` — that is MySQL/Hive DDL, not Iceberg-Spark; it produces a parser error in spark-sql against an Iceberg table.**
>    Both Trino and Spark forms produce the same metadata-only change on the same Iceberg table — see the "Engine-specific `ALTER COLUMN` syntax" callout above for full details. Pasting the Spark form into Trino (or vice versa) is a parse error, not a silent success.
...
> **Iceberg 1.5.2 library default for `write.merge.mode` is Copy-on-Write (CoW) — applies to Spark MERGE INTO only.** When **Spark** executes a MERGE INTO against a table on the library default, Iceberg rewrites the entire Parquet data file that contained that row, producing a new Parquet file with the updated content. **No separate delete files are created** in that path. Running `rewrite_data_files` after a Spark MERGE INTO session is still recommended for **small-file consolidation** (many individual MERGE runs each produce small rewritten files) — not to apply pending delete files, because there are none in the Spark-CoW path.
>
> **Trino 467 caveat: a MERGE INTO run from Trino does NOT use CoW — it writes position-delete files (MoR) regardless of the table's `write.merge.mode` property.** Trino 467's Iceberg writer is MoR-only per [trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272) ("Today Iceberg writes only support merge-on-read mode"). Full copy-on-write support in Trino is a roadmap item, NOT the 467 default. So if your dbt incremental model runs MERGE INTO on Trino (the common case on this stack), expect position-delete files (`content = 1` in `$files`) to accumulate — schedule periodic Spark `rewrite_position_delete_files` (Spark-only — see r17 §1b) to fold them back into data files. **DO-NOT-WRITE:** "Trino MERGE INTO uses CoW by default" — FALSE on Trino 467.
...
| **`merge`** | `MERGE INTO target USING source ON unique_key WHEN MATCHED THEN UPDATE SET col1 = s.col1, col2 = s.col2, ... WHEN NOT MATCHED THEN INSERT (col1, col2, ...) VALUES (s.col1, s.col2, ...)` — **explicit column lists** (no `UPDATE SET *` / `INSERT *` star shorthand; that's Spark/Delta-Lake syntax, NOT Trino). See the compiled-SQL example two subsections down for the literal form dbt-trino emits, and resource 27 §4.6B for the full Trino MERGE star-shorthand guardrail. | Mutable dimension and fact tables (orders, subscriptions, user profiles). **Requires `unique_key`.** |
...
> **CRITICAL — dbt's default MERGE has NO conditional predicate on the matched branch.** A very common AI-generated mistake is to claim dbt compiles `WHEN MATCHED AND s.updated_at > t.updated_at THEN UPDATE SET ...` (with a conditional predicate). **It does not.** The default is `WHEN MATCHED THEN UPDATE SET col1 = s.col1, col2 = s.col2, ...` with **no `AND condition` on the matched branch** — every matched row is overwritten unconditionally, regardless of whether the source's `updated_at` is newer or older than the target's. If a stale row arrives from an out-of-order source, dbt will happily overwrite a fresher target row with the older values. **Note the explicit column lists** — dbt-trino's compiled MERGE uses `UPDATE SET col1 = DBT_INTERNAL_SOURCE.col1, ...` and `INSERT (col1, col2, ...) VALUES (DBT_INTERNAL_SOURCE.col1, ...)`, NOT the Spark-style `UPDATE SET *` / `INSERT *` shorthand (which is not valid Trino MERGE syntax — see resource 27 §4.6B).
>
...
### Copy-on-Write vs Merge-on-Read for dbt-managed Iceberg tables (on this stack: dbt-trino MERGE is ALWAYS MoR, regardless of property)

When dbt-trino runs `MERGE INTO` on Iceberg, the physical write behavior is **merge-on-read regardless of the table's `write.merge.mode` property** — because **Trino 467's Iceberg writer is MoR-only** per [trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272) ("Today Iceberg writes only support merge-on-read mode"). Full copy-on-write support in Trino is a roadmap item, NOT the 467 default. So:

- **dbt-trino MERGE INTO on Trino 467 (the common case on this stack):** Updated rows are masked with small **position-delete files** + new data files for the changed rows; the original data files stay intact. Lower per-MERGE write cost, but every read scan must merge data + delete files until compaction. **This is what you get regardless of whether the table property says `write.merge.mode = 'copy-on-write'` or `'merge-on-read'`.** Schedule periodic Spark `rewrite_position_delete_files` (Spark-only — see r17 §1b) to fold the position deletes back into data files; Trino's `EXECUTE optimize` also folds them when processing whole partitions without `file_modified_time`/path predicates ([trinodb/trino#24086](https://github.com/trinodb/trino/issues/24086)).
- **Spark MERGE INTO on the Iceberg 1.5.2 library-default CoW table** (only relevant if your CDC pipeline runs MERGE from Spark, NOT from dbt-trino): every matched row's containing Parquet file is rewritten; no delete files are produced. Higher per-MERGE write cost, lower read cost.
- **Spark MERGE INTO on a MoR-property table (`write.merge.mode = 'merge-on-read'` explicitly set):** same as dbt-trino — position-delete files accumulate.

**For dbt-trino incremental models on this stack:** the question "should I switch to MoR?" is moot — **dbt-trino MERGE is already MoR**. The actual decision is "how often should I run `rewrite_position_delete_files` to keep read amplification under control?" — see r17 §1b cadence table. **DO-NOT-WRITE:** "dbt-trino MERGE uses CoW by default" — FALSE; "stick with the CoW default for dbt-trino" — FALSE on Trino 467, you don't have a CoW path from Trino at all.
...
**Small-file accumulation.** Frequent incremental runs create many small files as Iceberg writes per-MERGE outputs — either small rewritten Parquet files (Spark-CoW path) or small new data files + position-delete files (the dbt-trino-on-Trino-467 path, which is always MoR per the section above). **Schedule nightly compaction** via Trino's `ALTER TABLE ... EXECUTE optimize` or Spark's `CALL iceberg.system.rewrite_data_files`, or query performance degrades over weeks. For the dbt-trino-MERGE-is-MoR case, also schedule periodic Spark `rewrite_position_delete_files` to collapse accumulated position-delete files (Trino 467 has no equivalent — see r17 §1b). See resource 17 for the full maintenance schedule.
