...
> **TL;DR — a dbt snapshot has THREE orthogonal decisions; don't conflate them (read FIRST):**
> 1. **Change-detection strategy:** `strategy='timestamp'` (source has a reliable `updated_at`) vs `strategy='check'` (no reliable timestamp → `check_cols`). Details in 1a/1b below.
> 2. **Hard-delete handling — YES, dbt snapshots handle source-side hard DELETEs NATIVELY; you do NOT hand-roll a LEFT JOIN.** Set `hard_deletes='new_record'` to RECORD each deletion as a new version row with a `dbt_is_deleted='True'` column + `dbt_valid_from` = when dbt observed the delete (this is the "record that a customer was removed and when" answer). `'invalidate'` just closes the row (stamps `dbt_valid_to`, the legacy `invalidate_hard_deletes=true`); `'ignore'` (default) does nothing. **Full matrix + worked Oracle-DELETE example in the "HARD DELETES" block below.**
> 3. **`unique_key`** must reference a column in the snapshot's SELECT OUTPUT (alias rule below).
>
> dbt offers TWO change-detection strategies — pick exactly one per snapshot:
...
>
> **⚠️ ADAPTER-SUPPORT CAVEAT — verify on dbt-trino BEFORE relying on this (THIS stack).** dbt's `hard_deletes` (and the legacy `invalidate_hard_deletes`) is **adapter-implemented**, and dbt's docs enumerate support for **dbt-postgres / dbt-bigquery / dbt-snowflake / dbt-redshift only** — **dbt-trino is NOT on that list**, and the dbt-trino CHANGELOG does not record adding it. So on this Trino + Iceberg stack you **cannot assume `hard_deletes='new_record'/'invalidate'` works** — smoke-test it on a tiny throwaway snapshot first (delete one source row, run `dbt snapshot`, check whether `dbt_valid_to`/`dbt_is_deleted` actually gets stamped). **If it is unsupported on your dbt-trino version, use the reconciliation fallback:** a scheduled model/op that anti-joins the live source against the snapshot's OPEN rows and closes the orphans yourself — `... FROM snapshot s WHERE s.dbt_valid_to IS NULL AND NOT EXISTS (SELECT 1 FROM {{ source(...) }} src WHERE src.id = s.id)` → then `UPDATE`/`MERGE` those `dbt_valid_to = now()` (or insert a deleted-marker row). The table below describes the NATIVE behavior **where the adapter supports it**; don't promise it works on dbt-trino without the smoke test.
>
...
> - "dbt snapshots cannot detect hard deletes — you must hand-roll a LEFT JOIN / use an incremental model to stamp `dbt_valid_to` yourself." **FALSE.** dbt snapshots handle source-side hard DELETEs natively via the `hard_deletes` config (`'new_record'` records the deletion with `dbt_is_deleted='True'`; `'invalidate'` closes the row; legacy `invalidate_hard_deletes=true` on dbt ≤1.8). Do NOT recommend a manual LEFT JOIN against the prior snapshot — that re-implements (worse, and racily) what `hard_deletes` does for free. See the HARD DELETES block above.
