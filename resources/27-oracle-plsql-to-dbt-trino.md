...
4. **MERGE is the load-bearing translation.** Oracle's `MERGE INTO target USING source ON (...) WHEN MATCHED THEN UPDATE WHEN NOT MATCHED THEN INSERT` maps directly to **dbt incremental with `incremental_strategy='merge'` and `unique_key='...'`** — dbt-trino generates the Trino `MERGE INTO` SQL for you. This is the canonical SCD-**1** / upsert pattern on Iceberg (overwrite — keeps only the CURRENT row per key). **If you instead need SCD-2 HISTORY** — keep every change with effective dates and answer "what plan/price/status was this entity on as of `<date>`" — that's a **dbt SNAPSHOT**, NOT an incremental merge model: see **[resource 09 § Slowly Changing Dimensions — Option 1 dbt snapshot](09-lakehouse-schema-design.md#slowly-changing-dimensions-scd)** (creates `dbt_valid_from`/`dbt_valid_to`/`dbt_scd_id`; as-of query `WHERE dbt_valid_from <= TIMESTAMP '<date>' AND (dbt_valid_to IS NULL OR dbt_valid_to > TIMESTAMP '<date>')`). *This is the answer EVEN IF the question never says the word "snapshot" — route here for: build change history from a mutable / updated-in-place source that has no history, track plan/tier/price/status changes over time, "what plan/tier was this customer on in February / last quarter / on <past date>", point-in-time dimension lookup, version history of a dimension, how many accounts upgraded/changed tier last quarter, Oracle/Postgres table updated in place with no audit log. Do NOT bail with "SCD-2 isn't covered" — dbt snapshots in r09 ARE the SCD-2 mechanism on this stack.*
...
| "Trino has no per-row correlated join — Oracle's `CROSS APPLY` / `OUTER APPLY` can't be done in Trino, and Trino has no `LATERAL`." | **FALSE — Trino 467 HAS `LATERAL`.** Only the `APPLY` *keyword* (Oracle / SQL Server) is absent; `LATERAL` is the ANSI-standard equivalent and IS supported (verified verbatim at [trino.io/docs/467/sql/select.html](https://trino.io/docs/467/sql/select.html): *"Subqueries appearing in the FROM clause can be preceded by the keyword LATERAL. This allows them to reference columns provided by preceding FROM items."*). **Direct translation:** `CROSS APPLY (subq)` → `CROSS JOIN LATERAL (subq)`; `OUTER APPLY (subq)` → `LEFT JOIN LATERAL (subq) ON true`. The LATERAL subquery may correlate to the outer row AND contain `ORDER BY` / `LIMIT`, so the Oracle "each account's 3 most-recent tickets" shape ports directly: `SELECT a.*, t.* FROM accounts a LEFT JOIN LATERAL (SELECT ticket_id, subject, created_at FROM support_tickets s WHERE s.account_id = a.account_id ORDER BY created_at DESC LIMIT 3) t ON true`. **BUT for top-N-per-group, the `row_number() OVER (PARTITION BY account_id ORDER BY created_at DESC)` + outer `WHERE rn <= 3` rewrite is usually the better-planning, more idiomatic Trino form** (a correlated LATERAL-with-LIMIT can plan as a per-row CorrelatedJoin). Pick LATERAL for 1:1 migration parity / genuinely per-row logic; pick `row_number()` for top-N-per-group at scale. **DO-NOT-WRITE:** "Trino has no `LATERAL`" (FALSE — it does); `CROSS APPLY` / `OUTER APPLY` keyword in Trino (parse error — use `CROSS JOIN LATERAL` / `LEFT JOIN LATERAL ... ON true`). *Keyword anchors: CROSS APPLY Trino, OUTER APPLY Trino, LATERAL Trino, CROSS JOIN LATERAL, LEFT JOIN LATERAL, per-row correlated join, top-N per account, 3 most recent per group, SQL Server APPLY migration.* | [Trino SELECT - LATERAL](https://trino.io/docs/467/sql/select.html) |
...
> **QUICK ANSWER — "what is a dbt `ephemeral` model / is it a temp table or a view / what does ephemeral do differently / downsides of ephemeral / ephemeral vs view in dbt"** (read this if your question is about ephemeral specifically): **an `ephemeral` model is NEITHER a temp table NOR a view — it creates NO database object at all.** Its `SELECT` is **inlined as a CTE (`WITH <name> AS (...)`) into every downstream model that `ref()`s it, at dbt COMPILE time**, before any SQL reaches Trino. So it has **zero warehouse footprint / zero storage**, but it does **NOT save compute** — the same SELECT logic is textually inlined and re-run inside each downstream query. **Downsides:** (1) it cannot be queried directly or inspected in the warehouse (no object exists — harder to debug); (2) at scale it causes **compile-time SQL bloat** — referenced by N downstreams (or chained through other ephemerals), the inlined SQL multiplies, producing multi-thousand-line compiled queries and slower Trino planning; (3) you cannot run a data test on the relation itself. Use it ONLY for a small, lightweight intermediate referenced 1-2 times. **Full mechanics + the at-scale failure mode: [resource 28 §3.3 + §3.3A](28-complex-sql-performance-trino-dbt.md).** (`ephemeral` is the 4th row of the table below.)
...
| **`snapshot`** *(a SEPARATE dbt resource type, NOT a `materialized=` value)* | A **TABLE** with SCD-2 history (one row per version + `dbt_valid_from` / `dbt_valid_to` / `dbt_scd_id` / `dbt_is_deleted` meta-columns) | **YES — every historical version** | **Full** (grows with each change) | A `{% snapshot %}` block in `snapshots/`, run by **`dbt snapshot`** (or as part of `dbt build`); on each run dbt closes the prior version (stamps `dbt_valid_to`) and inserts the new one | **Tracking how a dimension CHANGES over time** — "what plan/tier/price/status was this account on as of `<past date>`", building change history from an in-place-updated source with no audit log. The Oracle SCD-2 procedure becomes a snapshot, NOT an incremental model. | When you only need the CURRENT row (use `incremental`/`merge` = SCD-1) — a snapshot keeps every version and costs more storage |

> **A dbt `snapshot` is the SCD-2 / history-tracking mechanism — do NOT conclude "snapshots aren't covered."** Full canonical (timestamp vs check strategy, `check_cols`, `hard_deletes`, the `{% snapshot %}` block, as-of-date query) is the SINGLE source of truth at **[resource 09 § Slowly Changing Dimensions — Option 1 dbt snapshot](09-lakehouse-schema-design.md#slowly-changing-dimensions-scd)**. *Route here for: "track plan_tier changes over time", "stop losing history when we overwrite", "what does a dbt snapshot do differently from a regular model", "store every plan/price/status change with effective dates".*
...
> **Cross-ref — "my `materialized='table'` full rebuild makes dashboards briefly see a `table does not exist` error / an empty table / zero rows" → how to make the old→new SWAP atomic.** The knob is **`on_table_exists`** (values `replace` / `rename`-default / `drop` / `skip`); set **`on_table_exists='replace'`** for a single atomic `CREATE OR REPLACE TABLE`, or stay on the default `'rename'`; only `'drop'` exposes the missing-table window. You do **NOT** need to convert the model to `incremental`. Full canonical (the four values, symptom→cause mapping, DO-NOT-WRITE): **[resource 28 § LEADING CANONICAL — atomic table SWAP on a `materialized='table'` full rebuild (`on_table_exists`)](28-complex-sql-performance-trino-dbt.md)**.
...
| **`delete+insert`** | DELETEs matching rows in target by `unique_key`, then INSERTs the new rows. | `unique_key` is not actually unique in the source (so MERGE's one-match-per-target-row rule would error). The Iceberg connector supports MERGE so this is rarely needed in this stack. **NOTE: this is NOT a usable fallback for a non-ACID Hive-connector table** — see the caveat row directly below. | `DELETE FROM target WHERE <unique_key> IN (SELECT <unique_key> FROM source) ; INSERT INTO target SELECT ... FROM source` |
...
> **CANONICAL — "dbt `incremental_strategy='merge'` works on my Iceberg catalog but FAILS on my legacy Hive-connector catalog — switch to `delete+insert`?"** (read this for ANY "merge fails on Hive / which connector supports upsert" question.) **`merge` requires the connector to support `MERGE INTO`; `delete+insert` requires the connector to support row-level `DELETE … WHERE <key> IN (…)`. On the Trino 467 Hive connector BOTH are gated on the SAME thing — table transactionality — so `delete+insert` is NOT a workaround for a plain Hive table.** Verified verbatim at [trino.io/docs/467/connector/hive.html](https://trino.io/docs/467/connector/hive.html): *"MERGE is only supported for ACID tables"* AND *"DELETE applied to non-transactional tables is only supported if the table is partitioned and the WHERE clause matches entire partitions."* So on an ordinary **non-ACID** Hive table, MERGE fails, **and arbitrary-key `delete+insert` fails the same way** (row-level DELETE on a non-partition key is unsupported; only whole-partition deletes work). **The right fix is to get the table onto the Iceberg connector** — `CALL iceberg.system.migrate(schema_name => '…', table_name => '…')` (Trino-native, in-place, no data rewrite — see [resource 17](17-iceberg-table-maintenance.md)), then point the dbt model's target at `iceberg.*` and use `incremental_strategy='merge'` with `unique_key`. If migration is genuinely blocked, the only Hive-native pattern is **`incremental_strategy='append'` + dedup downstream** (a `ROW_NUMBER() OVER (PARTITION BY <key> ORDER BY updated_at DESC) = 1` view over the append-only history), or, when you re-process whole partitions, `insert_overwrite`/partition-replace on a **partitioned** Hive table (whole-partition DELETE is allowed). **DO-NOT-WRITE:** "merge isn't supported on Hive, so just switch `incremental_strategy` to `delete+insert` and it'll upsert into the Hive table" — **FALSE on a non-ACID Hive table**; `delete+insert`'s row-level DELETE hits the very same ACID/partition gate as MERGE and the run fails identically. The connector capability — not the dbt strategy name — is the binding constraint.
...
>
> **Trino 467 DOES have a `to_char(timestamp, format)` function** (Teradata-compatibility module, verified at [trino.io/docs/467/functions/teradata.html](https://trino.io/docs/467/functions/teradata.html)) — BUT it is **NOT a drop-in for Oracle's `TO_CHAR`**, so you still migrate to `date_format` / `format_datetime`. Two reasons Oracle masks don't translate to Trino's `to_char`: (1) Trino's `to_char` accepts **LOWERCASE NUMERIC-ONLY** codes (`dd`, `mm`, `yyyy`, `yy`, `hh`, `hh24`, `mi`, `ss` + punctuation) — Oracle's **UPPERCASE** masks (`'YYYY-MM-DD'`, `'HH24:MI:SS'`) do **not** match and produce wrong/empty output; (2) Trino's `to_char` has **NO month-name / day-name support** (no `'Month'`, `'MON'`, `'Day'`, `'DY'`) — for a `'June 2026'` / `'Mon'` label you MUST use `date_format` / `format_datetime`. So: copy-pasting Oracle `TO_CHAR(dt, 'YYYY-MM-DD')` verbatim fails on the **uppercase mask**, NOT because `to_char` is unregistered. **DO-NOT-WRITE:** "Trino has no `to_char` / `Function 'to_char' not registered`" — FALSE (it exists, lowercase-numeric-only). The correct migration answer remains `date_format(dt, '%Y-%m-%d')` / `format_datetime(dt, 'yyyy-MM-dd')` (more capable, handle month names).
>
> **Oracle `TO_CHAR(<NUMBER>, mask)` (the CURRENCY / number-formatting overload) → Trino `format('$%,.2f', amount)`.** Keyword anchors: *Oracle TO_CHAR number mask, TO_CHAR(amount, 'FM$999,999,999.00'), format a number as currency / dollar amount in Trino, format money with commas and 2 decimals, $1,234,567.89, FM mask, thousands separator + currency symbol.* Oracle's `TO_CHAR(total_revenue, 'FM$999,999,999.00')` (NUMBER overload, distinct from the timestamp overload above) has **NO Trino `to_char` equivalent** — Trino's `to_char` is timestamp-only. **The answer is Trino's printf-style `format()`:** `format('$%,.2f', total_revenue)` → `'$1,234,567.89'` (`%,.2f` = thousands-grouped + 2 decimals; literal `$` prefix). Do **NOT** punt this to the application layer (`format()` does it in SQL), and do **NOT** write `format_number(amount, '#,##0.00')` — `format_number` IS a Trino built-in but only does compact-units magnitude (`'1.2M'`/`'500K'`), it does NOT accept an Oracle/Spark `'#,##0.00'` mask. See [resource 23 §3.1A `format()` LEADING CANONICAL](23-sql-best-practices-olap.md) for the full printf spec table.
>
...
> -- ✅ COPY THIS — the FULL month name + year ("June 2026" style):
> date_format(CAST(report_date AS timestamp), '%M %Y')      -- 'June 2026'
> -- ❌ %B is NOT a valid Trino date_format specifier — use %M (full month name) / %b (abbreviated 'Jun') — DO NOT COPY:
> --     date_format(CAST(report_date AS timestamp), '%B %Y')   -- WRONG: %B does not exist in Trino (it is a C/strftime code)
...
>
> ### DO-NOT-WRITE matrix — Trino has NO `::` cast operator; Oracle `TO_CHAR` UPPERCASE/month-name masks don't translate
>
> | Forbidden form | Where it comes from | What it does in Trino 467 | Trino-correct equivalent |
> |---|---|---|---|
> | `TO_CHAR(order_ts, 'YYYY-MM-DD')` (Oracle UPPERCASE mask) | Oracle | **NOT a "not registered" error** — Trino 467 HAS `to_char(timestamp, format)` (Teradata module), but it takes **lowercase-numeric-only** codes (`yyyy-mm-dd`), so the Oracle UPPERCASE mask produces wrong/empty output; and Trino's `to_char` has NO month-name codes. | `date_format(order_ts, '%Y-%m-%d')` or `format_datetime(order_ts, 'yyyy-MM-dd')` (canonical — more capable, handle month names) |
...
> - Trino `date_diff('month', d2, d1)` returns an **INTEGER (bigint) count of *complete* months** — it is **day-aware** and **drops the fractional unit** (it does NOT count calendar month-boundary crossings). No fractional part is produced. Example: `date_diff('month', DATE '2026-01-15', DATE '2026-03-10')` = `1` (Jan 15 + 1 month = Feb 15 ≤ Mar 10 is one complete month; +2 months = Mar 15 > Mar 10, so the partial second month is dropped — it becomes `2` only at Mar 15). Source: 467 `DateTimeFunctions.java` delegates to Joda `getDifferenceAsLong` ("fractional units are dropped").
...
| `TO_CHAR(dt, 'YYYY-MM-DD')` | `date_format(dt, '%Y-%m-%d')` (MySQL-style, FIRST-CHOICE) OR `format_datetime(dt, 'yyyy-MM-dd')` (Joda) OR `CAST(dt AS VARCHAR)` for ISO of a DATE column only. | See the LEADING CANONICAL block at the top of this section for the full Oracle TO_CHAR ↔ Trino format-string mapping table and DO-NOT-WRITE matrix. **Trino 467 HAS `to_char(timestamp, format)` (Teradata module) but with lowercase-numeric-only codes and no month names — so Oracle's uppercase/month-name masks don't translate; use `date_format` / `format_datetime`.** |
...
| `MONTHS_BETWEEN(d1, d2)` | `date_diff('month', d2, d1)` (integer count of *complete*, day-aware months — drops the fractional part) | Oracle's `MONTHS_BETWEEN` returns a **FRACTIONAL** number; Trino's `date_diff('month', ...)` returns an **INTEGER bigint** of complete months (NOT a count of boundary crossings). If downstream depended on the fractional residual (proration, accrual), compute it yourself with `date_diff('day', d2, d1) / 31.0`. See the LEADING CANONICAL block above. |
...
1. **Trino HAS `now()` — it is an ALIAS for `current_timestamp`.** Verified verbatim at [trino.io/docs/current/functions/datetime.html](https://trino.io/docs/current/functions/datetime.html): *"`now()` → timestamp with time zone — This is an alias for `current_timestamp`."* Both return **`TIMESTAMP(3) WITH TIME ZONE`** as of the **start of the query** (not call time — every reference inside one query yields the identical value), tied to the **session time zone**. The two are interchangeable; pick whichever reads better. Companion forms:
   ...
   - **Syntax pin:** `current_timestamp` / `current_date` / `current_time` / `localtimestamp` take **NO parentheses** (SQL-standard form); `now()` takes empty parens. Writing `current_timestamp()` is a parse error. Concretely: `UPDATE t SET updated_at = current_timestamp WHERE id = 7` ✅ / `UPDATE t SET updated_at = now() WHERE id = 7` ✅ / `UPDATE t SET updated_at = current_timestamp() WHERE id = 7` ❌ PARSE ERROR — do not copy. The dialect-confusion source: Spark SQL, PySpark, and Postgres all accept `current_timestamp()` with empty parens; Trino does not. For the full UPDATE/INSERT/DELETE worked landing block + inline-marked DO-NOT-WRITE rows, see **Resource 07 §LEADING CANONICAL — `now()` / `current_timestamp` / `current_date` in Trino → READ-ME-FIRST `current_timestamp` / `current_date` / `now()` parens rule (iter706 PIN)**.
...
| `LPAD(s, n, pad)` / `RPAD(s, n, pad)` | `lpad(s, n, pad)` / `rpad(s, n, pad)` | **Trino signature is `lpad(varchar, bigint, varchar) -> varchar`** (verbatim per [trino.io/docs/current/functions/string.html](https://trino.io/docs/current/functions/string.html)) — the first argument MUST be a VARCHAR. **UNLIKE Oracle `LPAD`, Trino does NOT implicitly coerce a NUMBER to a string.** For a numeric column (BIGINT / INTEGER / DECIMAL), you MUST `CAST` first: `lpad(CAST(account_id AS VARCHAR), 10, '0')` — same root cause as the `\|\|` / `CONCAT` no-numeric-coercion rule one row below (see also §7A.3.1). **DO-NOT-WRITE:** `lpad(<numeric_col>, n, '0')` directly on a BIGINT/INTEGER/DECIMAL column raises `Unexpected parameters (bigint, integer, varchar(1)) for function lpad. Expected: lpad(varchar, bigint, varchar)`. Keyword anchors: zero-pad number Trino, lpad numeric column, pad account number, lpad cast varchar, rpad numeric. **For padding a STRING to a fixed width (fixed-width flat-file export, right-pad with spaces, truncate-if-longer), see [resource 23 §3.1A "pad a STRING to a FIXED WIDTH" sub-canonical](23-sql-best-practices-olap.md)** — `rpad(product_code, 20, ' ')` pads AND truncates to 20. |
| `LTRIM(s)` / `RTRIM(s)` / `TRIM(s)` | `ltrim(s)` / `rtrim(s)` / `trim(s)` | Identical for the **whitespace** form. **Trino `trim` ALSO has a char-set form** — `trim([LEADING\|TRAILING\|BOTH] [chars] FROM source)` — for stripping a SPECIFIC character (e.g. leading zeros `'00042'`->`'42'`), not just whitespace. See the **§4.3-STRIP-ZEROS — strip leading/trailing characters (not whitespace)** block immediately below the table. **⚠️ TRAILING-SPACE MIGRATION TRAP — if a `WHERE status = 'ACTIVE'` filter (or a string JOIN key) DROPS ROWS after the Oracle→Trino port, the cause is trailing/leading spaces in the migrated data (often from an Oracle `CHAR(n)` source column, which Oracle blank-pads): Trino compares `VARCHAR` EXACTLY (no PAD SPACE), so `'ACTIVE' = 'ACTIVE   '` is FALSE. Fix: `WHERE trim(status) = 'ACTIVE'` (or `trim()` once at staging). Full canonical + the don't-use-LIKE/CAST-to-CHAR defangs: [resource 23 §3.1·STR "Trino VARCHAR comparison is EXACT"](23-sql-best-practices-olap.md).** |
...
| `TRUNC(n, d)` (numeric truncation to `d` decimal places — Oracle) | **`truncate(n, d)`** — lowercase 2-arg, same truncation semantics as Oracle (e.g. `truncate(12.3456, 2)` = `12.34`). Verbose equivalent: `truncate(n * power(10, d)) / power(10, d)`. **If HALF_UP rounding is acceptable, `round(n, d)` is an alternative** (but `round` is rounding, NOT truncation — they differ at the half-way mark and for negative numbers). | See §4.4C immediately below for the full numeric-TRUNC guardrail + DO-NOT-WRITE matrix. Verified at [trino.io/docs/current/functions/math.html](https://trino.io/docs/current/functions/math.html) — Trino has BOTH `truncate(x)` and `truncate(x, d)`; only the uppercase Oracle **name** `TRUNC` is **not registered** in Trino (write lowercase `truncate`). |
...
| `MOD(a, b)` | `mod(a, b)` or `a % b` | **Identical — including the sign of a negative result.** BOTH Oracle `MOD` and Trino `mod`/`%` follow the **DIVIDEND's sign** (truncated division: `m - n*TRUNC(m/n)`): `MOD(-10, 3) = mod(-10, 3) = -10 % 3 = -1` in BOTH engines; `MOD(10, -3) = 1` in both. ❌ **DO-NOT-WRITE "Oracle MOD follows the divisor's sign" / "Oracle MOD(-10,3) = 2"** — that is a COMMON MISCONCEPTION (it's the *floored / Euclidean* modulo of Python `%`, Ruby, Excel `MOD`, not Oracle). Oracle `MOD` and Trino `mod` agree; do NOT rewrite the logic. **If you genuinely want the always-non-negative (floored) result** (e.g. `-10 mod 3 = 2` for a hash-bucket index that must be 0..n-1), wrap it: `((a % n) + n) % n` → `((-10 % 3) + 3) % 3 = 2`. (Oracle's `REMAINDER(m,n)` uses `ROUND` not `TRUNC` and is a different function — Trino has no built-in `remainder`; use the `mod` forms above.) |
...
### 4.4C ORACLE `TRUNC` ↔ TRINO `truncate` GUARDRAIL — three distinct mappings via lowercase `truncate` / `date_trunc`, NO `TRUNC` keyword in Trino
...
|---|---|---|---|
| `TRUNC(n, d)` where `n` is numeric, `d` is decimal places | Truncates `n` to `d` decimal places (drops digits beyond position `d`, toward zero) | **`truncate(n * power(10, d)) / power(10, d)`** — the scale-by-power form, using the **1-arg** `truncate`. This is the canonical 467 way to truncate to `d` places (negative-safe; toward zero, NOT floor). | ⚠️ **Trino 467 `truncate` is 1-ARG ONLY** — the 2-arg `truncate(n, d)` decimal-places overload does **NOT** exist on 467 (it lands ~471+, [#23950](https://github.com/trinodb/trino/pull/23950)). Writing `truncate(n, d)` on 467 is a function-resolution error. Use the `truncate(n*power(10,d))/power(10,d)` form above. (See §4.4B/§4.4C worked example + DO-NOT-WRITE.) The Iceberg partition transform `truncate(col, W)` is a DIFFERENT, valid 2-arg construct — that's a partitioning spec, not the math function. |
...
> | Date → string | `TO_CHAR(dt, 'YYYY-MM-DD')` (Oracle UPPERCASE mask) | Oracle | Trino HAS `to_char` (Teradata, lowercase-numeric-only) but the Oracle UPPERCASE mask + month names don't translate | `date_format(dt, '%Y-%m-%d')` (MySQL) or `format_datetime(dt, 'yyyy-MM-dd')` (Joda) |
...
> | Iceberg compression on Trino 467 | `WITH (..., "write.parquet.compression-codec" = 'zstd')` | native Iceberg property key (Spark / Iceberg Java API) | Parse error — that key is the native Iceberg name, not a Trino WITH property on any Trino version | **Trino 467 has NO per-table compression property** — set the **catalog config** `iceberg.compression-codec=ZSTD` (default ZSTD) in `etc/catalog/iceberg.properties` (cluster-wide). **The per-table `compression_codec` property was added in Trino 477** ([trinodb/trino #25755](https://github.com/trinodb/trino/pull/25755)); the per-session `iceberg.compression_codec` is **Trino 473+** ([#24851](https://github.com/trinodb/trino/pull/24851)). On 467, `WITH (..., compression_codec='ZSTD')` and `SET PROPERTIES compression_codec='ZSTD'` BOTH error with `Catalog 'iceberg' table property 'compression_codec' does not exist`. **dbt-trino on 467 cannot set compression per-model via a table property** — it is catalog-level only; for Spark-written tables use Spark `SET TBLPROPERTIES ('write.parquet.compression-codec'='zstd')`. |
...
> **INSTALLING dbt_utils + the "`'dbt_utils.generate_surrogate_key' is undefined` / can't resolve the macro" troubleshooting card.** Keyword anchors: *dbt_utils not found, generate_surrogate_key undefined / can't resolve / not defined / "macro 'generate_surrogate_key' not found in any package", dbt deps ran/succeeded but macro missing/invisible at runtime, packages.yml setup, how to install dbt_utils, dbt package macro won't resolve, 'dbt_utils' is undefined.*
>
> **#1 CAUSE WHEN `dbt deps` SUCCEEDED but you get "macro `generate_surrogate_key` not found in any package" — it is a VERSION mismatch, NOT a setup/syntax problem.** `generate_surrogate_key` was introduced in **dbt_utils 1.0.0** (it RENAMED the old `surrogate_key` macro); on any pinned dbt_utils **< 1.0.0** the macro literally does not exist under that name, so dbt reports it "not found in any package" even though `dbt deps` installed the package fine. **FIX: pin `version: [">=1.1.0","<2.0.0"]` in packages.yml and re-run `dbt deps`** (or, on a legacy pin you cannot bump, call the old `dbt_utils.surrogate_key` — note its NULL-handling differs). Do NOT tell the engineer their `{{ ... }}` call is wrong, and do NOT invent a "macro import" line — the call site is fine; the package version is the bug. (Full cause table below.)
>
...
| `SELECT * FROM (SELECT a.*, ROWNUM rnum FROM (SELECT * FROM events ORDER BY event_id DESC) a WHERE ROWNUM <= 100) WHERE rnum >= 51` | Valid syntax | **CANONICAL Oracle 11g pagination pattern.** Two-level inline view: inner sorts, middle assigns ROWNUM and caps at upper bound, outer filters to lower bound. Returns rows 51-100 in sorted order. | `SELECT * FROM events ORDER BY event_id DESC OFFSET 50 LIMIT 50`. ⚠️ Trino requires **`OFFSET` BEFORE `LIMIT`** (clause order is `ORDER BY` → `OFFSET` → `LIMIT`/`FETCH`). Unlike PostgreSQL/MySQL/Oracle-12c-FETCH muscle memory, **`LIMIT 50 OFFSET 50` (LIMIT before OFFSET) is a PARSE ERROR in Trino 467** — `mismatched input 'OFFSET'`. Always write `OFFSET m LIMIT n`. |
...
`accepted_values` compiles to roughly `SELECT <col> FROM <model> WHERE <col> NOT IN ('v1', 'v2', ...)` (grouped on the value) — **rows returned = failures; zero rows = pass** (same pass/fail convention as every other dbt generic test). **CRITICAL NULL TRAP — `accepted_values` does NOT catch NULL.** dbt does NOT emit an explicit `IS NOT NULL` filter; instead, because `NULL NOT IN (...)` evaluates to **UNKNOWN** (not TRUE) under SQL three-valued logic, a NULL value is silently EXCLUDED from the failing-rows set and **PASSES** the test (dbt docs: `accepted_values` validates only the **non-null** values). So `accepted_values` alone will catch an out-of-set literal like `'none'` but will MISS a NULL. To reject BOTH, pair it with `- not_null` on the same column (`- accepted_values: {values: [...]}` AND `- not_null`). The values list is single-quoted by default. Verbatim from [docs.getdbt.com/reference/resource-properties/data-tests](https://docs.getdbt.com/reference/resource-properties/data-tests): *"The `accepted_values` test supports an optional `quote` parameter which, by default, will single-quote the list of accepted values in the test query. To test non-strings (like integers or boolean values) explicitly set the `quote` config to `false`."* So for numeric or boolean columns, omitting `quote: false` makes dbt emit `WHERE status_id NOT IN ('1', '2', '3', '4')` — Trino will silently or noisily mis-compare depending on type-coercion rules, and the test result is unreliable. The fix is one line:
...
> **READ THIS FIRST if your question contains any of these keywords: `source freshness`, `freshness`, `loaded_at_field`, `loaded_at`, `warn_after`, `error_after`, `dbt source freshness` command, `sources.yml`, `stale source`, `does freshness block downstream`, `freshness CI`.**
>
> **ALSO READ THIS FIRST for the same problem described as a NARRATIVE (no feature-name keyword):** "our ingestion/Kafka/Spark pipeline silently fell behind and dashboards show stale numbers", "the `dbt run` / `dbt build` SUCCEEDS but the source data hasn't been updated in N hours", "no visible error when the upstream/raw/source table is stale or hasn't refreshed", "the raw Iceberg table written by Kafka/Spark is X hours old and nothing flagged it", "how do I make dbt FLAG or FAIL when source data is too old / stale / behind", "detect upstream lag / late-arriving source / source not updated". **The answer to ALL of these is dbt source freshness (`freshness:` + `loaded_at_field:` + the separate `dbt source freshness` command) — declared below. Do NOT hedge or defer to external docs: the full YAML + command + worked example are in THIS block.**
>
> This block is the canonical reference for declaring and checking source staleness in dbt-trino on this stack. All claims below are verified at [docs.getdbt.com/reference/resource-properties/freshness](https://docs.getdbt.com/reference/resource-properties/freshness), [docs.getdbt.com/reference/commands/source](https://docs.getdbt.com/reference/commands/source), and [docs.getdbt.com/docs/deploy/source-freshness](https://docs.getdbt.com/docs/deploy/source-freshness) (WebFetched 2026-06-05).
...
| "Does it fail at build time or query time?" | **Build/run time** (during `dbt run` / `dbt build`'s preflight check), NOT at end-user query time. The Trino engine itself does not *store/enforce* the contract — dbt does, but the preflight IS warehouse-interactive (it queries Trino to learn the actual column types) and does not *materialize* the model if it fails. | § What a contract is and when it fires |
| "Does contract enforcement need a LIVE Trino connection? Can a CI that only runs `dbt parse` / `dbt compile` (no warehouse) catch a contract violation?" | **It NEEDS a live Trino connection. NO — `dbt parse`/`dbt compile` with no warehouse access do NOT enforce the contract.** dbt discovers the model's ACTUAL result-set column names + types by issuing an introspection query against Trino (a `SELECT … WHERE 1=0` / empty-result describe over the compiled model SQL) and comparing that to the YAML; with no connection it cannot learn the real types, so the data-type check never runs. To gate merges on contracts in CI, the CI job must run `dbt build` (or `dbt run`) against a reachable Trino. | § What a contract is and when it fires |
| "Do constraints (not_null, primary_key, etc.) get enforced on dbt-trino?" | **Only `not_null` is enforced** on dbt-trino + Iceberg (Trino translates it to an Iceberg NOT NULL column constraint). `primary_key`, `foreign_key`, `unique`, `check` are **definable in YAML but NOT runtime-enforced** by Trino — they are recorded as metadata only. The HARD guarantee is the column-name + data_type build-time check, which fires regardless of platform (and is warehouse-interactive — see the live-connection row above). | § Constraints on dbt-trino — what's enforced vs definable |
...
A model contract is a **schema lock** declared in the model's YAML (the `.yml` file next to the model `.sql` file under `models/`). When `contract.enforced: true`, dbt runs a **warehouse-interactive "preflight" check at the start of the model's build** (during `dbt run` / `dbt build`), before materializing the model:

1. It compiles the model's SELECT body.
2. It issues an **introspection query against Trino** — an empty-result describe over the compiled SELECT (effectively `SELECT … WHERE 1=0` / `LIMIT 0`) — so Trino reports back the SELECT's ACTUAL result-set column names + data types. **This step requires a live Trino connection.**
3. It compares those actual columns + types against the YAML-declared `columns:` list.
4. If ANY column is missing, extra, or has the wrong type, **dbt errors and refuses to MATERIALIZE the model** — no `CREATE TABLE`/`MERGE` is run, so production data is never touched. (Trino IS contacted for the introspection in step 2; what's prevented is the materialization.)

This is a **build/run-time check** (during `dbt run` / `dbt build`), NOT an end-user-query-time check — and crucially **NOT a pure offline compile-time check**: because step 2 must ask Trino for the real result-set types, `dbt parse` / `dbt compile` with NO warehouse connection **cannot** enforce the data-type contract. A lightweight CI that only runs `dbt parse`+`dbt compile` will NOT catch a contract violation; gate merges with `dbt build` (or `dbt run`) against a reachable Trino. The Trino engine does not itself *store/enforce* model contracts — dbt orchestrates the check via the dbt-trino adapter. The contract is a dbt-core feature available on every adapter (Snowflake, BigQuery, Postgres, Spark, Databricks, dbt-trino, etc.), not a Trino-specific mechanism.

> **Contract vs `not_null` DATA rejection — two different mechanisms, both can fail the build.** A dbt **CONTRACT** is a STRUCTURAL preflight check at BUILD time: it compares the SELECT's projected column **names + `data_type:` + declared constraint DDL** against the YAML, and errors before the model is **materialized** — it does **NOT scan data values**. (The preflight is warehouse-interactive — dbt issues an empty-result introspection query to Trino to learn the actual result-set types, so it needs a live connection; it just doesn't run the `CREATE`/`MERGE` if the types don't match. It is NOT a pure offline compile-time check.) The **NOT-NULL DATA rejection** (a NULL row hitting a `not_null`-constrained column) is a separate event that fires at **INSERT/MERGE time** inside Trino, because dbt-trino translated the `not_null` constraint into an Iceberg `NOT NULL` column DDL — Iceberg rejects the offending row at commit (verified at [docs.getdbt.com/reference/resource-configs/trino-configs](https://docs.getdbt.com/reference/resource-configs/trino-configs): *"only constraints with `type` as `not_null` are supported"*; `primary_key` / `unique` are metadata-only / not enforced on dbt-trino). Same `dbt build` can fail either way, but the layer is different — STRUCTURAL (contract preflight, no data scan) vs DATA (Iceberg NOT NULL write-side rejection).
...
**Crucially: no production data is written when the contract fails.** The preflight runs BEFORE Trino executes the model's *write* SQL — no `INSERT INTO ...` / `MERGE INTO ...` / `CREATE TABLE AS ...` reaches Trino, so "No changes were applied to the warehouse." (dbt DOES send Trino a small empty-result introspection query during the preflight to read the actual result-set types — that is how it detects the `varchar` vs `bigint` mismatch — so a live Trino connection is required; what's gated is the *materialization*, not all warehouse contact. A CI with no Trino connection therefore cannot run this preflight at all.) This is the load-bearing operational guarantee.
...
| `CONTRACT` as a SQL keyword (e.g., `CREATE TABLE ... WITH CONTRACT (...)`) | **FABRICATED.** There is no `CONTRACT` SQL keyword in Trino or in dbt's compiled SQL. The contract check is orchestrated by dbt at build time; the model's `CREATE`/`MERGE` SQL is not run if it fails (though dbt DOES send Trino a small empty-result introspection query to read the actual result types — see the live-connection note below). |
| "The contract is enforced at query time by Trino" | **WRONG (but don't over-correct to 'no Trino contact at all').** The contract is enforced at dbt BUILD/RUN time, not at end-user Trino query time, and Trino does not *store* the contract. BUT dbt's preflight IS warehouse-interactive: it sends Trino an empty-result introspection query (`SELECT … WHERE 1=0`) over the compiled model SELECT to learn the actual column types, then compares to YAML and refuses to MATERIALIZE on mismatch. So a live Trino connection IS required; what's prevented on failure is the materialization, not the introspection. |
| "Contract validation is a pure compile-time check — a CI running only `dbt parse`/`dbt compile` (no warehouse) will catch contract violations" | **WRONG.** Contract enforcement needs a LIVE warehouse connection: dbt must ask Trino for the model SELECT's actual result-set column types (an empty-result introspection query) to validate `data_type:`. `dbt parse`/`dbt compile` with no Trino connection do NOT enforce the contract and will NOT fail on a type/column mismatch. To gate merges on contracts in CI, run `dbt build` (or `dbt run`) against a reachable Trino. |
...
> **Keyword anchors:** dbt select comma vs space, dbt tag AND OR, dbt intersection union selector, dbt build multiple tags BOTH, dbt --select set operators, dbt graph operators +model model+. **Plain-question anchors (READ THIS FIRST too):** "dbt run rebuilds everything / takes 40 minutes / how do I run only a subset", "how do `--select` and `--exclude` work", "run just ONE FOLDER of models", "run a model plus everything DOWNSTREAM of it", "skip / exclude certain models from a run", "run only the models I changed", "run one model and its children". Verified at [docs.getdbt.com/reference/node-selection/set-operators](https://docs.getdbt.com/reference/node-selection/set-operators) and [docs.getdbt.com/reference/node-selection/graph-operators](https://docs.getdbt.com/reference/node-selection/graph-operators) and [docs.getdbt.com/reference/node-selection/exclude](https://docs.getdbt.com/reference/node-selection/exclude).
>
> **⭐ DIRECT ANSWER to "run just one folder + everything downstream of it" (the most common ask):** use the `path:` selector method + the trailing-`+` (downstream) graph operator:
> ```bash
> dbt run --select "path:models/marts/finance+"   # every model UNDER models/marts/finance, PLUS all their downstream children
> dbt run --select "path:models/staging"          # just that folder (no relatives)
> dbt run --select "+path:models/marts/finance"   # that folder + its UPSTREAM parents (so the inputs build first)
> ```
> (`path:` matches the directory; the `+` placement is the same as for a model name — `name+` = downstream, `+name` = upstream, `+name+` = both. You can also use `fqn:` or, if the folder has a `+tags:` config, `tag:`.)
>
> **`--exclude` — the inverse of `--select` (run/build everything EXCEPT the matched nodes).** Same selector syntax (paths, tags, graph operators, set operators). Combine with `--select`:
> ```bash
> dbt run                                         # everything (the slow default the engineer wants to avoid)
> dbt run --exclude "path:models/staging/legacy"  # everything EXCEPT that folder
> dbt run --select "tag:nightly" --exclude "tag:wip"   # nightly models, but skip work-in-progress ones
> dbt run --exclude "model_a+"                    # everything except model_a and its downstream
> ```
> Selection is **restrictive by default** — `--select` runs ONLY what matches (it does NOT auto-pull upstreams unless you add a leading `+`); `--exclude` subtracts from whatever `--select` chose (or from the full DAG if no `--select`).
...
- **Impact analysis** — `dbt ls --select model:fct_orders+ --resource-type exposure` lists every exposure (dashboard/ML pipeline) downstream of `fct_orders`, so you know who to notify before a breaking change. **Note the `+` goes AFTER the model (`model:fct_orders+`) — a TRAILING `+` selects DESCENDANTS (downstream consumers, which is where exposures live). A LEADING `+` (`+model:fct_orders`) selects ANCESTORS (upstream) and would return NO exposures (exposures are never upstream of a model). Mnemonic: `+` points the direction of the arrow — `name+` = "name and everything after it" (downstream), `+name` = "everything before it and name" (upstream).** ❌ DO-NOT-WRITE `dbt ls --select +model:fct_orders --resource-type exposure` for impact analysis — the leading `+` is the wrong direction and returns an empty set.
...
#### 7A.3.1 Trino dialect landmine — CONCAT and `||` require all-VARCHAR args (NO implicit numeric/date coercion); use `format()` to avoid CASTing every column

> **Findability anchor (read FIRST if your question contains any of these):** Oracle `||` throws "cannot concatenate varchar and integer" / "Unexpected parameters (varchar, integer) for function concat" in Trino; `'Account #' || account_id` type error where account_id is INTEGER/BIGINT; is there a Trino concat function that handles MIXED types; build a display label/string from numeric + string columns without `CAST(... AS VARCHAR)` on every piece; does `concat()` auto-coerce numbers (NO — `concat()` is varchar-only too, same as `||`); shorter alternative to casting every integer in a concat; printf-style string building. **The two answers: (A) `CAST(<numeric> AS VARCHAR)` on each non-string piece, or (B) — usually cleaner — `format('Account #%s — active for %s days', account_id, days_since_login)` (printf-style; `%s` accepts ANY type, `%d` a bigint, `%,.2f` a decimal — NO per-column CAST needed). See the `format()` LEADING CANONICAL at [resource 23 §3.1A](23-sql-best-practices-olap.md). `concat()` is NOT a mixed-type escape hatch — it requires all-VARCHAR args exactly like `||`.**
