...
|---|---|
| "Partitioning by `tenant_id` automatically prevents cross-tenant data leaks at query time." | **NO — partitioning is a STORAGE LAYOUT, not an access-control mechanism.** Partition pruning makes per-tenant queries fast (skips other tenants' files for I/O), but a query `SELECT * FROM events` (no WHERE) still reads every partition and returns every tenant's rows. Isolation must come from (a) Trino views that hard-code the filter, (b) Trino system access control (file-based rules or OPA) that rejects cross-tenant queries, or (c) per-tenant schemas/tables. Never rely on the partitioning alone. |
| "`bucket(tenant_id, N)` partitioning balances large tenants across workers." | **NO — Iceberg's `bucket()` hashes each distinct value to exactly ONE bucket.** All rows for `tenant_id='acme'` still hash to the same bucket and still land on the same worker at GROUP BY time. The hash partitions the SET OF TENANTS across N buckets, not the rows of a single tenant. For whale-tenant skew, use salt + two-level GROUP BY (see resource 18 Step 5), a dedicated whale table, or a nightly rollup — NOT `bucket()`. |
| "Trino views with `current_user` will safely filter the underlying table for me." | **WORKS only if the view is configured correctly.** A Trino view by default runs with the **DEFINER's** privileges (the user who created the view), so the view's underlying query runs as that high-privilege user — but the `WHERE` clause inside the view body controls what rows reach the caller. The pattern is: `CREATE VIEW tenant_events AS SELECT * FROM events WHERE tenant_id = current_user`. Grant SELECT on the view to user accounts that map 1:1 to `tenant_id`. **Caveats**: (a) Trino has both DEFINER and INVOKER security modes — verify you set `SECURITY DEFINER` explicitly; (b) the user-to-tenant_id mapping must come from a trusted source like JWT claim — not from a user-controllable parameter; (c) on this stack, OPA is the auth backend so views often layer on top of OPA-enforced row filtering. See [Trino CREATE VIEW docs](https://trino.io/docs/current/sql/create-view.html). |
| "File-based access control rules (`security.config-file`) are the production-recommended way on this stack." | **NO — on this stack OPA is the authorization backend.** Per prod_info.md, this prod uses **Open Policy Agent (OPA)** plugin for Trino authorization. File-based access control rules in resources/ are conceptual illustrations only — they do NOT reflect the prod setup. For specific permission rules and user-to-tenant mapping, defer to the **external governance document** (not in this repo). Resources here cover general OPA-on-Trino concepts only. |
| "Hashing tenant_id with `bucket(tenant_id, N)` will hide tenant identity from MinIO storage paths." | **NO — the bucket NUMBER appears in file paths, not the tenant name. But that's NOT a security feature.** Anyone with MinIO read access still sees `bucket=42/00000-0-...parquet` and can read the rows inside, which contain the literal `tenant_id` column. If you need tenant data to be unreadable from MinIO directly, the right answer is encryption at rest (MinIO server-side encryption with per-tenant keys) or per-tenant prefixes with MinIO bucket policies — not partition bucketing. |
| "Row-level filters in Iceberg are enforced by the storage layer." | **NO — row-level filters in this stack are enforced by Trino (the query engine), not by Iceberg or MinIO.** OPA + Trino can apply row filters and column masks; Spark and direct MinIO access bypass them entirely. If your threat model includes ingestion engineers with direct Spark access reading other tenants' rows, row-level filtering at Trino is insufficient — you need separate namespaces / tables / MinIO prefixes with separate credentials. |
| "Resource groups give per-tenant memory isolation." | **PARTIALLY TRUE — resource groups give per-tenant QUERY CONCURRENCY and CPU/memory budgeting, but NOT per-row isolation.** Resource groups assign queries from user X to a soft/hard CPU and memory budget; they do not control which rows X can read. Combine resource groups (for noisy-neighbor performance isolation) WITH access control (for data isolation) — they solve different problems. See [Trino resource groups docs](https://trino.io/docs/current/admin/resource-groups.html). |
| "Per-tenant schemas (Model 1) are always safer than partitioned shared tables (Model 2)." | **GENERALLY YES for strict isolation, but with two caveats.** (a) HMS performance degrades with many thousands of tables — at 500+ tenants per-namespace becomes a metastore-scaling problem. (b) Schema changes have to be replayed across every namespace, which is operationally painful. The right model depends on tenant count + isolation strictness + ops budget — see "The three isolation models" section below. **For 50-500 tenants with normal SaaS schemas, Model 3 (shared tables + Trino access control + OPA on this stack) is the default recommendation.** |

> **Why these specific myths matter.** Each is a load-bearing claim about how tenant isolation works on Iceberg + Trino. Stated as an absolute, they cause engineers to either over-engineer (one schema per tenant for 1000 tenants because they thought partitioning wasn't isolating) OR to confidently leak data (assuming `tenant_id` partitioning alone prevents cross-tenant reads, or that bucket partitioning hides tenant identity at the storage layer). **The correct discipline:** when about to say "tenant isolation is guaranteed by X", check (a) what mechanism X is (storage layout vs access control vs encryption), (b) which threat it actually addresses, and (c) whether the prod stack uses it (OPA vs file rules on this stack).
...
>
> **Official Trino OPA plugin precedence rule — quote it verbatim.** The Trino OPA access control documentation states this precedence explicitly: **"If `opa.policy.batch-column-masking-uri` is set it overrides the value of `opa.policy.column-masking-uri`."** This is the exact language in the upstream docs (Trino OPA access control properties reference). Practical reading: when both URIs are configured (the recommended production pattern), Trino routes per-table column-masking calls to the batch endpoint and does NOT additionally call the single-column endpoint for the same masking decision — the batch URI takes precedence for the columns it covers. The single-column URI still serves as the fallback path when `batch-column-masking-uri` is **unset** entirely. Cite this sentence in design docs or PR reviews when someone asks "what happens if both are configured?" — the answer is in one upstream sentence and you do not need to infer it from behavior.
>
> **Trino coordinator restart is REQUIRED after any change to `opa.policy.*` properties.** This is the single most common deployment footgun for OPA wiring. The `etc/access-control.properties` file (where every `opa.policy.uri`, `opa.policy.batched-uri`, `opa.policy.row-filters-uri`, `opa.policy.column-masking-uri`, and `opa.policy.batch-column-masking-uri` lives) is read by the Trino coordinator **only at startup** — it is NOT hot-reloaded. If you add `opa.policy.batch-column-masking-uri=...` to the file and then re-run your masking integration test, the test will use the **previous** configuration (no batch endpoint) and the test will still hit the per-column path. The OPA pod may even show no new traffic on the batch handler. Engineers commonly conclude "my Rego is wrong" or "the property name must be different" and spend hours debugging — when the real fix is `kubectl rollout restart deployment/trino-coordinator` (or the equivalent Helm/manifest action for your deployment) and re-running the test against the freshly-restarted coordinator. **Operational rule: any edit to `etc/access-control.properties` (any `opa.policy.*` line, the `access-control.name` line, or any new property) requires a coordinator restart before the change takes effect.** Workers do not need to restart for access-control property changes; only the coordinator reads this file.
...
>
> **WARNING — JWT auth does NOT populate groups on this stack (production gotcha).** `userGroup` reads from `input.context.identity.groups` (the same list OPA sees). On the production JWT auth stack, **`groups` is NOT populated from a JWT claim** — OSS Trino 467's JWT authenticator extracts ONLY the username (`sub` claim) from the token and ignores every other claim, including any `groups`/`roles`/`memberOf` array you put in the JWT. **Even if your JWTs contain a `groups` claim, `userGroup` selectors will silently never match unless you ALSO configure a separate group provider.** The groups list is filled by a **separately configured group provider** in `etc/group-provider.properties`:
>
> ```properties
...
> **MEMORY LIMITS specifically — `query_max_memory` (session property) vs `softMemoryLimit` (resource-group ceiling) vs `query.max-memory` (cluster ceiling).** The memory side of tier enforcement has three distinct knobs that engineers routinely conflate. Get this wrong and a tenant either bypasses your tier limit or you accidentally cap legitimate admin work:
>
> | Knob | Where it lives | What it caps | Can `SET SESSION` change it? | Enforcement |
> |---|---|---|---|---|
> | **`query_max_memory`** | Session property — `SET SESSION query_max_memory = '20GB'` | **Per-query** distributed memory (sum across all workers for one query) | **YES — but only DOWNWARD.** A client can `SET SESSION query_max_memory = '5GB'` to voluntarily lower their own cap. They **cannot** raise it above the cluster-level `query.max-memory` value (Trino rejects the override with an error). | The Trino coordinator kills the query when its distributed memory crosses the effective per-query cap. |
> | **`softMemoryLimit`** (resource group) | `etc/resource-groups.json` per group | **Aggregate cluster memory across ALL concurrent queries in the group** (e.g., total bytes held by every query landed in `global.tenant_acme`) | **NO.** No session property raises a resource group's `softMemoryLimit`. The cap is evaluated by the coordinator's admission controller — new queries queue when the group is at its cap. | Server-side at query-submission time; cannot be bypassed by any client-side state. |
> | **`query.max-memory`** | `etc/config.properties` (coordinator) | The **absolute cluster-wide ceiling** for any single query, regardless of group | **NO** — and this is what makes `query_max_memory` safe to expose. Even if a tenant runs `SET SESSION query_max_memory = '500GB'`, Trino rejects it because it exceeds the configured `query.max-memory`. | Hard ceiling enforced at session-property validation. |
>
> **Why this matters for the question "can a tenant bypass my memory limit?":**
> - If your tier limit is the **resource group's `softMemoryLimit`** (e.g., free tier gets 20% of cluster memory in aggregate): **NO**, a tenant cannot bypass it. There is no session property that raises a resource group cap. They can run `SET SESSION query_max_memory = '...'` all day; it has zero effect on the group's aggregate budget.
> - If your tier limit is a **per-query cap you intended to enforce via `query_max_memory` set by the session property manager** (e.g., free tier gets at most 5GB per query as a default): **YES, partially** — a tenant can `SET SESSION query_max_memory = '<larger value>'` and override the default, **up to** the cluster-level `query.max-memory`. To make the session-property-manager default behave like a per-tier per-query ceiling, add an OPA deny on `SetSystemSessionProperty` for `query_max_memory` (same pattern as `query_max_execution_time`).
>
> **Practical recommendation:** for per-tier memory enforcement on this stack, prefer the **resource group `softMemoryLimit`** as your primary lever (it's a true ceiling, no OPA needed) and add `query.max-memory-per-node` in `etc/config.properties` as the defense-in-depth per-query-per-worker cap. Use `query_max_memory` via the session property manager only as a *suggested default* for client behavior — and only treat it as a true cap if you've also deployed the OPA `SetSystemSessionProperty` deny rule for `query_max_memory`. The full lockdown chain for memory is: resource group `softMemoryLimit` (group cap) + `query.max-memory-per-node` (per-worker cap) + optional OPA-enforced `query_max_memory` (per-query cap). Skipping the OPA piece is fine if you're already capped at the group and per-node levels — it's not fine if `query_max_memory` is your *only* per-query cap.

> **CRITICAL — admission control vs mid-flight kill: which knob stops a query that's ALREADY RUNNING?** This is the most common follow-up confusion after the three-knob table above, and it has a sharp practical answer. The three memory knobs do **different jobs at different points in a query's lifecycle**, and engineers debugging a live "why didn't my runaway free-tier query get killed?" incident routinely pick the wrong lever.
>
> | Knob | Enforcement timing | Does it KILL an already-running query? |
> |---|---|---|
> | **`softMemoryLimit`** (resource group) | **Admission control** — evaluated by the coordinator BEFORE a new query is allowed to start | **NO.** Once a query is admitted and running, `softMemoryLimit` does not touch it. New queries from the same group **queue or are rejected** while the group is over budget; in-flight queries keep running with whatever memory they hold. The runaway tenant query that's already consuming 50GB will keep consuming 50GB until it finishes naturally — `softMemoryLimit` only prevents the NEXT free-tier query from starting. |
> | **`query.max-memory`** (cluster ceiling, `etc/config.properties`) | **Per-query, in-flight** — checked continuously as each query accumulates memory across workers | **YES.** When any single query's distributed memory crosses `query.max-memory`, the coordinator kills it mid-flight with `EXCEEDED_MEMORY_LIMIT`. This is the only memory knob in the table above that terminates an already-running query. |
> | **`query.max-memory-per-node`** (per-worker, `etc/config.properties`) | **Per-query-per-worker, in-flight** — checked continuously on each worker | **YES.** When a query's memory on any single worker crosses the per-node cap, the coordinator kills it with `EXCEEDED_MEMORY_LIMIT`. Defense-in-depth against pathological skew where one worker holds disproportionate memory. |
>
> **Phrased another way: "queue vs kill."** `softMemoryLimit` is a **queue** — it makes new queries wait. `query.max-memory` and `query.max-memory-per-node` are **kills** — they terminate running queries that breach the per-query budget. If your incident is "tenant X's query is RIGHT NOW eating the cluster, why wasn't it stopped?", the answer is almost never `softMemoryLimit` — it's that you don't have a tight enough `query.max-memory` (or `query.max-memory-per-node`). Tightening `softMemoryLimit` will *prevent the next 10 queries from that tenant from starting*, but it will not interrupt the one currently running.
>
> **What this means for the design:** to fully cover the "noisy free-tier tenant" failure mode, deploy **both** layers together:
> 1. **Resource group `softMemoryLimit`** (e.g., `"softMemoryLimit": "20%"` on the `free_tier` group) — bounds the aggregate memory the entire free tier can hold at once. Stops new queries from piling on when the group is already at budget. This is the *throttle*.
> 2. **`query.max-memory`** (e.g., `query.max-memory=10GB` in `etc/config.properties`) — bounds the largest individual query anywhere in the cluster. Kills mid-flight runaway queries before they fill the group budget single-handedly. This is the *circuit breaker*.
> 3. **`query.max-memory-per-node`** (e.g., `query.max-memory-per-node=2GB`) — bounds per-worker memory for any one query. Kills queries with pathological data skew before they OOM a worker. This is the *per-worker safety net*.
>
> If you only deploy `softMemoryLimit` and a 50GB runaway lands in the free-tier group, it admits successfully (group was under 20% at admission time), then balloons to 50GB while running, and `softMemoryLimit` does nothing — only the next free-tier query that tries to start gets queued. Without `query.max-memory`, you have no in-flight kill switch, and your only recourse is `CALL system.runtime.kill_query(...)` from an admin session. Always set both.
>
> **For the "kill this RIGHT NOW" case during an incident:** even with `query.max-memory` deployed, the operational tool to terminate a specific runaway query immediately is `CALL system.runtime.kill_query(query_id => '...')` from an admin user — see the "Immediate-relief tool" callout immediately below.
...
> **CoW vs MoR at DELETE time — what changes by ENGINE on this stack.** Iceberg supports two physical write modes for DELETE, but on this stack the choice is largely determined by **which engine ran the DELETE**, not just the table property. The 4-step purge sequence is **always required** for GDPR compliance — the mechanism in Step 1 differs, but Steps 2-4 are mandatory either way.
>
> - **Trino 467 DELETE (the most common case on this stack).** Trino 467's Iceberg writer is **merge-on-read only regardless of `write.delete.mode`** — see [trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272) ("Today Iceberg writes only support merge-on-read mode"). The DELETE writes small **position-delete files** that mark which rows to skip at read time; the original Parquet data files are completely untouched. Step 2 (`rewrite_data_files`) is **essential** — without it, the current snapshot still references the original data files which physically contain the deleted rows on MinIO. **This is true even when `SHOW CREATE TABLE` reports `write.delete.mode = 'copy-on-write'`** — Trino's writer ignores the property.
> - **Spark DELETE on a Copy-on-Write table (the Iceberg 1.5.2 library default for `write.delete.mode`).** `DELETE FROM` rewrites all affected Parquet data files immediately, producing new files without the deleted rows. The new snapshot points at the rewritten files directly. Step 2 (`rewrite_data_files`) may not be strictly necessary in this mode — the data files are already clean — but running it is still safe and cleans up any residual delete files. **The previous snapshot still references the original (pre-rewrite) data files on MinIO, so steps 3 and 4 remain required to physically remove those bytes.**
> - **Spark DELETE on a Merge-on-Read table (`write.delete.mode = 'merge-on-read'` explicitly set via Spark TBLPROPERTIES).** Same as the Trino case above — writes position-delete files, Step 2 essential.
>
> The table property `write.delete.mode` is controlled by Spark-side `ALTER TABLE ... SET TBLPROPERTIES` and only governs **Spark-executed** DELETE behavior. On this stack, **assume MoR for any DELETE that touched the table from Trino** (because Trino's writer is MoR-only regardless of property), and **always run step 2** — it is a no-op cost on Spark-CoW tables and a correctness requirement everywhere else. The 4-step sequence (DELETE → rewrite → expire → orphan-sweep) is the same on either mode. **DO-NOT-WRITE:** "Trino DELETE uses CoW by default" — FALSE on Trino 467.
>
...
> ```
>
> **There is NO Trino-native equivalent for setting `write.delete.mode` / `write.update.mode` / `write.merge.mode`.** A common wrong instinct is to reach for `ALTER TABLE iceberg.analytics.events SET PROPERTIES delete_mode = 'merge-on-read'` — but `delete_mode` (and `write_delete_mode`) are NOT in Trino's Iceberg table-property list. Per [trino.io/docs/current/connector/iceberg.html](https://trino.io/docs/current/connector/iceberg.html), the documented Trino 467 Iceberg properties are: `format`, `partitioning`, `sorted_by`, `location`, `format_version`, `max_commit_retry`, `delete_after_commit_enabled`, `max_previous_versions`, `orc_bloom_filter_columns`, `orc_bloom_filter_fpp`, `parquet_bloom_filter_columns`, `object_store_layout_enabled`, `data_location`, `extra_properties` — none of the write-mode properties are exposed (and `compression_codec` is NOT a 467 property either — 477+, [#25755](https://github.com/trinodb/trino/pull/25755); on 467 compression is the catalog config `iceberg.compression-codec`, default ZSTD). **The Iceberg-native property name is `write.delete.mode`, set from Spark SQL** (via the `ALTER TABLE ... SET TBLPROPERTIES (...)` form shown above). At Trino CREATE-TABLE time only, you can passthrough `extra_properties => MAP(ARRAY['write.delete.mode'], ARRAY['merge-on-read'])` to stamp the property in metadata — but that does NOT change Trino's writer behavior, only what Spark honors on its own writes. **And this matters less than it sounds: Trino 467's Iceberg writer is MoR-only regardless of the table property** ([trinodb/trino#17272](https://github.com/trinodb/trino/issues/17272) — "Today Iceberg writes only support merge-on-read mode"). So a DELETE run from Trino on this stack already produces position-delete files whether the property is set or not; the 4-step GDPR sequence (DELETE → rewrite → expire → orphan-sweep) is the operationally correct path on either mode (see r22 §13.x for the locked guardrail).
>
