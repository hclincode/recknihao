...
5. **Schedule compaction from day one.** From **Trino 467** (the query engine in this stack) the canonical compaction is `ALTER TABLE iceberg.analytics.events EXECUTE optimize(file_size_threshold => '256MB')` — run nightly. From **Spark** (the ingestion engine) the equivalent is `CALL iceberg.system.rewrite_data_files(table => 'analytics.events', options => map('target-file-size-bytes','268435456'))`. The two are NOT interchangeable: pasting the Spark `CALL` form into Trino fails with `Procedure not registered`, and pasting the Trino `ALTER TABLE ... EXECUTE` form into Spark is a syntax error. **Equally important: the Trino `file_size_threshold` value is a DataSize STRING WITH a unit suffix (`'128MB'` / `'256MB'` / `'512MB'` / `'1GB'`) — NOT bare bytes. Writing `file_size_threshold => '134217728'` (bare-bytes string) or `=> 134217728` (bare integer) FAILS at parse time with a Trino DataSize parse error.** 134217728 bytes = `'128MB'`. The Spark `target-file-size-bytes` options-map key, by contrast, IS a bare-bytes string per Iceberg spec — that's why the two engines look different. Without compaction, you accumulate thousands of tiny files and queries crawl. See [resource 17 — Iceberg table maintenance](17-iceberg-table-maintenance.md) for the full Trino-EXECUTE-vs-Spark-CALL disambiguation matrix and the DataSize-unit-suffix inoculation.
...
- **Forgetting compaction.** Spark writes one Parquet file per task. After 30 days you have 50,000 tiny files. Queries scan all of them. Schedule compaction nightly from day one — on **Trino 467** that is `ALTER TABLE iceberg.analytics.events EXECUTE optimize(file_size_threshold => '256MB')`; on **Spark** it is `CALL iceberg.system.rewrite_data_files(table => 'analytics.events', ...)`. Pick one engine to own the schedule (typically Spark, since ingestion already runs there). See [resource 17](17-iceberg-table-maintenance.md) for the full engine-split matrix.
...
- **Skipping snapshot expiry.** Iceberg keeps every old snapshot forever by default. Storage grows without bound. Run snapshot expiry weekly — on **Trino 467** that is `ALTER TABLE iceberg.analytics.events EXECUTE expire_snapshots(retention_threshold => '7d')`; on **Spark** it is `CALL iceberg.system.expire_snapshots(table => 'analytics.events', older_than => current_timestamp - interval '7' day, retain_last => 10)`. Note: Trino's minimum `retention_threshold` is `7d` by default (controlled by the `iceberg.expire_snapshots.min-retention` catalog property) — to expire snapshots younger than 7d you must lower the floor first or run from Spark. Pair with weekly `remove_orphan_files` to delete files no live snapshot references. See [resource 17](17-iceberg-table-maintenance.md) for the canonical maintenance schedule and engine-split rules.
